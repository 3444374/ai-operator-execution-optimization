...
| `feasibility/results/squad_capability_full_10570_20260805/` | SQuAD 全量 10570 行 DuckDB-ai gate（`--mode full` + `--strict-attribution` + 真实 service-config-hash + fail-closed）；含 server/pgvector 版本列的 per-row CSV + manifest | **fail-closed FAILURE**：10569/10570 成功、**1 行 cap=64 截断 NULL**（572700c8…，模型超长生成）；exactly-once/integrity/三 hash 一致/归因==10570 全通过；EM 80.32% / F1 89.36%（独立复算一致）。cap=64 对 SQuAD dev 的真实边界，按协议不抬 cap |
