...
| `feasibility/results/squad_database_e2e_duckdb_ai_20260805/` | SQuAD database-E2E runner 单臂实测（DuckDB-ai，全 10570，cap=64，strict-attribution，统一 sink `document_completions`）；`report.json` + `per_row_evidence.csv`（EM/F1 独立复算一致） | **database-E2E 边界已测**：wall 93.9s = scan 0.14 + construct 0.23 + adapter 93.2（op_jct 87.8）+ sink 0.26；**scan+construct+sink <1%（模型调用独占 99%）**；`correct_rows/s` 90.42（主 headline）、succ_rows/s 112.56、sunk 10570。状态 `failure`/`false`/`eligible_with_documented_failure`（1 偶发截断）。单臂测量，非排名；direct_client/project 臂待补 |
...
| `code/scripts/baselines/squad_database_e2e_runner.py` | SQuAD bounded-output **database-E2E 顶层 runner**（DuckDB-ai 臂）：一个计时墙包住持久表扫描→prompt 构造→DuckDB adapter（operator-only 时间戳保留）→统一 sink（`write_completions`/`document_completions`）；报告层记 `database_e2e_wall_s`+分段、runner 层算 `correct_rows_per_s`（主 headline）/`successful_rows_per_s`/failure rate；状态字段拆 `capability_gate_status`/`formal_run_gate_passed`/`comparison_admission`。不注入项目 credit/actor/backpressure | database-E2E 边界测量；正式数据库系统排名前提；`direct_client`/`project_static` 臂留 stub |
