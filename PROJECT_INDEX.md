...
| `code/scripts/experiments/run_saor_native_system_matched.py` | 本地系统级 matched matrix 编排与 readiness/执行器绑定 fail-closed 入口 | 平衡编排 8 个唯一物理臂；`--rehearsal` 每臂仅运行一个 warm-up、无 formal cell，且 runner 不授予 GPU formal 执行权限 |
| `code/scripts/analysis/summarize_saor_native_system_matched.py` | 已提交 matched matrix evidence 的纯离线两层 fail-closed 汇总 | 输出五臂 system、四臂 Project sanity、all-runs/Job/resource 与固定 validation；共享同一 SAOR 物理 run，native request tails 不支持时保留 `unavailable`，不判 winner、不授权 formal |
