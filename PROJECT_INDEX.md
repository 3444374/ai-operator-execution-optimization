...
| `code/src/baselines/text/products/` | DuckDB-ai / OceanBase 等数据库产品原生 adapter + direct_client 直连服务 control（按"每次模型调用的执行风格"共置；direct_client 的 provenance 是 `direct_client_control`，因用项目自写 `asyncio.Semaphore` 而标记 `custom_scheduling_code=True`，非产品 baseline） | 产品 adapter 需真实 SQL AI Function + capability gate 通过；direct_client 是 project-scheduled control，`formal_baseline_eligible=false` |
