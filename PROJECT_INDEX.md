...
| `code/scripts/baselines/squad_database_e2e_runner.py` | SQuAD bounded-output **database-E2E 顶层 runner**（DuckDB-ai + direct_client 臂）：一个计时墙包住持久表扫描→prompt 构造→adapter（DuckDB-ai 或 direct_client，operator-only 时间戳保留）→统一 sink（`write_completions`/`document_completions`）；报告层记 `database_e2e_wall_s`+分段、runner 层算 `correct_rows_per_s`（主 headline）/`successful_rows_per_s`/failure rate/`truncation_count`；状态字段解耦 `single_run_valid`/`formal_run_gate_passed`（单次恒 false）/`comparison_admission`。不注入项目 credit/actor/backpressure | database-E2E 边界测量；正式数据库系统排名前提；`direct_client` 臂已实现（per-request httpx async，固定并发 32，无项目调度），`project_static` 臂留 stub |
...
| `code/src/baselines/text/products/` | DuckDB-ai / OceanBase 等数据库产品原生 adapter + direct_client 直连服务 control（按"每次模型调用的执行风格"共置；direct_client 的 provenance 是 `direct_service_control`，非产品 baseline） | 产品 adapter 需真实 SQL AI Function + capability gate 通过；direct_client 是 control，`formal_baseline_eligible=false` |
| `code/src/baselines/text/products/direct_client.py` | direct_client 臂：httpx async + `asyncio.Semaphore(32)` 固定并发、per-request `/v1/chat/completions`，复用 `build_completion_request_body`；暴露 `finish_reason`/`completion_tokens`/per-request latency（DuckDB-ai 不暴露） | 揭示同 cap 下"截断的产品语义"差异（partial text vs NULL）；`direct_service_control`，非产品 baseline |
