...
| `deploy/autodl/saor_project_mechanism_formal_contract.json` | VTC-style 主参照、5% headline、吞吐/JCT/SLO/no-service 非劣、固定输出上界与 30s empirical repayment 的预注册决策合同 | `locked_pending_rehearsal/formal_authorized=false`；有效 final rehearsal 已通过并登记 validation/archive SHA，须独立复核后才能另行授权 formal |
