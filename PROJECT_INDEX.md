...
| `experiments/results/multicard_saturated_2048_20260806/README.md` | 多卡饱和三臂 1w+3f（bounded 天花板 + duckdb harness_sharded_diagnostic + project_static）；group 口径 project +2.55%/Welch p=0.0284（显著高于 duckdb harness，≠ 优于产品）；ADDENDUM 含 lb_rr@64（72480 不可审计，lbrr64 未提交）+ bounded collapse（ps8_collapse 已提交，group 口径 c64=36560/c128=24836） | 审计饱和对比、harness 身份订正、project 显著高 vs duckdb harness 的边界 |
| `experiments/results/multicard_rich_metric_2048_20260806/README.md` | 多卡 rich-metric 饱和 screening（committed aggregator 可复现）；身份 harness_sharded_diagnostic + 措辞订正 | 审计 cache-hot screening + EM/F1（2048 子集分母）+ 调度开销 35-37% |
...
| `experiments/results/multicard_lbrr_scale_ramp/README.md` | lb_rr 规模爬坡（64..10570，C_total=64，nginx round-robin，warmup_per_cell=false）；ttft 口径峰值 72934@2048，4096+ 坍塌；256 门禁全过（exactly-once + length=0 + nginx 8000/8001 对称） | 审计 lb_rr 完整规模曲线 + 单入口欠喂 + 4 臂 regime 一致；uncontrolled-cache 非 cache-controlled |
...
| `deploy/autodl/dual_gpu_duckdb_ai_capability_gate.example.json` | DuckDB `ai` community extension 的两 shard harness capability gate（历史文件名） | 每 shard 仍是一个 BASE_URL；只作 `harness_sharded_diagnostic`，不得称扩展原生双 endpoint 或进入产品原生主排名 |
