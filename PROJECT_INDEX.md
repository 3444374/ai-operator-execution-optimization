...
| `experiments/plans/state_aware_work_unit_evaluation_20260808.md` | 分阶段 work descriptor、四个开题最小方法证据与 §5.2 SAOR 唯一维护入口；当前范围为单租户多 Job/workload class，dynamic K parked；bounded-ready $0.125W_e$ 通过开发门但 observation/selector 归因待解 | 设计/调整动态调度前读；先做 project bounded-ready + FIFO/DRR/VTC-style/strict-priority 内部归因 gate，再决定 1+3 formal；多租户为后续层次化扩展、非当前 blocker，原生 baseline 不接该机制，reservation/4-Job 保持阻塞 |
...
| `experiments/results/state_aware_work_unit/saor_bounded_ready_gate_20260813/` | v0.5.2 project-owned bounded concrete-ready observation 双轮 GPU gate、跨 trace lifecycle join 与无损事件证据 | $0.125W_e$ 两轮全过、$0.25W_e$ 被 bulk guard 拒绝；post-hoc 审核要求先做 Project selector 内部归因 gate，原生 baseline 不接入。只完成 development rehearsal，formal/4-Job/reservation 尚未运行 |
| `experiments/results/state_aware_work_unit/saor_matched_ready_selector_rehearsal_20260813/` | frozen-static + bounded-ready FIFO/DRR/VTC-style/strict-priority/SAOR 双轮 Project selector 归因、completion-accounted lag 与完整 compact evidence | 12/12 cell、0 incident；SAOR 为效率—前台 tail 的观测非支配折中点，但 DRR/VTC-style 已满足 30s fg SLO 且吞吐/bulk JCT/最长无服务更好。固定顺序 n=2，`selector_victory_decided=false`、`formal_authorized=false`；下一步做原生系统 matched comparison 与 single-head + shared FIFO bridge |
...
| `research/saor_model_scenario_audit_20260811.md` | SAOR 数学审计：capacity-only 负结果、fixed-envelope release、release-only 上界；§12 定义 $H_B/W_e$ priority/debt、事件证据、bounded-ready 结果与归因门 | 设计后继、解释 static/strict-priority 权衡或撰写定理前读；$0.125W_e$ 仅 development candidate，Project 内部同 ready-window 归因 gate 前不称 selector 已验证或证明完成 |
...
| `deploy/autodl/saor_bounded_ready.example.json` | Project 独立 ready-set observation 四臂开发模板；bounded 两臂使用 `saor_bounded_ready`，窗口由签名化 K/W 派生 | 双轮 gate 已完成：$0.125W_e$ 冻结候选、$0.25W_e$ 拒绝。模板只作开发复现；Project 内部同 ready-window 归因门前不启动 formal，原生 baseline 不复用 |
| `deploy/autodl/saor_matched_ready_selector_ablation.example.json` | Project frozen-static reference + 同一 bounded-ready request/work/logical-bytes 合同下 FIFO、DRR、external VTC-style、strict-priority 与 guarded-debt 六臂归因模板 | 仅作 Project internal selector ablation 的 1--2 轮 rehearsal；不含原生 baseline，通过前禁止 formal |
| `deploy/autodl/saor_ready_observation_bridge.example.json` | Project frozen-static/single-head shared FIFO/bounded-ready FIFO 三臂 observation bridge | 用同 selector 依次隔离共享容量和 bounded-ready exposure；仅作 1--2 轮 rehearsal，不含原生 baseline、不授权 formal |
