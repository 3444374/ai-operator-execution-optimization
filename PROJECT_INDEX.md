...
| `experiments/plans/state_aware_work_unit_evaluation_20260808.md` | 分阶段 work descriptor、四个开题最小方法证据与 §5.2 SAOR 唯一维护入口；dynamic K parked；bounded-ready $0.125W_e$ 通过开发门但 observation/selector 归因待解 | 设计/调整动态调度前读；先做同 ready-window FIFO/DRR/VTC/strict-priority 归因 gate，再决定 1+3 formal；reservation/4-Job 保持阻塞 |
...
| `experiments/results/state_aware_work_unit/saor_bounded_ready_gate_20260813/` | v0.5.2 bounded concrete-ready observation 双轮 GPU gate、跨 trace lifecycle join 与无损事件证据 | $0.125W_e$ 两轮全过、$0.25W_e$ 被 bulk guard 拒绝；post-hoc 审核要求先做 matched-observation selector gate。只完成 development rehearsal，formal/4-Job/reservation 尚未运行 |
...
| `research/evaluation_metrics_survey_20260731.md` | AI 算子/推理服务文献 + 数据库厂商指标调研；含 equal-share/differentiated-service、公平 backlog、matched-observation baseline 与缓冲成本合同 | 设计多 Job 指标、selector 因果对照或判断公平/非劣时读；不能只用 Jain、聚合吞吐或 active K/W |
...
| `research/saor_model_scenario_audit_20260811.md` | SAOR 数学审计：capacity-only 负结果、fixed-envelope release、release-only 上界；§12 定义 $H_B/W_e$ priority/debt、事件证据、bounded-ready 结果与归因门 | 设计后继、解释 static/strict-priority 权衡或撰写定理前读；$0.125W_e$ 仅 development candidate，matched-ready gate 前不称 selector 已验证或证明完成 |
...
| `deploy/autodl/saor_bounded_ready.example.json` | 独立 ready-set observation 四臂开发模板；bounded 两臂使用 `saor_bounded_ready`，窗口由签名化 K/W 派生 | 双轮 gate 已完成：$0.125W_e$ 冻结候选、$0.25W_e$ 拒绝。模板只作开发复现；matched-ready 归因门前不启动 formal |
