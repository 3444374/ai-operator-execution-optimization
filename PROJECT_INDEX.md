...
| `experiments/results/opening_fourjob_interference_20260809/{README.md,data/combined/}` | 1-short+3-long 的 Project full/quarter/static/shared 与三条原生 single/four-job 正式报告、120-row逐Job/75-row组级/18-row阶段紧凑数据 | 分离quota-only、真实竞争与shared调度影响；查看四Job状态/公平性/long离散度或待画图输入时读，不作跨框架绝对排名 |
