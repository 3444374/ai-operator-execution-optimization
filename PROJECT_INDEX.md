...
| `code/scripts/baselines/squad_database_e2e_runner.py` | SQuAD bounded-output database-E2E 顶层 runner（DuckDB-ai + direct_client + project_static）。project_static shell-out profiler，使用独立 completion evidence、实际 source-scan prompt fingerprints、DB/importer 完整性读取与 sink readback 形成非循环证据链；冻结 token budget / per-endpoint K / active-work / actor topology与请求语义 | 三臂可运行；project_static 在统一计时墙完成前 `blocked_unified_timing_boundary`，只能做正确性门禁，不能进入跨臂正式排名 |
