...
| `experiments/results/state_aware_work_unit/saor_feeding_gap_diagnostic_20260817/` | main@345bee2f 冻结 D0/D1/P0 feeding-gap diagnostic 的 fail-closed 中止报告与 compact evidence（group_runs/manifest/clean gate/contract snapshot） | 10/12 cell 后 D0 formal rep3 单请求 `ReadError` 触发预注册 incident 中止；summarizer 未运行、四判决未产生；描述性 D1/D0≈0.953、P0/D1≈0.989 仅供方向参考。root/archive SHA `f4b9793d…` 完整保留，`locked_failed_feeding` 不变，重跑决策待定 |
...
| `deploy/autodl/saor_native_system_matched.example.json` | 五臂 2-Job 系统级 matched comparison 的总编排合同与 placeholder-safe readiness gate | 必须与 native/project 两个执行器配置一同提供；模板保持 `formal_authorized=false`，只允许先做静态审计 |
...
| `deploy/autodl/saor_native_system_matched_project.example.json` | Project frozen-static、bounded-ready FIFO/DRR/VTC-style/SAOR 五个可映射场景配置 | 总编排器仅选择其中 frozen-static 与 SAOR 两臂，并校验共享 K/W、ready-state、校准和服务签名；不是 GPU 执行授权 |
