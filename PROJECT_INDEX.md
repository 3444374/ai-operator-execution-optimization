...
| `code/scripts/baselines/squad_database_e2e_runner.py` | SQuAD bounded-output **database-E2E 顶层 runner**（DuckDB-ai + direct_client + project_static 三臂）：duckdb_ai/direct_client 是进程内 adapter（一个计时墙包住持久表扫描→prompt 构造→adapter→统一 sink `write_completions`/`document_completions`）；**project_static 是 shell-out**——runner 在通用 scan 前分流，子进程调用 `postgres_ai_operator_profile.py` 跑冻结最佳静态 K，profiler 独占 scan+organize+model+sink，wrapper 合并 request-trace(时间戳) + `document_completions` readback(output_text) → `BaselineRequestResult`；报告层记 `database_e2e_wall_s`+分段、runner 层算 `correct_rows_per_s`（主 headline）/`successful_rows_per_s`/failure rate/`truncation_count`；状态字段解耦 `single_run_valid`/`formal_run_gate_passed`（单次恒 false）/`comparison_admission` | database-E2E 边界测量；正式数据库系统排名前提；三臂均已实现（project_static 经 profiler 子进程；必传 `--token-budget`/`--project-max-inflight`/`--project-actor-workers`/`--project-ray-actor-max-concurrency`，强制 `actor_workers × concurrency >= K` 故 effective K == 声明 K；run-scoped completion evidence 使 sink readback 非循环；profiler `e2e_s` 比进程内臂 wall 更宽，计时段不直接可比）|
...
| `code/src/baselines/text/products/project_static.py` | project_static 臂 wrapper（无连接）：子进程调用 `postgres_ai_operator_profile.py`（冻结静态 K + token-budget + actor 拓扑），profiler 独占 scan+organize+model+sink；wrapper 从 profiler 的 run-scoped completion-evidence CSV（`output_text` 独立于 sink）+ summary CSV 组装 `BaselineRequestResult`。强制 `actor_workers × concurrency >= max_inflight`（effective K == 声明 K，无静默夹）；冻结 `temperature=0`/`http_transport=httpx_async`/prefix-caching | 论文方法 under test；`project_scheduled_method` + `custom_scheduling_code=True`，非 baseline/control；`scheduler_owner=project_ray_static_k`（不声称 active-work） |
...
| `code/tests/baselines/text/test_project_static.py` | project_static wrapper 纯函数单测（ProjectStaticConfig 校验含 effective-K 拓扑约束/build_profiler_argv 锁定 effective K·temperature·transport·prefix-caching·no-manifest/read_completion_evidence/read_summary_timing dry_run 过滤+last-wins/merge_results failed-row started 回退）| 修改 `project_static.py` argv 或 CSV 合并后运行 |
| `code/tests/observability/test_completion_evidence_trace.py` | profiler `write_completion_evidence` emit 单测（output_text 按 doc_id 展平/failed-row 空/列 schema）| 修改 `traces.write_completion_evidence` 后运行 |
...
| `deploy/autodl/dual_gpu_squad_database_e2e.example.json` | SQuAD bounded-output database-E2E runner（DuckDB-ai 臂）冻结服务配置 + 运行模板；冻结 endpoint/model/cap=64/DuckDB 并发 32/sink/归因，并归档支撑 `--service-config-hash` 的真实 vLLM 配置（launch cmd/revision/dtype/parallelism/VRAM/env，REPLACE_ME 待正式前填）| 正式 database-E2E 排名前的服务 provenance 合同；`correct_rows_per_s` 为主 headline，不以 raw rows/s 排名 |
