...
| `feasibility/results/squad_capability_256_v3_20260805/` | SQuAD 256 行 DuckDB-ai capability gate v3（原始 whitespace 词数的 max-答案分桶 + largest-remainder、workload 完整性 fail-closed、vLLM counter 可归因、full-set exactly-once、脱敏、七步 README） | EM 80.86% / F1 89.86%、attribution=attributable、workload_integrity=verified；缺 sample manifest 且“normalized”措辞不准确，保留作单臂 capability evidence，后续门禁须用修正版重跑 |
