...
| `experiments/plans/state_aware_work_unit_evaluation_20260808.md` | 分阶段 work descriptor、四个开题最小方法证据与 §5.2 SAOR 唯一维护入口；fixed-envelope formal 已完成 resolution-aware v2 validation，dynamic K parked，v0.5 通用有界词典序设计已冻结 | 设计/调整动态调度前读；当前 SAOR 为 `formal-valid/not-promoted`，首轮只做 2-Job 的 0.125K/0.25K actual-work debt cap 与队首定向 reclaim barrier，reservation 作通过后的鲁棒性消融，两 Job 未闭环前不扩 4-Job |
...
| `deploy/autodl/saor_bounded_priority.example.json` | static、原 SAOR、0.125K/0.25K actual-work fairness-debt cap 四臂开发模板；显式 per-Job priority/SLO/window/cap | 服务器恢复并重新通过 runtime preflight 后只跑两轮独立 rehearsal；lossless release-event ledger 是机制真值，未完成前不启动 formal |
