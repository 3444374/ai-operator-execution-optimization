...
| `experiments/results/state_aware_work_unit/saor_native_system_matched_manifest_20260819/` | 八臂 matched comparison 的两 Job SHA/512-row identity + 独立授权模板；完整 prompts 留在 Git 外 | readiness 逐 Job 校验并验证 combined=Job0+Job1；merge/rehearsal 均不构成 formal 授权 |
...
| `deploy/autodl/saor_native_system_matched.example.json` | 八臂 2-Job 系统级 matched comparison 的总编排合同与 readiness gate；冻结 Git 外 `matched_long_short_1024.jsonl` 及 Job0/Job1 的 SHA/512 行边界和共同 endpoint 映射 | 必须与 native/project 两个执行器配置一同提供；`gpu_formal_locally_authorized=false` 保持，merge/rehearsal 均不授权 formal，独立审核后仍需单独签发 `--formal-authorization` artifact |
| `deploy/autodl/saor_native_system_matched_native.example.json` | Daft Native、Daft Ray、Ray Data HTTP 三个原生执行器的显式 source/service/protocol/manifest 合同 | 原生臂保留各自 scheduler ownership；缺失或漂移任一矩阵字段时总编排器 fail closed |
| `deploy/autodl/saor_native_system_matched_project.example.json` | Project frozen-static、bounded-ready FIFO/DRR/VTC-style/SAOR 五个场景配置 | 五臂执行完整 512+512 行并冻结共同 endpoint、K/W、token-budget、actor topology 与校准；不是原生 baseline，也不是 GPU 执行授权 |
...
| `code/src/experiments/saor/native_system_summary.py` | native-system matched 封存身份校验、失败证据保留与两层汇总核心 | 重算 authorization/snapshot/config/manifest/service/scheduler/schedule/cell identity；失败只发布带 reason 的 all-runs，不发布性能排名 |
