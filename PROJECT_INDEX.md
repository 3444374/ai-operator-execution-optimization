...
| `deploy/autodl/saor_native_system_matched.example.json` | 五臂 2-Job database-E2E matched comparison 的总编排合同；冻结 typed Job release、manifest/SHA、MFU denominator 与共同 endpoint | native 三臂不接 Project 控制；static/SAOR 分工明确；merge/rehearsal 均不授权 formal |
...
| `code/src/experiments/saor/native_system_preflight.py`、`native_system_artifacts.py` | SAOR live system-preflight producer 与 smoke/rehearsal root 深校验核心 | 不信任自报 passed；核验 Ray 状态及实际 CUDA PID、重算 bounded raw evidence/cell artifact SHA/sink，并将三 config SHA 与 frozen native provenance 绑定到完整 tar 镜像 |
