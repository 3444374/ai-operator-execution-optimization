...
| `experiments/plans/state_aware_work_unit_evaluation_20260808.md` | 分阶段 work descriptor、四个开题最小方法证据与 §5.2 SAOR 唯一维护入口；当前范围为单租户多 Job/workload class，dynamic K parked；bounded-ready、同窗口 selector 归因与 observation bridge 均已完成 development rehearsal | 设计/调整动态调度前读；SAOR 当前是观测非支配折中且 `formal_authorized=false`，下一步只做同一 2-Job native-system matched comparison；多租户非当前 blocker，原生 baseline 不接 bounded-ready，reservation/4-Job 保持后置 |
...
| `deploy/autodl/saor_project_mechanism_formal_contract.json` | VTC-style 主参照、5% headline、吞吐/JCT/SLO/no-service 非劣与 30s empirical repayment 的预注册决策合同 | `locked_pending_rehearsal/formal_authorized=false`；先重跑 final rehearsal 并登记 validation SHA |
