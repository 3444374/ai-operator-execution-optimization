...
| `experiments/plans/state_aware_work_unit_evaluation_20260808.md` | 分阶段 work descriptor、四个开题最小方法证据与 §5.2 SAOR 唯一维护入口；当前范围为单租户多 Job/workload class，dynamic K parked；projected-debt 最终六臂 rehearsal 已通过 | 设计/调整动态调度前读；SAOR 为待 formal 重复的 Pareto 候选且 `formal_authorized=false`，下一步先独立审核/登记 rehearsal validation，再分别推进 1+3 Project mechanism formal 与 2-Job native-system matched comparison；原生 baseline 不接 bounded-ready，reservation/4-Job 保持后置 |
