...
当前 `REL_18_3` extension/planner-visible `SemMap` 与同步 UDS recording slice 已通过功能测试；最短期
任务是拆分 PostgreSQL scan/pump、neutral provider port 与 recording/UDS adapters，然后实现 exact
`SemFilter` 和最小第二 physical path。载体审查决定是否需要最小 core patch；accepted-prefix、多在途、
