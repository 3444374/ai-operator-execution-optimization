...
| 单流增量执行近期规格 | [详细设计](experiments/plans/semloom_incremental_session_design.md) | 单步操作、输入/结果所有权、进展和资源账本；受控核心及有界异步HTTP真实smoke已验证，生产协议、批处理和PG接入待进行；[方法续体入口](code/src/semantic_methods/README.md)提供逐行方法适配；组织窗口已接入独立单成员请求路径，PG及单请求多成员待验证 |
