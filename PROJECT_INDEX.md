...
| `code/src/baselines/text/products/project_static.py` | project_static 臂 wrapper：子进程调用 `postgres_ai_operator_profile.py`（冻结静态 K + token-budget），profiler 独占 scan+organize+model+sink；wrapper 合并 request-trace(时间戳/status/finish_reason) + `document_completions` readback(output_text) → `BaselineRequestResult`；不 scan/sink 自己（runner 在 scan 前分流）| 论文方法 under test；`project_scheduled_method` + `custom_scheduling_code=True`，非 baseline/control |
...
| `code/tests/baselines/text/test_project_static.py` | project_static wrapper 纯函数单测（ProjectStaticConfig 校验/build_profiler_argv 冻结标志/read_request_trace/read_summary_timing dry_run 过滤+last-wins/read_output_text/merge_results failed-row started 回退）| 修改 `project_static.py` argv 或 CSV 合并后运行 |
