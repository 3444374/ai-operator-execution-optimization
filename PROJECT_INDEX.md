...
| `experiments/results/state_aware_work_unit/saor_project_feeding_ceiling_20260814/` | 当前完整签名 direct bounded ceiling、compact evidence 与完整 archive | direct 13,684.90 tok/s 对封存 SAOR 12,713.03，feeding=92.898%<95%；evidence valid、formal 停止 |
...
| `code/src/experiments/saor/feeding_ceiling.py` | SAOR 六臂 reference 与单 cell direct bounded ceiling 的逐字段匹配及 feeding 汇总合同 | evidence validity 与 ≥95% gate 分离；direct 不接 bounded-ready/credit |
...
| `code/scripts/analysis/summarize_saor_feeding_ceiling.py` | 对封存 SAOR cell 与当前 direct ceiling 做 signature/evidence/throughput 比较 | ratio<0.95 写 valid `failed_feeding`，不自动授权 formal |
