...
| `experiments/plans/state_aware_work_unit_evaluation_20260808.md` | 分阶段 work descriptor、四个开题最小方法证据与 §5.2 SAOR 唯一维护入口；dynamic K parked；bounded-ready $0.125W_e$ 通过开发门但 observation/selector 归因待解 | 设计/调整动态调度前读；先做 project bounded-ready + FIFO/DRR/VTC-style/strict-priority 内部归因 gate，再决定 1+3 formal；原生 baseline 不接该机制，reservation/4-Job 保持阻塞 |
...
| `research/evaluation_metrics_survey_20260731.md` | AI 算子/推理服务文献 + 数据库厂商指标调研；含 equal-share/differentiated-service、公平 backlog、原生 baseline 与同 observation 的项目内部消融、缓冲成本合同 | 设计多 Job 指标、selector 因果对照或判断公平/非劣时读；不能只用 Jain、聚合吞吐或 active K/W |
...
| `code/scripts/analysis/audit_saor_formal_readiness.py` | 固定包络 SAOR formal/development 与六臂 matched-ready selector internal ablation 的 FCFS/calibration/manifest/request 合同 fail-closed 静态预检 | 服务器运行前只读执行；归因模板用 `--profile matched_ready_selector_ablation`，不发送模型请求且不把内部消融称原生 baseline |
