...
| `code/scripts/analysis/audit_saor_native_system_matched.py`、`audit_vllm_0251_source.py`、`validate_saor_native_system_rehearsal.py` | 四阶段 readiness、冻结 vLLM Python exact-source 与 completed rehearsal 封存入口 | 外层使用 DRIVER_PYTHON、vLLM 审计使用显式子进程；只有 static/service/system/smoke 全过才 rehearsal-ready，formal 再绑定 validation/root/archive |
