...
| `experiments/results/multicard_scale_ramp_enhanced_20260807/README.md` | bounded HTTP + harness-pre-split DuckDB AI 九档三重复 enhanced ramp；含 during-cell gauges、identity、失败 cell 和可重建 aggregate 的紧凑证据 | bounded 27/27、DuckDB 22/27；两路径身份/计时粒度分开，逐请求输出与日志留服务器 |
| `experiments/results/multicard_lbrr_scale_ramp_enhanced_20260807/README.md` | 单 DuckDB→nginx round-robin→双 vLLM 九档三重复 enhanced ramp；27/27，含 backend balance、gauges、TTFT/ITL 与资源证据 | `gateway_system_diagnostic` + query-barrier 口径；不作 DuckDB 原生多 endpoint 排名 |
...
| `experiments/plans/full_grid_sweep_plan.md` | 4 臂（bounded/duckdb/lb_rr/project）× 规模 × 并发全网格 DESIGN 计划 + project_static 2-endpoint hang 根因（ray_adapter.py:273 无界 ray.get + project_static.py:411 无界 subprocess）/三层修复/256 验证门/十字切片推荐/成本排序 | 规划全网格扫掠、修 project_static hang 前必读；纯规划文档，未执行 |
...
| `opening/slides/opening_defense_20260812_v7.pptx` | 20 页中文开题答辩历史基线 | 保留作为 v8 增量编辑的来源 |
...
| `opening/slides/update_opening_defense_v8_artifact_tool.mjs` | 从 v7 原位替换第 2–4 页并保留模板、其余页面和 notes | 重建或审计当前开题 PPT |
...
| `opening/slides/build_opening_defense_v9_artifact_tool.mjs` | 基于学校模板、项目材料和对外叙事生成 26 页 v9；自动生成图中对外文字版本、讲稿来源与逐页渲染 | 重建或继续修改当前开题 PPT |
| `opening/slides/opening_defense_20260812_v9.pptx` | 当前 26 页对外开题答辩版；合并重复文献、验证与结尾页面，专业英文名首次附中文解释 | 现场汇报与逐页审阅；正式答辩前在目标 PowerPoint/WPS 环境再打开一次 |
...
| `figures/audit/palimpzest_deep_reading_figures_audit_20260824.md` | Palimpzest 精读配图选择与裁剪审计 | 核对 arXiv v2 Figure 1–7 的独立论证角色、CIDR 版本边界、来源页码、SHA256、裁剪方式和视觉 QA |
| `figures/audit/abacus_deep_reading_figures_audit_20260824.md` | Abacus 精读配图选择与引用审计 | 核对 PVLDB 正式版 Figure 1–8 的独立论证角色、错误路径修复、来源页码、SHA256、图像一致性和证据边界 |
| `figures/audit/sema_deep_reading_figures_audit_20260824.md` | Sema 精读配图选择与裁剪审计 | 核对 arXiv v1 正文 Figure 1–8 的独立讲解价值、正式 venue 边界、来源页码、SHA256、裁剪方式和视觉 QA |
...
| `experiments/results/saor_priority_reachability_smoke_20260812_91ffcaa/` | static/SAOR/foreground strict-priority 两轮 release-only 上界短测 | 查为什么 current SAOR 的 soft score 效果有限；strict-priority fg P99 14.27s、SLO 0%，仅 development smoke，不作 formal/proposed 排名 |
| `experiments/results/state_aware_work_unit/saor_bounded_priority_gate_20260813/` | v0.5.1 static/SAOR/$0.125W_e$/$0.25W_e$ 双轮 GPU development gate、lossless event 与请求时序交叉验证；`raw/regression_20260814/` 补存门禁修复的 compact evidence 与完整 archive SHA | 两 cap 均未过 fg 门；$0.25W_e$ 第 2 轮及 08-14 回归均无 debt recovery 被 fail-closed。回归不计性能重复，且早于 completion/repayment ledger，不能证明 repayment bound |
| `experiments/results/state_aware_work_unit/saor_bounded_ready_gate_20260813/` | v0.5.2 project-owned bounded concrete-ready observation 双轮 GPU gate、跨 trace lifecycle join 与无损事件证据 | $0.125W_e$ 两轮全过、$0.25W_e$ 被 bulk guard 拒绝；后续 Project selector 内部归因与 FIFO observation bridge 已完成。只属 development rehearsal，原生 baseline 不接入，formal/4-Job/reservation 尚未运行 |
| `experiments/results/state_aware_work_unit/saor_matched_ready_selector_rehearsal_20260813/` | frozen-static + bounded-ready FIFO/DRR/VTC-style/strict-priority/SAOR 双轮 Project selector 归因、completion-accounted lag 与完整 compact evidence | 12/12 cell、0 incident；SAOR 为效率—前台 tail 的观测非支配折中点，但 DRR/VTC-style 已满足 30s fg SLO 且吞吐/bulk JCT/最长无服务更好。固定顺序 n=2，`selector_victory_decided=false`、`formal_authorized=false`；observation bridge 已完成，下一步只做原生系统 matched comparison |
| `experiments/results/state_aware_work_unit/saor_ready_observation_bridge_rehearsal_20260813/` | frozen-static→single-head shared FIFO→bounded-ready FIFO 双轮桥接与 compact evidence | 6/6 cell、0 incident；共享容量解释效率提升/前台隔离损失，bounded-ready 在同 FIFO 下继续提高效率并部分恢复前台，但仍未满足 30s SLO。bridge 已完成；下一步只剩原生系统 matched comparison |
| `experiments/results/state_aware_work_unit/saor_project_mechanism_rehearsal_20260814/` | residual-aware projected-debt 六臂最终 rehearsal 报告、compact JSON/CSV 与完整 113-file archive | `63d17300` root 6/6、0 incident、validation passed；固定 admission cap=256 的 6,144-request audit 通过，15/15 repayment completed、P95 3.234s、1,108/1,108 projection 一致。机制证据有效；当前签名 feeding-negative 阻断 formal |
| `experiments/results/state_aware_work_unit/saor_project_feeding_ceiling_20260814/` | 当前完整签名 direct bounded ceiling、identity-bound compact evidence 与完整 archive | 一次性 gate：direct 13,684.90 tok/s 对封存 SAOR 12,713.03，feeding=92.898%<95%；artifact/arithmetic valid、PG/Ray clean evidence unavailable、formal 停止 |
| `experiments/results/state_aware_work_unit/saor_native_system_matched_manifest_20260819/` | 五臂 matched comparison 复用的两 Job SHA/512-row identity + 独立授权模板；完整 prompts 留在 Git 外 | readiness 逐 Job 校验并验证 combined=Job0+Job1；merge/rehearsal 均不构成 formal 授权 |
| `experiments/results/state_aware_work_unit/saor_native_system_matched_calibration_20260819/` | Project token-budget6144/K128/W65536/actor selection + native 三臂 identity（Daft C1/B1、Ray Data C8/B16） | runner 校验每文件 SHA 与实际 Project batching/actor、native adapter/C/B；vendor default/单次 screen 不夸大为统计最优 |
| `experiments/results/state_aware_work_unit/saor_native_system_matched_rehearsal_20260821/` | `862d0008` gateway 前五臂 smoke/rehearsal 的 compact validation 与诊断观测 | 只证明五臂可运行/封存闭环；原生 request tail/fairness unavailable，已被新 gateway 合同标记为待重跑，formal 未运行 |
| `experiments/results/state_aware_work_unit/saor_native_system_matched_gateway_rehearsal_20260821/` | `93271012` 共同 gateway 后五臂 T0--T4、request SLO、actual-work fairness、Project A/B 与独立 validation | 五臂 5/5 rehearsal passed；SAOR 相对 static 效率提高但 P99/lag/Jain 变差，single-run 不判显著性；pre/post isolation partial，formal 未运行 |
| `code/src/observability/request_gateway.py` | 五臂共同的 Job-labelled observation-only HTTP 透传层 | 一次转发、body SHA/retry/actual usage/时钟证据；不得加入 queue、admission、retry、batch 或 route choice |
| `code/src/observability/README.md` | 被动观测层职责与禁止接管执行策略的边界 | 修改 gateway/metrics/profiling 前读；queue/backpressure/batching 不得进入观测层 |
| `code/src/experiments/saor/native_system_observation.py` | 从 gateway trace 统一推导 T0--T4、P99/SLO、actual-work fairness 与 within-run isolation | 五臂共同口径；full-solo slowdown 需独立 matched-solo control，不由本模块补造 |
...
| `deploy/autodl/saor_active_set_release.example.json` | 固定 calibration envelope 下 direct/static/FIFO/DRR/external-VTC/SAOR-Release 六臂 active-set + project/direct matched-solo 十 scenario 模板 | 跑 `bulk-only → foreground-arrival → foreground-drain` 决定性 control gate；direct 与 Project arms 分身份解释，K/work/actor/budget 全从本机 selection contract 注入；先 readiness audit 和 rehearsal |
...
| `experiments/results/opening_fourjob_interference_20260809/{README.md,data/combined/}` | 1-short+3-long 的 Project full/quarter/static/shared 与三条原生 single/four-job 正式报告、120-row逐Job/75-row组级/18-row阶段紧凑数据，含 isolated-normalized progress fairness CSV | 分离quota-only、真实竞争与shared调度影响；报告full/reserved/static三反事实与baseline-relative empirical Pareto边界。compact数据不能补event-level lag/starvation，不作理论公平性质或跨框架绝对排名 |
...
| `code/scripts/baselines/run_text_native_multijob.py` / `code/src/baselines/text/orchestration/native_multijob.py` | Daft Native/Ray、Ray Data 原生两 job 错峰薄编排 | 每 job 启动双 endpoint official shard，禁止项目控制；保存 barrier、服务/vLLM/GPU 证据并验证 Ray worker nofile；matched archive 可封存 Job manifest、相对化 shard locator |
...
| `code/src/baselines/common/redact.py` | 命令行、连接串与异常内 named-secret 脱敏共享模块（`redact_argument_list`/`redact_database_url`/`redact_text`）| 任何把 sys.argv / DSN / 异常文本写入 evidence 的 gate 或 runner 复用，禁止第三份拷贝 |
| `code/src/baselines/common/database_identity.py` | PostgreSQL/pgvector actual-version typed identity 与跨记录一致性验证 | shard、Job、cell、矩阵汇总复用；拒绝空值/sentinel/版本漂移，入口脚本不得复制业务规则 |
...
| `experiments/plans/lotus_semantic_frontend_execution_integration_20260821.md` | LOTUS `sem_map` 语义实现与物理 backend 子计划 | 细化 v1.2.4 AST/prompt/output parity、native LM 与 project backend；服从 PostgreSQL 内置算子主计划，未修改 DataConnector 路径必须保留为外部 LOTUS 产品 baseline |
...
| `code/src/scheduling/submission_control/saor.py` | SAOR finite-action DPP、weighted common-backlog fairness debt 与显式预测 service 合同 | 修改控制律前运行对应纯策略测试；现有 formal 中 `slo_weight=0`，只支持 fairness/release 方向性定位，不支持完整 DPP/SLO 或 winner claim |
...
| `code/tests/experiments/test_saor_feeding_gap_diagnostic.py` | D0/D1/P0 合同、旧负判决锁、四路归因、结构化 clean gate 与完整 synthetic matrix 回归 | 修改 feeding-gap runner、preflight、summary 或 0.95 判决后运行 |
...
| `code/scripts/analysis/audit_saor_formal_readiness.py` | 固定包络 SAOR formal/development、六臂 matched-ready selector internal ablation 与三臂 ready-observation bridge 的 FCFS/calibration/manifest/request 合同 fail-closed 静态预检 | 服务器运行前只读执行；归因模板用 `--profile matched_ready_selector_ablation` 或 `--profile ready_observation_bridge`，不发送模型请求且不把内部消融称原生 baseline |
| `code/scripts/analysis/audit_chat_prompt_overhead.py` | 从 request/submission 原始 CSV 独立重算 chat template 每请求 prompt-token overhead、检查一请求一 submission/一致性并保存输入 SHA | 模型/template/protocol 签名变化或 estimated-work 上界失败时运行；raw prompt evidence 不加 overhead，调度 effective work 才加入校准项 |
| `code/scripts/analysis/summarize_saor_matched_ready_ablation.py` | 六臂 Project matched-ready selector internal ablation 的身份、correctness、全 Job ready lifecycle 与 proposed 机制证据 fail-closed 汇总 | 只生成归因复核输入；固定不判 selector 胜出、不授权 formal，原生 baseline 数量为 0 |
| `code/src/experiments/saor/project_mechanism_formal.py` | Project mechanism 六臂身份、位置平衡 schedule、formal authorization 与 completion-fairness raw join 的共享合同模块 | 把 rehearsal/formal wrapper 和离线汇总从执行细节解耦；当前合同拒绝 formal |
| `code/src/experiments/saor/feeding_ceiling.py` | SAOR 六臂 reference 与单 cell direct ceiling 的 frozen-identity/feeding 汇总合同 | 绑定 group/manifest/runtime snapshot/validation/archive SHA 后再分离 artifact validity 与 ≥95% gate；direct 不接 bounded-ready/credit |
| `code/src/experiments/saor/feeding_gap_diagnostic.py` | D0 direct K-only、D1 direct K+W、P0 bounded-ready FIFO K+W 的独立差距归因合同与 fail-closed 汇总 | 对 1+3 配对 repeat 计算 D1/D0、P0/D1；缺 occupancy/全组件证据即 invalid，任何分类均保留旧 `locked_failed_feeding` |
| `code/src/experiments/saor/feeding_gap_preflight.py` | feeding-gap diagnostic 专用的 PostgreSQL、Ray namespace 与全部 endpoint 结构化 clean gate | 只证明跑前无已知残留/服务空闲，不代替环境签名、性能或 exactly-once 证据 |
| `code/src/experiments/shared_vllm/cli.py` | shared-vLLM runner 的 typed CLI 参数解析 | 薄 CLI 共同复用，禁止 experiment script 反向导入另一 script |
| `code/src/experiments/shared_vllm/preflight.py` | endpoint health + running/waiting idle gate | runner 复用实时门禁；当前历史 ceiling 未保存结构化 PG/Ray clean record |
| `code/scripts/experiments/run_saor_project_mechanism.py` | Project mechanism audit-aware runner；绑定 evaluation contract、matched-ready readiness 与 output-root 合同快照 | 当前只允许 `--rehearsal`；未经 validation SHA 授权的 formal fail closed |
| `code/scripts/experiments/run_saor_feeding_ceiling.py` | 当前签名 direct ceiling wrapper；运行前校验六臂 reference、artifact identity 与 typed work cost | 只允许一个 rehearsal cell；不得扩成 selector baseline |
| `code/scripts/experiments/run_saor_feeding_gap_diagnostic.py` | 绑定旧负合同和新三臂 diagnostic contract 的 1+3 runner | 不接 SAOR formal；先写 PG/Ray/endpoint clean gate，D1 仅为无 Job fairness 的 K+W diagnostic control |
| `code/scripts/analysis/summarize_saor_project_mechanism_formal.py` | 1+3 六臂 Project mechanism 正式证据、completion service lag/no-service、debt repayment 与 constrained-Pareto gate 汇总 | evidence validity 与 claim gate 分离；static fairness 为 N/A，严格优先级只作边界 control，原生 baseline 数量为 0 |
| `code/scripts/analysis/summarize_saor_feeding_ceiling.py` | 对封存 SAOR cell 与当前 direct ceiling 做 identity-bound throughput 比较 | 强制 evaluation contract + 两个 archive；group/manifest/snapshot/validation/archive SHA 全匹配后才判 artifact valid，ratio<0.95 不自动授权 formal |
| `code/scripts/analysis/summarize_saor_feeding_gap_diagnostic.py` | 三臂 diagnostic 的 paired-ratio、occupancy、Ray/vLLM/MFU/tail/SLO/energy 汇总入口 | 输出四种预注册归因之一或 invalid evidence，不修改旧 formal/feeding 合同 |
| `code/scripts/analysis/summarize_saor_ready_observation_bridge.py` | frozen-static→single-head shared FIFO→bounded-ready FIFO 三臂桥接的身份、correctness、observation evidence 与两段 raw effect 汇总 | 分离共享容量和 ready exposure；固定两个效应均未判决、不授权 formal，原生 baseline 数量为 0 |
| `deploy/autodl/saor_bounded_priority.example.json` | static、原 SAOR、$0.125W_e$/$0.25W_e$ actual-work fairness-debt cap 四臂开发模板；显式 per-Job priority/SLO/window/cap | 历史 scenario ID 仍写 K，但 fraction 实际乘 endpoint work limit；2026-08-13 双轮未晋级，保留单-head 回归语义 |
| `deploy/autodl/saor_bounded_ready.example.json` | Project 独立 ready-set observation 四臂开发模板；bounded 两臂使用 `saor_bounded_ready`，窗口由签名化 K/W 派生 | 双轮 gate 已完成：$0.125W_e$ 冻结候选、$0.25W_e$ 拒绝。模板只作开发复现；后续同窗口归因与 bridge 均未授权 formal，原生 baseline 不复用 |
| `deploy/autodl/saor_matched_ready_selector_ablation.example.json` | Project frozen-static reference + 同一 bounded-ready request/work/logical-bytes 合同下 FIFO、DRR、external VTC-style、strict-priority 与 guarded-debt 六臂归因模板 | 双轮 12/12 已完成；仅作 Project internal selector attribution，不含原生 baseline、不授权 formal |
| `deploy/autodl/saor_project_mechanism_formal.example.json` | 位置平衡的 Project 六臂 1 warm-up + 3 formal 配置；与历史 development 配置分离 | 只可经 contract wrapper 运行；K/W/ready bytes/weights/manifest/service signature 全冻结 |
| `deploy/autodl/saor_project_feeding_ceiling.example.json` | 与 Project mechanism reference 逐字段匹配的单臂 direct bounded ceiling | formal 前仅补该 cell；调度归 direct client/vLLM，不使用 bounded-ready |
| `deploy/autodl/saor_feeding_gap_diagnostic.example.json` | 同 manifest/service/K128/W65536 的 D0/D1/P0 平衡 1+3 diagnostic 模板 | D1 是 Project diagnostic control；P0 才进入 PG/Daft/Ray/bounded-ready FIFO 路径，服务器恢复后用全新 root 运行 |
| `deploy/autodl/saor_feeding_gap_diagnostic_contract.json` | 7.10% gap 的四路 0.95 归因、必采指标和退出规则 | SHA 绑定旧 `locked_failed_feeding`；`may_change_prior_feeding_decision=false`，当前 code-ready/GPU-unrun |
| `deploy/autodl/saor_project_mechanism_formal_contract.json` | VTC-style 主参照、5% headline、吞吐/JCT/SLO/no-service 非劣、固定输出上界与 30s empirical repayment 的预注册决策合同 | `locked_failed_feeding/formal_authorized=false`；逐字段绑定有效 rehearsal 与当前签名 92.898% feeding-negative |
| `deploy/autodl/saor_ready_observation_bridge.example.json` | Project frozen-static/single-head shared FIFO/bounded-ready FIFO 三臂 observation bridge | 双轮 6/6 已完成并分离共享容量与 ready exposure；不含原生 baseline、不授权 formal |
| `deploy/autodl/saor_native_system_matched.example.json` | 五臂 2-Job PostgreSQL-source→validated-completion matched comparison 的总编排合同；冻结 typed Job release、manifest/SHA、MFU denominator、共同 endpoint 与 `writeback=none` | native 三臂不接 Project 控制；static/SAOR 分工明确；merge/rehearsal 均不授权 formal |
...
| `code/scripts/analysis/audit_saor_native_system_matched.py`、`run_saor_native_system_preflight.py`、`audit_vllm_0251_source.py`、`validate_saor_native_system_rehearsal.py` | 四阶段 readiness、实际 system probe、冻结 vLLM Python exact-source 与 completed rehearsal 封存入口 | 外层使用 DRIVER_PYTHON、vLLM 审计使用显式子进程；system 会重跑 health/PG/Ray/bounded raw gate，只有 actual smoke root 深校验后才 rehearsal-ready，formal 再绑定完整镜像 validation/root/archive |
| `code/scripts/serving/launch_vllm_with_identity.py` | vLLM endpoint exec 前的 runtime identity sidecar launcher | 由 `deploy/autodl/start_endpoints.sh` 通过 VLLM_PYTHON 调用，绑定 PID/start-time/argv0/sys.prefix/package identity |
| `code/src/experiments/saor/native_system_preflight.py`、`native_system_artifacts.py` | SAOR live system-preflight producer 与 smoke/rehearsal root 深校验核心 | 不信任自报 passed；核验 Ray 状态及实际 CUDA PID、重算 bounded raw evidence/cell artifact SHA/completion evidence，并将三 config SHA 与 frozen native provenance 绑定到完整 tar 镜像 |
...
| `code/src/experiments/saor/native_system_completion.py` | 五臂 no-writeback completion trace correctness 核心 | 冻结 manifest doc-id 集合与 executor trace 的行身份、内容 digest、exactly-once 不通过不得排名；不访问输出 sink |
...
| `code/tests/baselines/text/test_native_multijob.py` | 原生两 job manifest 互斥、错峰启动、四 shard、bounded 排除、命令审计、进程超时、失败脱敏与可搬迁 artifact 测试 | 修改 Daft/Ray Data 多 job 观察编排后运行 |
