...
| PG调用/绑定近期规格 | [详细设计](experiments/plans/postgresql_call_binding_design.md) | 公共描述/绑定及一个Filter→一个Map的接入、寿命、兼容与验收；共同调用/tuple绑定已验证；外层carrier与组合待接入 |
| 单流增量执行近期规格 | [详细设计](experiments/plans/semloom_incremental_session_design.md) | 单步操作、输入/结果所有权、进展和资源账本；静态复核完成，动态验证/实现待进行 |
