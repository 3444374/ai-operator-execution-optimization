...
| `experiments/plans/state_aware_work_unit_evaluation_20260808.md` | 分阶段 work descriptor、四个开题最小方法证据与 §5.2 SAOR 唯一维护入口；fixed-envelope formal 已完成 resolution-aware v2 validation，dynamic K parked；v0.5.1 双轮 development gate 已按门禁停止 | 设计/调整动态调度前读；bounded cap 未晋级，当前先修 Daft/Ray ready-set observation 与 coordinator 单-head 可见性断点；reservation 仍作后续鲁棒性消融，两 Job 未闭环前不扩 4-Job |
...
| `experiments/results/state_aware_work_unit/saor_bounded_priority_gate_20260813/` | v0.5.1 static/SAOR/0.125K/0.25K 双轮 GPU development gate、lossless event 与请求时序交叉验证 | 两 cap 均未过 fg 门；0.25K 第 2 轮无 debt recovery 被 fail-closed。定位完整 ready-backlog 模型与每 Job 单-head pull 实现不一致；不启动 formal/4-Job/reservation |
| `experiments/results/state_aware_work_unit/saor_bounded_ready_gate_20260813/` | v0.5.2 bounded concrete-ready observation 双轮 GPU gate、跨 trace lifecycle join 与无损事件证据 | 0.125K 两轮全过并注册 formal candidate；0.25K 两轮 bulk SLO 越界拒绝。只完成 development rehearsal，formal/4-Job/reservation 尚未运行 |
...
| `research/saor_model_scenario_audit_20260811.md` | SAOR 数学审计：capacity-only 负结果、fixed-envelope release、release-only 上界；§12 定义通用 priority/SLO budget + actual-work debt cap、release-opportunity 非饥饿界、事件证据与首轮门 | 设计 SAOR 后继、解释 static/strict-priority 权衡或撰写定理前读；v0.5 仅设计冻结，当前不声称实现、验证或证明完成，post-formal 状态为 formal-valid/not-promoted |
...
| `deploy/autodl/saor_bounded_ready.example.json` | static、原 SAOR、0.125K/0.25K bounded-ready 四臂模板；ready 窗口由签名化 K/W 自动派生 | 双轮 development gate 已完成；0.125K 注册 formal candidate、0.25K 被 bulk SLO 门拒绝。当前模板保留开发复现，正式矩阵不得把 0.25K 当 proposed |
