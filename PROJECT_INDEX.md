...
| `feasibility/results/squad_capability_full_10570_20260805/` | SQuAD 全量 10570 行 DuckDB-ai gate（`--mode full` + `--strict-attribution` + service-config-hash + fail-closed）；原始证据 report.json/per_row/manifest **保持不变**，README §8 含审计订正 | **fail-closed FAILURE（维持）**：10569/10570 成功、1 NULL；exactly-once/三 hash 一致/归因==10570 全通过；EM 80.32%/F1 89.36%。**经定点诊断订正**：1 NULL 是高并发偶发生成尾部风险（非确定性 rambling）。baseline = `eligible_with_documented_failure`，进入失败感知比较 |
| `feasibility/results/squad_truncation_diag_572700c8_20260805/` | full gate 失败行 572700c8 的定点截断诊断（direct vLLM + DuckDB `ai_try_complete` × cap{64,128,256} × 3 重复；cache/retry off；附 `ai_completion_request_json`） | **截断不可复现**：cap=64 孤立重放 3×3 全部 `stop`/46 token/文本一致；DuckDB 与 direct 请求体逐字段一致 → 推翻「确定性 rambling」，改为「高并发偶发尾部风险」。诊断专用，不回灌 cap=64 |
