...
> **原始 trace 存放位置（2026-09-21 迁出 git）**：本目录的 per-run 原始数据（requests/submissions/resources/flush 等 trace、日志、raw/ 内容）已按 `results/RAW_ARCHIVAL_20260807.md` 政策从 git 跟踪移除；迁出前已逐文件 SHA-256 与 git blob 内容核对一致，镜像于 AutoDL `/root/autodl-tmp/ai-operator/experiments/results/` 与本地 `C:\Users\ays\Desktop\results\experiments\results\` 下的同名目录。git 内保留聚合 CSV、summary、manifest 与本 README。
...
- 配置：`/root/autodl-tmp/gates/prefix_cache_routing_4ep_1.5b.json`。结果目录（远端）：`results/data_organization/prefix_cache_routing_4ep_1.5b_20260731/`。manifest：`status=completed`，8/8 ok，**0 incident**。
...
- **证据保留**：失败首跑目录原位保留为 `results/prefix_cache_routing_4ep_1.5b_20260731_failed_raystale/`（不删——其 stderr 记录了 14 分钟挂起的根因）。
