...
- **raw**：服务器 `results/system_e2e/multicard_lbrr_scale_ramp/scale_<S>/lb_rr_c64_rep1/`（`gate_output/lb_rr_c64/shard_0/{summary.json,requests.csv}` + `ttft_metrics.json` + `gpu_resource.csv` + `ramp_run.json`）；commit 时提交裁剪版（summary/ramp_run，不含大 requests.csv）。
