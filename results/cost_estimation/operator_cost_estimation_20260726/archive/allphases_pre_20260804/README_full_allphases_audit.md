...
> **当前权威口径（2026-08-04）**：审计发现旧 loader 把 warmup 混入模型和候选
> repeat 聚合。下文 283-row grouped-holdout / CE hierarchy 数字均为历史 all-phase
> 结果，已经移动到 `archive/allphases_pre_20260804/`，不得用于正式 claim。当前只认
> `ce_context_loo_20260804.md` 与
> `ce_context_loo_formal_only_23feature_20260804.json.gz`：204 条 formal、17 contexts；
> CE5 MAE 7.91s、candidate pairwise 0.800、macro regret 4.58%、max regret 26.23%，
> 因 row-level pairwise 0.684 未过既有门槛，仍不晋级。双 4090 新机器采样门禁见
> `../operator_cost_profile_pilot_20260804/`。

以下旧章节保留用于说明方法演进，数值状态统一为“历史审计”，除非段落明确引用上述
formal-only 证据。
...
> 继续作为历史证据。后续审计发现旧 LOO 加载器把 warmup 混入 formal 聚合；两份
> all-phase JSON 已改名为 `*_allphases_audit_20260804.json.gz`，仅供追溯。当前权威证据是
> `ce_context_loo_formal_only_23feature_20260804.json.gz`，只加载 `phase=formal`。
...
- `archive/allphases_pre_20260804/e2e_cost_model.json`：历史主切分特征 schema；
- `archive/allphases_pre_20260804/e2e_cost_model_seed_*.json`：历史五个切分。
...
grouped holdout。完整历史表见
`archive/allphases_pre_20260804/ce_hierarchy_table_20260804.md`。5-seed mean：
...
formal-only 23-feature 复算后，CE5 仍是最强候选之一（MAE 7.91s、macro regret 4.58%、
pooled regret 0.62%、7/13 pick）；但沿用预注册的行级 pairwise 只有 0.684，仍未达到
0.75，因此结论仍是**不晋级**。repeat 聚合后的候选 pairwise 为 0.800，属于更贴近
计划选择语义的
新诊断口径，必须在
下一轮新数据前预注册，不能事后替换门槛。逐 fold 证据、候选不均衡和精确补测预算见
`ce_context_loo_20260804.md`；机器可复算数据见
