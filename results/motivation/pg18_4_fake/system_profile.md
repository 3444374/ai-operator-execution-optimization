...
当前项目口径更新：本文件是 PG18.4 本地同构 fake-model 历史画像，只用于预演阶段计时、写回边界和早期变量选择。正式主线已经收敛为”数据库 AI 负载的执行优化与调度”，真实瓶颈归因应优先引用 `results/motivation/gpu/` 下的 GPU-backed 结果。
...
- 本地数据库能否连接。连接验证见 `results/feasibility/pg18_4_connection_validation.md`。
...
```text
results/motivation/pg18_4_fake/system_profile.csv
```
