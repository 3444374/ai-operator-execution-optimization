...
当前项目口径更新：本文件是 fake/CPU 历史预研分析，只解释早期为什么关注 task/object/fan-in、operator invocation、writeback 和 backpressure。正式主线已经收敛为”数据库 AI 负载的执行优化与调度”，真实瓶颈归因应优先引用 `results/motivation/gpu/` 下的 GPU-backed 结果。
...
> 当前本地实验只作为历史预研，说明早期为什么关注 task/object/fan-in、writeback 和 backpressure。它们不能替代真实数据库、真实 AI 算子、真实 GPU-backed 模型服务验证，也不能作为真实 GPU 链路瓶颈归因。

## 2. 结果文件

| 文件 | 实验 |
|---|---|
| `fake_embed_pipeline.csv` | fake `AI_EMBED(text)` 端到端动机测试 |
| `workload_matrix.csv` | 三类 AI 算子场景对比 |
| `granularity.csv` | task/object/fan-in/operator invocation 收益来源拆分 |
| `backpressure.csv` | 模型服务 queue wait / token backlog / backpressure 模拟 |
...
```text
results/motivation/fake_cpu/fake_embed_pipeline.csv
```
...
```bash
motivation/benchmarks/granularity.py
```

正式结果：

```text
results/motivation/fake_cpu/granularity.csv
```
...
```bash
motivation/benchmarks/backpressure.py
```

正式结果：

```text
results/motivation/fake_cpu/backpressure.csv
```
...
优先补三件事，顺序应服务“外部执行链路调优”主线，而不是先花大量时间做 GPU/数据库内执行 baseline：

1. **PostgreSQL + 外部 worker baseline 矩阵**：比较 Python batched worker、Ray task、Ray actor，验证数据库读取、批处理、AI 算子执行和写回链路中是否仍有同样瓶颈。
2. **写回与汇聚形态对比**：比较主控进程 fan-in 后批量写回、多 worker 各自写回、任务表/队列式 vectorizer worker 写回，记录连接数、事务大小、writeback_s、失败重试和冲突。
3. **生产式 GPU-backed E2E 主动机画像**：把 backpressure 从离散模拟迁移到 Ray actor、Ray Serve、vLLM 或轻量 embedding service，记录 queue wait、token backlog、actor idle time、tokens/s、GPU utilization 和 writeback。如果 GPU 环境可用，这应作为下一组主实验，用来证明真实计算端点下外部执行链路是否是主要损耗来源；不把 GPU 迁移或 GPU kernel 优化作为主线。
...
| 类型 | 文件 | 用途 |
|---|---|---|
| 连接验证 | `results/feasibility/pg18_4_connection_validation.md` | 只说明本地 PG18.4 + pgvector 可连接，项目脚本能读写数据库 |
| 系统画像 / 瓶颈定位 | `results/motivation/pg18_4_fake/system_profile.md` | 说明 PG18.4 真实数据库触发链路中的瓶颈、可优化点和下一步 baseline |
| 系统画像原始数据 | `results/motivation/pg18_4_fake/system_profile.csv` | 4096 行 formal CSV，含 warm-up/formal、python/ray_actor、fine/coalesced |
...
| 主控 fan-in 后批量写回 | 集中汇聚是否成为瓶颈，writeback 边界是否清晰 | 待补 |
...
2. 再补主控 fan-in 写回 vs 多 worker 各自写回，以及 pgvector `vector(128)` 写回。
3. 再接轻量真实 embedding 或 Ray actor/Ray Serve 模型服务队列。
4. 如果 GPU 环境可用，优先补一个 small-scale GPU-backed E2E profile，作为课题主动机实验；不先做 GPU 迁移收益或 GPU kernel 优化。

这条顺序能避免两种偏差：只在 fake/CPU 条件下做阶段 attribution 会缺少生产式主动机；直接只看 GPU 端到端总时间又无法解释收益来源。因此应先做一条可分阶段计时的 GPU-backed E2E 主链路，再在同一条真实链路上做大块消融和后续细分实验。
