...
本目录保存数据库 AI 算子动机测试的**唯一正式结果**和分析。`results/feasibility/` 不再存放动机测试 CSV 副本。
...
python motivation/benchmarks/granularity.py \
  ...
  --repeats 3 \
  --output results/motivation/fake_cpu/granularity.csv
```
...
python motivation/benchmarks/backpressure.py \
  ...
  --queue-limit 0 8 32 \
  --repeats 3 \
  --output results/motivation/fake_cpu/backpressure.csv
```
