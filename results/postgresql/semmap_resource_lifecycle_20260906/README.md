...
当前实施入口仍是 [Map 合同 §8.4.3](../../../docs/plans/postgresql_semmap_generation_contract.md)，本文件记录实际执行证据。
...
依据 [Map 合同 §8.4.7](../../../docs/plans/postgresql_semmap_generation_contract.md)，从已推送的
...
用户在清理后要求确认两个算子可完整运行，并授权验证无误后合并。计划见 [Map 合同 §8.4.8](../../../docs/plans/postgresql_semmap_generation_contract.md)。
