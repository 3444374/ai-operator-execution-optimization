"""Qualify the gateway-only choice slice with isolated PG18.3 and fake HTTP."""
import argparse
import hashlib
import json
import os
...
assert commit == "7d72d9ad0998bf308c17edd68ddb38971d30dafc"
...
tap = run("tap", as_pg + ["env", "PATH=" + env["PATH"], "make",
...
try:
    run("reg-start", pg_ctl + ["-l", str(reg / "server.log"), "-o",
        f"-c listen_addresses='' -c shared_preload_libraries=semloom_pg -k {socket} -p 55441", "-w", "start"])
    env.update(PGHOST=str(socket), PGPORT="55441", PGUSER="postgres")
    server_version = run("reg-server-version", as_pg + [str(args.prefix / "bin/psql"),
        "-h", str(socket), "-p", "55441", "-d", "postgres", "-Atc", "SHOW server_version;"]).strip()
...
save("qualification.json", dict(source_commit=commit, tests=tests, steps=steps,
    ...
    gateway_wire_v4_implemented=True, pg_wire_v4_implemented=False, isolated_prefix_only=True))
