...
本次完成了图像类型 `ai_semantic.embed(bytea,jsonb)→real[]` 的工程接入与受控验证：
PG18.3 严格编译、回归 1/1、TAP 2,159 项及 9 组实际 PG/Ray/解码检查通过，
CPU 准备与模型阶段的资源分别保留，17 个 Job 完成回收。当时真实模型调用为 0 次；
后续 151 次真实 CLIP 前向的三路径逐维一致见
[有限真实验证](../m1_m2_f_real_20260920/README.md)。
本结果不支持 GPU 计算中故障处理或匹配性能已验证的结论；工程能力不直接作为方法贡献。
...
计划见[工作包 F](../../../docs/plans/data_organization_batching.md#work-package-f)。
...
工程参照沿用[主架构 §8.7–8.8](../../../docs/plans/postgresql_ai_semantic_operator_architecture_20260827.md#pgml-engineering-reference)
