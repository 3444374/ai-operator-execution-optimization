...
> 安排见[独立工作包计划](../../../docs/plans/data_organization_batching.md#当前实施执行修复与数据库查询套件2026-09-10)。

日期：2026-09-10。角色：内部工程与校准记录。研究对象为 **PostgreSQL 内置 AI 语义算子的外部分布式物理执行与调度优化**。
当前合同见[数据执行计划](../../../docs/plans/data_organization_batching.md#当前-pg-单-map-数据执行切片)。
...
![单卡并发、规模、窗口和重复观测](../../../docs/figures/data/backup/map_capacity_20260910.png)

TP1 指张量并行度 1，BF16 是 16 位浮点精度；EM 是答案完全匹配率，F1 是答案词重叠分数。
图中完成行吞吐包含合法但答错的结果；答案质量另报。前三个面板是单次诊断，不能与第四个面板的重复值混为同一证据。
图源、SVG 与检查见[图审计](../../../docs/figures/audit/map_capacity_20260910.md)。
