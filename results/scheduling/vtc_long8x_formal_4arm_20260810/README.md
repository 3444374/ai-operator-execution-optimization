...
- **聚合（进 git，本目录）**：`manifest.json` + `records/*.json`（16 cell per-cell summary）+ `group_runs.csv`（runner 直接产的 per-cell 运行表）+ `formal_summary.csv`（由 formal `group_runs.csv`/records 独立复算的 per-arm 3-rep 聚合；vs-independent 3 列因无 independent 臂而省略）。其中 `start_skew_s_max=max(observed_start-offset)-min(observed_start-offset)`，不把预设 arrival offset 跨度误记为同步偏差。
...
**正确性门 1–10：全过 ✅** — manifest `completed`/incidents=[]；16/16 group（4w+12f）；8-client arrival replay 最大 lateness 0.0002s、offset-normalized barrier skew 最大 0.000081s；每 client 完成行数精确 [274,289,282,431,416,435,417,419]；`request_success_delta=2963`（exactly-once，0 missing/dup/unexpected）；actor/HTTP/timeout/ReadError 全 0；两 endpoint 都接收；每轮 final active/waiting 全归零；traces 非空；用 time-series mean/p95/max。

**喂饱门（资格，按 3-rep mean）**：adaptive PASS；DRR/VTC/FIFO FAIL（见 §4）。
...
表中 GPU util 是百分数，MFU 是 0–1 分数；`formal_summary.csv` 的对应列 `mfu_pct_mean` 已换算为百分数。
...
- **推断**：adaptive 过门主要与其允许升到 K160 一致（静态 K128 用不到）；后续 frozen-K160 也过门，说明 K 上限足以解释该现象。3 静态臂 ~5% 低于 direct 天花板可能包含 shared-credit 协调开销，但本实验没有单独分解该开销。
- **不能声称**：
  - **"adaptive > DRR（动态优于同上限静态）"**——本轮静态 DRR 是 K128，并非同上限对照；后续 frozen-K160（见 `../vtc_long8x_drr_k160_control_20260810/`）同样过门且数值略高，因此当前证据不支持把 adaptive 过门归因于在线动态控制。
  - 原生 VTC / 理论 guarantee。
...
饱和 regime 下首份 formal 多臂 A/B；后续 frozen-K160 也过门且与 adaptive 数值接近，因此 **动态控制的额外吞吐收益尚未得到支持，K160 上限已是充分解释**。这不是“动态收益严格为零”的统计证明：K160 follow-up 未与 4 臂交错，且其相对 adaptive 的 +0.33% 小于 adaptive 重复波动。state-aware 仍 `待验证`（claim_matrix 第 59 行不升级）。控制器行为正确（干净 ramp、不振荡），且首次在饱和 regime 真正 actuate（补上早前“结构性零效应”的缺口）。

## 7. 下一步

见 `../vtc_long8x_drr_k160_control_20260810/`。要验证同上限下的动态增量，需 phase-change/变载 workload（on-off gate 此前从未跑），并让 frozen-K160 与 adaptive 交错重复。claim_matrix 第 59 行维持待验证。

## 不能声称的边界

本实验是 external completion-corrected VTC-style baseline；不称原生 VTC、不称理论 guarantee、不称“动态优于静态”，也不称“动态收益严格为零”。
