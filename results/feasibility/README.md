...
- 数据库 AI 算子端到端动机测试、系统画像、瓶颈定位和可优化点分析放在 `results/motivation/`。
- GPU-backed E2E profile 不放这里，放 `results/motivation/`。
...
这些文件只能说明 GPU 模型服务能跑通；端到端瓶颈和优化结论仍应进入 `results/motivation/`。
