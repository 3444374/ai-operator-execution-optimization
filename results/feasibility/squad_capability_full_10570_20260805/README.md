...
- **定性（经定点诊断订正）**：**不是基础设施或数据故障，而是 DuckDB-ai baseline 在统一 cap=64 下、
  ...
  （见 `results/feasibility/squad_truncation_diag_572700c8_20260805/`）；full-set 跑里观察到一次
...
- **失败行归因订正**：§5/§6 原写「模型 rambling/loop、temp=0 确定性」**不成立**。定点诊断
  （`results/feasibility/squad_truncation_diag_572700c8_20260805/`，`9aefeba`）证明 cap=64 孤立重放
