...
- `check_drawio.py`：06 为 109 cells / 100 vertices / 7 edges / 14 SVG images；07 为
  65 / 50 / 13 / 9；08 为 37 / 35 / 0 / 0。三图全部通过。
...
## 2026-08-12 P03 箭头与图例修订

- 将右侧六张流程卡重新分配为 58–66 px 高、相邻间距 18 px，五段蓝色连接均显示完整线身与向下箭头。
- 三条青绿色反馈支线拆为独立 SVG path，分别显示左向箭头；反馈总线与主流程保持明确分区。
- 新增图内图例：蓝色实线为执行主流程、青色虚线为状态反馈、实线框为执行步骤、虚线框为字段分组。
- 右侧流程文字取消 `spacingLeft`，全部按卡片几何中心对齐。
