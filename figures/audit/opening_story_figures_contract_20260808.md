...
- 2026-08-22 对外用语修订：图内不再使用 `active work`、`endpoint`、`formal`、`W65K` 和“到达受限”等未解释简写，
  改为“在途工作量”、“模型服务实例”、“统计运行”和“请求到达较慢”。29% 仍指运行期间观测到的在途工作峰值相对于 65,536 配置上限的比例。
- 2026-08-24 层级修订：联合图不再把三个 panel 写成三个平级概念。panel a 明确属于动机一；
  panels b、c 共同属于动机二，并分别标为配置层与模型服务层。拆分版 P07A、P07B 使用对应的
  父命题作为总标题；Job 层由多 Job 图另行补充。数据、坐标、误差线和统计口径均未改变，
  详细审计见 `opening_motivation_hierarchy_revision_20260824.md`。
...
- panel b 来源：R0/R1/R2 transfer ceiling 的 batch64 各 30 次重复；GPU-resident、pinned FP16、
  pageable FP32 分别约 9.82K/8.72K/1.96K img/s。这只说明输入表示、精度转换和主机端数据组织会改变阶段吞吐，不把差异全部归因于 PCIe 带宽或某一次拷贝。
...
### 6.1 PPT 第 29 页三组图（v4，2026-08-23）

- 文件：`data/report_main/opening_cost_model_decision_quality_v4.{png,svg,pdf}`；输入仍为
  `ce_context_loo_rerun_20260807.json`，没有加入新实验数据。
- 图 a：六种估计方法各占一个小图，统一使用 0--52 s 横纵坐标。横轴为真实端到端时间，纵轴为
  预测时间；同一横坐标上的空心点位于 `y=actual`，实心点位于 `y=predicted`，两点间竖线直接
  表示该候选预测相差的秒数。每种方法严格包含 20 个留出场景 × 4 种上限 = 80 组候选均值，
  不能将其写成 429 个独立预测。蓝色竖线表示预测偏短，橙色竖线表示预测偏长。
- 图 a 数值回读（中位绝对相对偏差 / 候选均值平均绝对误差）：均值 43.1% / 6.41 s，解析模型
  17.8% / 3.34 s，查表 33.4% / 5.42 s，岭回归 19.8% / 3.18 s，LightGBM 30.7% / 5.08 s，
...
- 2026-08-24 可见文字复核：总标题改为“相同总上限下：共享未用份额提高总体效率，但各
  Job 改善幅度不同”，panel b 改为“共享未用份额提高总体效率”，panel c 首次写全“完成速率
  公平指数（Jain）”。这些修改只把内部术语换成可直接阅读的条件和现象，没有改变数据或统计口径。
...
## 12.1 2026-08-22 开题报告图文口径复核

- A：报告副本同步到当前权威图，容量曲线继续使用 65K 达到已测峰值 97.8%、65K 到 98K
  吞吐约增加 2.3% 而 P99 由 36.8 s 增至 40.0 s 的原始数据。
- C：标题改为“服务部署条件改变后”，避免把 2 endpoint/0.90 与 4 endpoint/0.43 的完整配置差异
  简化成只改变缓存压力；纵轴术语改为“共享前缀缓存命中率”。
- D：传输面板标题由“瓶颈不是 PCIe”改为“输入表示与主机端数据组织会改变阶段效率”。R0/R1/R2
  的实测吞吐不变，但不再从主机级画像直接断言完整系统的唯一瓶颈。
- H：报告副本使用同一 full-pool 独立运行参照，Jain 为 0.988 与 0.876；标题和图注改用“共享未使用
  份额”，不再直接使用 Project A/B、work credit 和 formal 等内部工作用语。
- E：排序准确率 0.75、平均决策损失 5% 和最坏情境损失 15% 保留为实验前采用的参考值；标题、图例
  和图注不再使用“门禁、晋级”，并把 context、pairwise accuracy 和 decision regret 换成对应的中文解释。
- 以上修改只影响图中标签和报告副本同步，没有更换 CSV、统计方式或实验结论。PNG 和 SVG 由同一
  Matplotlib 脚本重新生成；复核后未发现坐标裁切、图例遮挡或只靠颜色区分数据系列的问题。
