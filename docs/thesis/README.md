...
- 开题 Top 15 已升级为 15/15 严格 CCF-A 正式 research paper；权威清单见
  `../docs/research/top15_ranked_papers.md`，自包含快照见
  `literature/top15_reading_notes/`。
- `../docs/research/reading_notes/` 当前有 49 篇历史文献笔记，现按泛读库管理；新的权威精读笔记进入
  `../docs/research/精读文献笔记/`。当前共有 17 篇主笔记和 160 张论文原图裁剪件；Kalypso 作为 arXiv
  核心补充，新增 IMLane 使用正式 PVLDB 2026 版本；两篇都不进入现有 Top 15、十五篇速览或已定稿正文。
  `../docs/research/reference/` 当前工作区有 7 份可解析 PDF 实体（Galois、Abacus、Palimpzest、Sema、
...
| `feishu/` | 飞书进度汇报稿 |
| `literature/` | 开题文献精读清单（`reading_list.md`）+ 既有 Top 15 笔记快照（`top15_reading_notes/`）；泛读、精读和 PDF 分别在 `docs/research/reading_notes/`、`docs/research/精读文献笔记/` 与 `docs/research/reference/` |
...
| 开题报告 Word 版 | `report/数据库_AI_负载的执行优化与调度研究_开题报告.docx` | 2026-08-25 导出；尚未包含截至 2026-08-30 的 Sema-like 架构、PostgreSQL 18.3 实现状态与后续路线，重新对外使用前需从 Markdown 权威稿生成并检查版式 |
| 开题报告 QA | `report/opening_report_20260824_qa.md` | 2026-08-30 已校准 PostgreSQL 18.3 recording carrier、真实语义缺口与工程顺序；七部分结构、16 张图和 53 条参考文献沿用原审查，Word/PPT 发布面仍需重新生成或增量修改后单独检查 |
...
| 开题报告专用图片 | `report/figures/` | 当前正文引用 5 张背景/方案图和 11 张数据图；权威可编辑源在 `../docs/figures/architecture/editable/`，数据图源在 `../docs/figures/opening_figure_set/` 与 `../docs/figures/data/report_main/` |
...
| 文献精读清单 | `literature/reading_list.md` | 候选清单已补；未来全文精读进入 `docs/research/精读文献笔记/` |
| 开题 Top 15 历史快照 | `literature/top15_reading_notes/` | 15 篇既有笔记的自包含快照，不代表新精读库完成状态 |
| GPU 调度与数据放置补充调研 | `docs/research/gpu_scheduler_data_placement_supplement_20260715.md` | 已补，作为策略控制器设计依据与后续精读清单 |
| 本地 PDF 子集索引 | `docs/research/reference/README.md` | 已登记当前已下载的部分论文，非完整文献库 |
...
1. 本地 Markdown 权威报告已同步 PostgreSQL 18.3 recording `SemMap/SemFilter`、shared runtime 与同步
   provider seam 的当前事实；下一步先完成真实 `SemanticPlanSpec` 和同步 exact model reference，再实现
   第二 physical path、cost/quality 与 carrier audit。现有 Word、PPTX 和飞书发布面仍是旧快照，未同步。
