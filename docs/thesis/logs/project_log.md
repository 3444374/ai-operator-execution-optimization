...
- 保留实际打开 PPT 检查、人工 shape 修改保护、版本与引用核验、发布授权和本地稿与远端页面验证要求。
- 本次只改规则与日志，未修改报告、PPT、图件或远端文档。
...
- 当前材料状态统一为 16 篇精读主笔记、146 张论文原图裁剪、16 图/53 条参考文献的中文报告，
  以及 26 页 PPT v9；v9 已独立完成 26/26 页 QA，但 2026-08-22 至 08-25 的报告更新尚未回灌，
  因而报告—PPT 差异审查仍待执行。旧 v8、20 页内容大纲和 2026-08-12/20 审查记录保留为历史
  快照，并增加当前替代入口。
- SQuAD database-E2E 统一解释为完成性与答案质量对照。Project 的外层计时多包含指标采集、记录
  写入和作业结束处理，三条路径不到 1% 的记录差异不用于性能排名；该限制已同步 Claim Matrix、
  答辩问答、图资产说明/审计和 PPT v9 构建源。
- 本轮只修改 Markdown 与未来重新生成时使用的构建源字符串，没有运行全量 builder、覆盖当前
  PPTX 或改变图中数值。用户手工调整过的演示文件继续按增量编辑规则保护。
- `opening_defense_outline_20260808.md` 作为对外内容基线，不再直接使用项目内部的阶段代号和
  管理状态词；相关句子已改写为具体实验条件、执行动作和不能外推的范围，并同步 production
  descriptor、同候选集 Project 对照、五臂 rehearsal 与当前 26 页 v9 的实际状态。

## 2026-08-27 开题目录规则收敛

- `docs/thesis/AGENTS.md` 只保留开题论证层级、证据边界、写作/图表、文献版本、PPT 人工编辑保护和
  发布同步规则；当前页数、材料状态和工具/skill 枚举继续由 README、navigation 和平台能力承担。
- `work_rules.md` 与 `ppt_rules.md` 从并列规则源改为按任务触发的专项清单：前者只处理跨材料变更
  路由，后者只处理 PPT 模板、增量编辑、图表、备注和实际打开检查；`docs/thesis/AGENTS.md` 明确何时
  读取，两份清单不能覆盖根或 opening 规则。
- 规则继续固定两项研究内容、共同代价估计与跨模态验证的角色；未修改开题报告、PPTX、飞书稿、
  参考文献题录或任何对外图件。

## 2026-08-27 开题入口历史状态标注

- 将 `docs/thesis/README.md` 底部 2026-07-15 飞书同步说明明确标为历史记录，避免旧链接和旧“三层”
  表述被误读为当前发布状态；当前报告、PPT 与飞书入口不变，未修改任何对外正文或图件。
- 更新 `docs/thesis/practice_plan.md`：专业实践计划统一为两项研究内容、共同代价估计和多模态验证，
  写回改为统一工程 baseline；阶段时间和学校表格结构不变。
- 精读库登记 Kalypso 后，将 `docs/thesis/README.md` 与 `literature/reading_list.md` 的物理计数校准为
  16 篇主笔记、134 张原图和 6 份本地可解析 PDF。Kalypso 项目内副本在大小写重命名事故中一度
  丢失，随后已从下载目录 1439 行原件逐字节恢复并通过 SHA-256 核对；它只作为 arXiv 核心补充和
  研究边界校正依据，不进入现有 Top 15、十五篇开题主线、报告正文或 PPT。

## 2026-08-25 答辩 QA LaTeX 文件归档

- 将答辩 QA 的 `.tex/.pdf` 与已有设计说明统一移动到 `report/opening_defense_qa/`，并删除原位置的 `.aux/.log/.out/.toc/.synctex.gz` 五个可再生成中间文件。
- 新目录只保留可编辑源、最终 25 页 PDF 和设计说明；问答正文、PDF 内容、开题报告、Word 与 PPT 均未改动。

## 2026-08-25 新增开题答辩 QA LaTeX/PDF

- 新增 `report/opening_defense_qa/opening_defense_qa.tex`，按“一句话结论—30/60 秒主答—步骤—追问—证据状态—不能声称”的双层结构整理 83 道开题答辩问题；其中 Q15--Q53 共 39 题详细覆盖总体方案、分阶段工作量表征与数据组织，以及固定容量下的提交、路由与多 Job 调度。
- 增加 7 条连续追问链、两项研究内容总分表、已有工作与本课题差异表和术语白话表；`sim-filter` / `project-sim-filter` 明确解释为先用低成本相似度或投影表示减少需要进入昂贵语义 join 的候选记录对。
- 使用 XeLaTeX 实际生成同目录本地 `opening_defense_qa.pdf`，共 25 页；三次编译后日志无 LaTeX warning、overfull/underfull 或未定义引用，已检查全页缩略图以及正文、追问链和跨页表格的原尺寸渲染。
- 本轮未修改开题报告正文、Word、PPTX、飞书材料或实验数据。

## 2026-08-25 开题报告参考文献按首次出现顺序重排

- 按正文首次出现顺序重新分配 `report/opening_report.md` 的 53 条参考文献编号，并同步替换 58 处正文引文组；文献题录内容、正文论述和图表均未改动。
- 自动检查确认 53 条题录全部在正文出现，首次出现序列为 1—53，正文没有无对应题录的编号，文末也没有未引用条目。
- 重新生成学校模板 Word 版 `report/数据库_AI_负载的执行优化与调度研究_开题报告.docx`，并把顺序编码要求加入开题文献规则。

## 2026-08-25 开题报告 Word 模板版生成

- 将 `report/opening_report.md` 按 `templates/硕士生开题报告模板0604.docx` 转换为 `report/数据库_AI_负载的执行优化与调度研究_开题报告.docx`，保留学校封面、A4 页面与页边距、页脚页码和末页签字表。
- 正文沿用模板的宋体/Times New Roman、小四、1.5 倍行距和两端对齐；一级标题使用四号黑体，二、三级标题使用小四黑体，图注使用五号黑体。报告包含 16 张正文图片、2 个正文表格和 53 条参考文献；姓名填写为艾筠舜，学号、专业和指导教师因未提供而留空。
- 已检查 1 个 A4 纵向节、17 个内嵌图片对象（含封面校名图）、页码域、表格宽度、图片替代文本和标题/图注直接格式，未改写 Markdown 正文内容。

## 2026-08-25 评审意见最小幅度落实

- 未新增或替换图片，仅在报告正文中补充数据库内集成的验证层次、取消与迟到结果处理，并区分共享空闲容量的基础作用与后续作业选择、服务目标保护和路由增量。
- 代价估计收敛为首个语义等价 SQL 计划排序案例，多 SQL 和对两项研究内容的增强改为后续可选实验；进度安排删除论文撰写与送审事项，保留音频、视频重负载的后续接口和问题观察。

## 2026-08-25 Parrot 精读配图与文献统计同步

- Parrot 精读主笔记已按正式 OSDI 2024 PDF 加入 Figure 1–19；开题文献清单只同步精读图像总数 115→134 和本地可解析 PDF 数 4→5。
- 本轮未修改开题报告正文、参考文献条目、图片、PPTX、Word 或飞书/Wiki 内容。

## 2026-08-24 Parrot 与 DLPM 精读纳入第二章研究现状

- 根据两篇新增全文精读笔记，在 `report/opening_report.md` 第 2.3 节分别补充 Parrot 的应用依赖与共享提示词感知方法，以及 DLPM/D²LPM 兼顾公平服务和前缀缓存复用的方法。
- 两段均交代实验硬件、模型或模型副本规模、比较对象、代表性结果和不利情形。正文只把这些结果作为相关工作的证据，没有写成本课题已经获得的性能收益，也没有把模型服务内部调度与数据库上游调度混为一谈。
- 第 2.4 节相应补充两篇论文与本课题的关系。参考文献总数保持 53 条，精读主线由 13 篇更新为 15 篇；本轮未改第一章、研究内容、实验结果、图片和 PPTX。

## 2026-08-24 IMBridge 精读纳入第二章研究现状

- 逐项对照当前 13 篇全文精读笔记与 `report/opening_report.md`，原有 12 篇已在数据库 AI 查询、分布式执行和模型服务三组研究现状中使用，本轮补入新增 IMBridge。
- 正文说明 IMBridge 处理的两个问题：预测函数中可复用的初始化状态被重复建立，以及数据库交付批次不能稳定匹配模型调用批次；随后介绍函数改写和独立预测算子，并限定 4 页演示论文中截图数字的可解释范围。
- 此处修订只扩充第二章研究现状，未把该方法写成本课题已经实现的技术，也未改动第一章、两项研究内容、当前实验和 PPTX。参考文献现为 53 条。
...
- 依据 `docs/research/reading_notes/` 与 `docs/research/reference/REFERENCE_INDEX.md` 新增 9 篇与正文论点直接对应的文献，覆盖数据库内推理、推理阶段与容量管理、多作业公平和代价估计；FairServe、DLPM、Autellix 和 CONCERTO 按预印本标注。
...
- 搭建本机 drawio 渲染环境（playwright-core + Edge，`docs/figures/scripts/render_drawio.js`），重导 P11 PNG/SVG
...
- P08B 由平级图像动机页降为动机补充页，避免与 M1（13.9–31×）和 M2（近饱和区）重复。
  ...
  `docs/figures/README.md`、`docs/figures/opening_figure_set/README.md`、audit 契约已同步。PPT 由用户自行更新。
...
- 修复绘图脚本 Windows 中文字体回退（补入 Microsoft YaHei/SimHei），重画 P08A/P08B 的
  PNG/SVG/PDF 并刷新图集中文名副本；`docs/figures/README.md`、`docs/figures/opening_figure_set/README.md`
  与 `docs/figures/audit/opening_story_figures_contract_20260808.md` 已同步。
...
- `docs/thesis/feishu/opening_report_wiki.md` 已与本地报告逐字一致。线上文档未覆盖：现有用户
...
- 完成 v5 的内容、视觉和架构图审阅，确认当前文件落后于 2026-07-29
  项目实验结论，不能直接作为当前汇报版本。
- 新增 `docs/thesis/slides/opening_defense_v6_design.md`：正文沿用 v5 四章节，
...
## 2026-07-24 文献精读语料迁出 docs/thesis/，opening 仅留开题精读清单 + Top15 拷贝

- 文献精读笔记（44 篇）、PDF（69 个）、清单与评估从 `docs/thesis/literature/` 迁至 `docs/research/`（项目级文献目录）。理由：opening 是阶段性开题工作区，不该承载项目级长期文献资产；opening 自己的 README/navigation 也指向 docs/research/ 查文献。
- `docs/thesis/literature/` 现仅保留：`reading_list.md`（开题精读优先级）+ `top15_reading_notes/`（开题要求精读的 15 篇笔记拷贝 + figs，自包含快照；权威版在 `docs/research/reading_notes/`）。
- 索引/链接同步：`docs/thesis/README.md`、`docs/thesis/AGENTS.md`、`docs/thesis/navigation.md` 的 `literature/` 描述已更新；完整变更清单见根 `PROJECT_LOG.md` 2026-07-24 条目。
...
- 本地来源：`docs/thesis/report/opening_report.md`（已同步修改），图源：`docs/figures/data/report_main/b26_arrow_vs_daft_stage_breakdown.png`。
...
- 根据当前已经完成 pgai SQL 触发面集成和真实 GPU-backed `AI_EMBED` 完整链路复测的事实，更新 `docs/thesis/report/opening_report.md` 和 `docs/thesis/feishu/opening_report_wiki.md`。
...
- 已覆盖同步新版开题报告飞书 docx，并重新插入 8 张正式 PNG；回读确认 revision 更新到 `72`，未检出 fake/CPU、图 4-7、表 4-4、Mermaid 旧图或本地 `docs/figures/` 路径残留。

## 2026-07-15 开题报告飞书新版 docx 同步

- 使用 user 身份将 `docs/thesis/feishu/opening_report_wiki.md` 覆盖同步到新版开题报告飞书 docx：`https://my.feishu.cn/docx/CRgXdyTlToXpgjxo3otcf3kInGb`。
...
- 回读线上文档确认 revision 更新到 `51`，关键图注附近为真实飞书图片 URL；关键词检查未发现本地 `docs/figures/` 路径和旧的“三岛/Killer/联合最优/边界确认/阶段画像/Ours-v0”等表述残留。
...
- 新增 `docs/research/gpu_scheduler_data_placement_supplement_20260715.md`，用于说明策略控制器设计参考了 GPU / LLM 推理服务调度、Ray/Ray Data 异构数据管线、GPU 数据库算子与数据库 AI 算子等文献线索。
- 该文件当前作为“设计依据与后续精读清单”，不替代逐篇精读笔记；未下载或未逐篇核验的条目仍按待核验处理。
- 已同步 `docs/thesis/README.md`、`docs/thesis/literature/reading_list.md` 和 `PROJECT_INDEX.md` 入口。
...
- 已重新生成 `docs/figures/architecture/runtime_strategy_control_loop.png` / `.svg`，并同步图表审计与策略设计文档。
...
- 根据用户反馈修订 `docs/thesis/assets/figures/system_architecture_ai_data_execution.svg` 与 PNG 预览。
...
- 同步更新生成脚本 `docs/thesis/assets/generate_system_architecture_figure.py` 和质检记录 `docs/thesis/assets/figures/system_architecture_ai_data_execution_audit.md`。
- 当前图稿用于人工确认系统架构图方向，尚未自动替换到开题报告正文或重新生成 PPTX。

## 2026-07-13 系统架构图初稿

- 使用 `figure-designer` 重新设计开题系统架构图，图类型定位为 Solution Overview / System Architecture。
- 新增可复现生成脚本 `docs/thesis/assets/generate_system_architecture_figure.py`。
- 生成 SVG 与 PNG：
  - `docs/thesis/assets/figures/system_architecture_ai_data_execution.svg`
  - `docs/thesis/assets/figures/system_architecture_ai_data_execution.png`
- 新增质检记录 `docs/thesis/assets/figures/system_architecture_ai_data_execution_audit.md`，检查向量格式、字体、颜色、图注和无装饰性图表元素。
...
- 新增 `docs/thesis/feishu/README.md`，规定本地 Markdown 是源稿、飞书是发布面。
- 后续实际写入飞书时使用 `lark-doc`；如遇嵌入 Base 或飞书幻灯片，再分别使用 `lark-base` 和 `lark-slides`。

## 2026-07-12 默认参考 skill

- 在 `docs/thesis/AGENTS.md` 中登记开题工作默认参考 skill：`karpathy-guidelines`、`humanizer`、`academic-research-suite`、`deep-research`、`nature-academic-search`、`vibe-research-workflow`、`ppt-master`、`nature-paper2ppt`、`lark-doc`、`lark-base`、`lark-slides`。
- 在 `docs/thesis/literature/reading_list.md` 中明确：文献检索优先使用 `nature-academic-search`，系统综述和研究缺口判断按需使用 `deep-research` 和 `academic-research-suite`。
- 补充说明：skill 是方法参考，不是固定流程；后续执行要结合本项目真实阶段、已有实验、学校模板和导师要求灵活使用。

## 2026-07-12 ECharts 图表规则

- 新增 `docs/thesis/assets/echarts_rules.md`，记录 ECharts SSR 生成 SVG、sharp 转 PNG、PPT 用 PNG、Word / 报告用 SVG 的流程。
- 在 `docs/thesis/ppt_rules.md` 和 `docs/thesis/assets/README.md` 中登记 ECharts 图表生成与嵌入规则。
- 在 `docs/thesis/AGENTS.md` 中补充 `figure-designer` 和 `nature-figure` 作为图表设计和论文级图表审查的可参考 skill。

## 2026-07-12 开题导航规则

- 新增 `docs/thesis/navigation.md`，说明开题材料需要项目内容、实验结果、文献、PPT 素材和飞书同步信息时分别从哪里找。
- 在 `docs/thesis/README.md` 和 `docs/thesis/AGENTS.md` 中登记 `navigation.md` 为开题目录阅读入口。
- 明确报告、PPT、飞书版之间的关系：报告负责完整论证，PPT 负责现场讲解，飞书负责发布同步，三者必须保持同一口径。

## 2026-07-12 开题报告与 PPT 初稿

- 补全 `docs/thesis/report/opening_report.md`，形成开题报告初稿，覆盖研究背景、国内外现状、研究目标、关键问题、技术路线、初步实验、预期创新点、进度安排和预期成果。
- 补全 `docs/thesis/slides/opening_ppt.md`，形成 16 页 PPT 内容源稿，并为每页加入 `汇报讲稿` 和 `答辩备注`。
- 更新 `docs/thesis/feishu/progress_update.md`，同步当前已完成内容、真实 GPU-backed 实验事实、问题、下周计划和待确认事项。
- 更新 `docs/thesis/literature/reading_list.md`，补充 Snowflake AISQL、Ray、Daft、Arrow、Lance、pgai、pgvector、PostgresML、Spark SQL tuning 等候选资料分类和精读顺序。
- 扩展 `docs/thesis/qa_bank.md`，补充 `AI_EMBED` 优先级、PG18.4/PG18.3 边界、pgvector 写回边界、模型推理淹没外部链路、传统查询优化区别等答辩问题。
- 更新 `docs/thesis/README.md`，将报告、PPT、飞书、文献和问答状态从占位阶段同步为初稿阶段。
...
- 调整 `docs/thesis/report/opening_report.md`、`docs/thesis/slides/opening_ppt.md`、`docs/thesis/outline.md`、`docs/thesis/README.md`、`docs/thesis/feishu/progress_update.md` 和 `docs/thesis/qa_bank.md`。
...
- 根据用户反馈，清理 `docs/thesis/report/opening_report.md` 中“当前最适合写入开题”“本文件用于承接学校模板”“草稿”等工作流提示。
...
- 在 `docs/thesis/templates/README.md` 中记录：最终 DOCX 必须使用学校 Word 模板生成，继承章节样式、字体、行间距、图表标注和参考文献格式。

## 2026-07-12 继续完善报告与飞书源稿

- 补充 `docs/thesis/report/opening_report.md` 的进度安排细节、预期关键技术指标和主要参考文献初稿。
- 新增 `docs/thesis/feishu/opening_report_wiki.md`，作为“开题报告与开题汇报”飞书 wiki 的本地源稿。
- 更新 `docs/thesis/feishu/README.md`、`docs/thesis/README.md` 和 `docs/thesis/navigation.md`，登记飞书 wiki 源稿入口。
- 更新 `docs/thesis/feishu/progress_update.md`，同步当前“已按模板重排报告”和“已新增飞书 wiki 源稿”的进展。

## 2026-07-12 研究内容写法规则

- 根据用户提供的既往开题反馈经验，明确“研究内容”不能写成工程任务清单，应写成“问题、技术难点、拟采用方法、评估方式”的闭环。
- 重写 `docs/thesis/report/opening_report.md` 第 3.2 节，将原先的执行路径画像、batch 划分、endpoint routing、写回优化和多 workload 验证，改为可检验的研究问题。
- 同步更新 `docs/thesis/feishu/opening_report_wiki.md` 的研究内容口径，便于后续补全飞书文档。
- 更新 `docs/thesis/work_rules.md`，记录研究内容写作规则：工程步骤放在技术路线或进度安排里，研究内容必须说明为什么难、怎么解决、如何证明有效，并包含对比对象、评价指标、消融实验和适用边界。

## 2026-07-12 研究内容范围收敛

- 根据用户反馈，进一步区分“研究手段”和“研究内容”：阶段划分、执行画像和瓶颈归因保留为动机测试、方案设计和评价基础，不再作为独立研究内容或预期创新点。
- 将 `docs/thesis/report/opening_report.md` 第 3.2 节收敛为三个方法问题：模型服务感知的批处理执行调度、写回压力感知的结果汇聚与持久化协同、多类数据库 AI 算子的策略选择与适用边界。
- 使用 `humanizer` 检查并改写相关段落，减少机械模板句式和元话语，保留正式开题报告语气。
- 同步更新 `docs/thesis/feishu/opening_report_wiki.md` 和 `docs/thesis/report/opening_report.md` 的预期创新点。

## 2026-07-12 可行性分析表格化

- 根据用户反馈，重写 `docs/thesis/report/opening_report.md` 第 4.2 节，用表格说明已有实验链路、GPU-backed `AI_EMBED` 分阶段结果、双 endpoint Ray 动机测试和 fake/CPU 历史预研。
- 可行性分析明确区分正式 GPU-backed 实验事实、PG18.4 本地预演事实、fake/CPU 预研和不能声称的边界。
- 同步更新 `docs/thesis/feishu/opening_report_wiki.md` 的初步实验依据部分，补充关键结果表和预研结果使用方式。

## 2026-07-12 正文去除写作元话语

- 根据用户反馈，清理 `docs/thesis/report/opening_report.md` 研究内容中的“该部分不把目标写成”“不能写成”等写作提醒式表述。
- 将相关句子改为正式研究表述，直接说明研究对象、变量、评价方法和适用边界。
- 同步清理 `docs/thesis/feishu/opening_report_wiki.md` 中同类表达。
...
- 使用 user 身份将 `docs/thesis/feishu/opening_report_wiki.md` 覆盖写入开题飞书 wiki：`https://my.feishu.cn/wiki/GCxowlVJbinzgRkoHDmc06cSn9J?from=from_copylink`。
...
- 新增 `docs/thesis/assets/figure_plan.md`，记录开题必须优先完成的三类图：课题总体研究框架图、端到端执行路径与阶段画像图、可行性实验关键结果图组。
- 在 `docs/thesis/report/opening_report.md` 第 3 节新增“总体研究框架”，用 Mermaid 描述三类 AI 算子场景、可观测批处理执行过程、三项研究内容和评价证据之间的关系。
- 同步更新 `docs/thesis/feishu/opening_report_wiki.md` 的研究内容部分，加入总体研究框架图源稿。
...
- 更新 `PROJECT_OUTLINE.md`、`PROJECT_INDEX.md` 和 `docs/thesis/work_rules.md`，要求实验分析说明实验目的、链路流程、参数含义、数据来源、结果读法、不能证明内容、结论类型和下一步验证。
- 明确正式报告可以更凝练，但结论边界和分析精细程度不能低于学习材料中的标准。

## 2026-07-12 开题飞书按学校模板重排

- 根据用户要求，严格参照 `docs/thesis/templates/硕士生开题报告模板0604.docx` 的大标题与章节结构调整开题飞书 wiki。
- 将 `docs/thesis/feishu/opening_report_wiki.md` 从汇报式结构改为模板式报告结构，保留封面占位、`1. 课题背景、目的和意义` 到 `7. 主要参考文献` 的章节顺序，并移除“当前主线”“一句话说明”“初步实验依据”“后续计划”“需要确认”等模板外标题。
...
- 新增 `docs/thesis/feishu/motivation_feasibility_wiki.md`，作为“动机测试与可行性测试”飞书 wiki 的本地源稿。
...
- 使用 `docs/thesis/templates/opening_ppt_template_version_v6_long_notes_source_checked.pptx` 作为学校 PPT 模板，基于 `docs/thesis/report/opening_report.md`、`docs/thesis/slides/opening_ppt.md` 和 GPU-backed 动机实验生成 16 页开题汇报 PPTX。
- 生成目录：`docs/thesis/archive/opening_defense_20260712/`；最终交付文件：`docs/thesis/slides/opening_defense_20260712.pptx`。
...
- 将 `docs/thesis/report/opening_report.md` 与 `docs/thesis/feishu/opening_report_wiki.md` 中的总体研究框架图和三张 GPU-backed 实验结果图，从 Mermaid 图替换为本地 PNG 图片引用。
...
- 更新 `docs/thesis/feishu/README.md`，记录本地 PNG 不能仅依赖 Markdown 覆盖导入，后续同步需配合 `docs +media-insert`。
...
- 同步更新 `docs/thesis/report/opening_report.md`、`docs/thesis/feishu/opening_report_wiki.md` 和 `docs/thesis/assets/charts/experiment_charts_audit.md` 中对图 4-5 的说明。
...
- 根据用户要求，新增 `docs/thesis/assets/charts/scripts/generate_all_meaningful_experiment_charts.py`，把项目中对研究有解释价值的实验数据都生成候选图。
- 输出目录为 `docs/thesis/assets/charts/all_meaningful/`，共生成 14 张 PNG / SVG 图，覆盖 CPU/GPU endpoint、PG18.4 fake-model、fake/CPU 历史预研和 feasibility 组件 benchmark。
...
- 更新 `docs/thesis/assets/README.md`、`docs/thesis/assets/charts/scripts/README.md` 和 `docs/thesis/assets/charts/experiment_charts_audit.md`，明确这些图是候选图集，报告正文仍应优先使用 GPU-backed 主证据。
...
- 新增 `docs/thesis/assets/charts/selected_motivation_figures.md`，记录主线图、备用图、不建议进入主线的图和推荐讲解顺序。
...
- 根据用户补充要求，在 `docs/thesis/report/opening_report.md`、`docs/thesis/feishu/opening_report_wiki.md` 和 `docs/thesis/assets/charts/selected_motivation_figures.md` 中补充“三类 workload 为什么选”和“为什么调 batch / partition / task / actor / routing / backpressure / writeback”的依据说明，明确依据来自外部系统资料和项目实验信号，而不是主观选择。
...
- 为方便查找，新增集中目录 `docs/thesis/assets/charts/selected/`，其中 `report_main/` 存放报告和 PPT 正文建议图，`ppt_backup/` 存放 PPT 备份和飞书补充图；同步将开题报告本地正文和飞书源稿的图片引用切换到 `selected/report_main/` 下的短文件名版本。
...
- Refined `docs/figures/architecture/cross_layer_method_framework.png` / `.svg` into a research-plan figure for Section 4.1.
- Inserted the figure into `docs/thesis/report/opening_report.md` and local Feishu source `docs/thesis/feishu/opening_report_wiki.md` as Figure 4-1; renumbered subsequent Section 4 figures.
...
- Synchronized the research-plan figure drawing cautions into `docs/figures/AGENTS.md` and `docs/thesis/ppt_rules.md`.
...
- Regenerated the opening-report architecture figures:
  `docs/figures/architecture/system_architecture_ai_data_execution.*`,
  `docs/figures/architecture/cross_layer_method_framework.*`, and
  `docs/figures/architecture/runtime_strategy_control_loop.*`.
- Updated `docs/thesis/report/opening_report.md` and `docs/thesis/feishu/opening_report_wiki.md`: Figure 4-1 now states the three-layer upstream execution strategy, and Figure 4-2 now uses the runtime control-loop figure instead of the previous Mermaid chain sketch.
...
- Updated `docs/thesis/report/opening_report.md` and `docs/thesis/feishu/opening_report_wiki.md` so the prose matches the current figures.
...
- 从 `docs/thesis/slides/opening_defense_20260720_v4.pptx` 拷贝生成 `docs/thesis/slides/opening_defense_20260720_v5.pptx`。
- 按用户要求未重跑 `docs/thesis/slides/build_ppt.py`，仅使用 `python-pptx` 对 v5 做增量修改。
...
- 按用户反馈新增 v5 第 22 页“后续工作：多模态泛化验证”，插入在“可行性与创新点”和“进度安排”之间。页面使用 `docs/figures/ppt_cropped/b26_arrow_vs_daft_stage_breakdown.png`，说明 Daft 接入后的 DB 读取与组织开销仍小于 0.1s，从而支撑后续将同一套 Daft/Ray pipeline 从 prompt 列扩展到 image/frame 列；该页明确多模态是后续验证目标，不声称已经完成。
## 2026-08-07 开题叙事与证据边界冻结

- 新增 `docs/thesis/claim_matrix.md`，作为报告、PPT、答辩问答和开题实验准入的当前内部依据。
...
- 新增 `docs/thesis/first_principles_reassessment_20260808.md`：开题前完成合格强静态 baseline 与必要性证据，开题后再完成同上限 static/dynamic、变化负载、多 job 和图像统一 formal。
...
- 在 `docs/thesis/claim_matrix.md` 建立发布材料 readiness 表，分离内容/证据冻结与图、PPT、
...
- 更新 `docs/thesis/slides/README.md`、v7 QA、图文一致性审计、`docs/thesis/README.md`、
  `docs/thesis/claim_matrix.md`、`PROJECT_INDEX.md` 与根日志。中文 Markdown 报告和飞书云文档
  后续继续，Wiki 不同步。

## 2026-08-12 中文开题报告按学校模板收口

- 将 `docs/thesis/report/opening_report.md` 重组为学校模板要求的七部分，先定义两项研究内容与
  共同代价估计，再在研究方案中说明数据结构、固定上限调度实现、实验设计和前期可行性证据。
- 14/14 张主讲图和 2/2 张单作业补充图统一改从 `docs/figures/opening_figure_set/` 引用，移除报告
...
- 新增 `docs/thesis/report/opening_report_20260812_qa.md`，完成学校模板、31 条引用、图表、实现边界
...
- 调整 `docs/thesis/report/opening_report.md` 中图 5 至图 11 的正文标题，使标题直接说明图中比较的对象或主要现象；未修改图片文件及实验数据。
...
- 根据用户复核，将“正式对外材料不得直接照搬项目管理用语”加入根目录和 `docs/thesis/` 的长期规则；要求写出实际对象、条件和动作，并在定稿前执行高风险词与未解释英文检查。
...
- 调整 `docs/thesis/report/opening_report.md` 第 3.2.2 节和第 4.1 节：研究内容只说明固定模型服务能力下需要研究的提交、作业选择和服务实例路由问题，研究方案只保留主要比较思路，不在这些位置展开差额轮转的内部记账过程。
...
- 修正报告图 2、图 3、图 5 和图 6，并同步 `docs/figures/architecture/editable/`、`docs/figures/data/report_main/`、`docs/figures/opening_figure_set/` 与 `docs/thesis/report/figures/` 中的对应文件；未修改桌面 `0819.pptx`，未生成 Word 文档。
...
- 同步改写正文图注与 `opening_report_20260820_qa.md`，删除只为解释旧图而存在的 `contract`、`frozen-static`、`vendor scheduler ownership` 等内部说法。图源、尺寸、SHA256、能够说明与不能说明的内容记录于 `docs/figures/audit/opening_report_minimal_figure_corrections_audit_20260823.md`。

# 2026-08-23 开题报告图文关系与证据口径复核

- 保留 `docs/thesis/report/figures/` 中现有图片，不重绘图片，也不修改桌面 `0819.pptx`。
...
- 核对 `docs/research/精读文献笔记/` 的八篇全文精读材料和 65 张论文原图裁剪件，并将五个不符合目录命名规则的主笔记改为与所在目录同名；配套图形审计中的使用路径同步更新，笔记正文内容不改。
...
## 2026-08-24 Galois 精读配图状态同步

- Galois 精读主笔记新增 Figure 1–4 和 Figure 7–11 共 9 个论文原图裁剪件；选图、页码、SHA256 与视觉检查保存在 `docs/figures/audit/galois_deep_reading_figures_audit_20260824.md`。
- `docs/thesis/README.md` 和 `docs/thesis/literature/reading_list.md` 的现行精读统计同步为 9 篇主笔记、74 张论文原图裁剪件。本次只同步导航与文献清单，未修改开题报告正文、PPT、飞书或 Top 15 历史快照。

## 2026-08-24 开题报告按评审意见重构

- 依据用户提供的逐章评审意见重写 `docs/thesis/report/opening_report.md`，保留学校模板七部分，但把研究结构固定为两项：分阶段工作描述与数据组织、固定准入范围内的请求释放/服务实例路由/多 Job 协调。算子代价估计改为共同使能组件，图像改为跨模态验证。
- 新增统一术语表，区分逻辑请求、工作项、提交批次、模型请求、服务内部批次、Job、工作负载类别、`N_max/W_max`、估计工作量、实际服务量和预测时间；补充计时范围、评价指标、风险替代方案和更新后的进度安排。
- 相关工作新增 IMBridge、IMLane、OceanBase AI Function、PostgreSQL 18 Custom Scan/版本状态与 vLLM 指标官方资料，参考文献由 51 条增至 60 条。IMLane 只采用官方已公开信息，并在正文注明全文和代码尚未公开。
- 新增 `docs/figures/architecture/opening_target_architecture_status.{png,svg}`，分开显示目标 PostgreSQL planner-visible 算子路径和当前外部可运行证据链；报告图片由 11 张压缩为 6 张。生成脚本、报告副本和图件审计已同步。
- 新增 `docs/thesis/report/opening_report_20260824_qa.md`，完成结构、术语、主张、图片、参考文献和外部事实复核。本轮未修改实验数据、Word、PPT、飞书和 Wiki；这些发布面后续恢复时必须以本地报告为源重新生成。

## 2026-08-24 开题报告第一、二章信息层次调整

- 第一章删除 LOTUS、Daft、Ray、vLLM 等具体实现链路，只保留 PostgreSQL 数据库内算子与外部物理执行的总体关系；第一章术语表由十项缩为五项，其余执行术语移到第三章首次使用处解释。
- 三类动机统一为工作表示、容量与运行状态、多 Job 完成目标。前者对应研究内容一，后两者共同对应研究内容二；代价估计继续标为支撑问题和共同使能组件。
- 将包含 29%、65K 和 97.8% 等详细数字的工作量/状态/容量图从第一章移到第 4.4 节。目标架构图改为图 1，该动机图改为图 2，其余图号不变。
- 第二章各小节减少对本课题具体字段、准入和调度方法的提前展开，完整研究差异集中到第 2.4 节。本轮未修改实验数据、图片文件、PPT、Word 或飞书文档。

## 2026-08-25 开题报告内部化表达局部清理

- 保持七部分结构、三个动机问题到两项研究内容的映射、实验数据和结论不变，只修改少量元叙事、抽象角色描述和进度安排中的生硬措辞。
- 将“形成三个同级问题”改为三个相互关联的问题，统一“语义工作项”术语，并直接说明代价估计向数据库计划比较和两项研究内容提供的运行前信息。
- 将外部物理执行与 PostgreSQL 接入的两条验证路径改为正面说明相同输入输出含义和记录对应关系；拆分整体系统完成情况中的长句，使已有条件和后续验证事项分开表述。
- 图 2 仅删除右上角“开题第 5 页｜研究问题”页码残留；图 7、图 15 按用户要求未修改。PPTX、Word、飞书、Wiki、参考文献和实验源数据均未修改。

## 2026-08-24 Palimpzest 精读配图状态同步

- Palimpzest 精读目录和主笔记名已按系统正确拼写更正为 `palimpzest_cidr2025`，并新增附件 arXiv v2 Figure 1–7 共 7 个论文原图裁剪件；选择、版本边界、页码、SHA256 与视觉检查保存在 `docs/figures/audit/palimpzest_deep_reading_figures_audit_20260824.md`。
- `docs/thesis/README.md` 和 `docs/thesis/literature/reading_list.md` 的现行精读统计同步为 10 篇主笔记、81 张论文原图裁剪件。本次只同步导航与文献清单，未修改开题报告正文、PPT、Word、飞书或 Top 15 历史快照。

## 2026-08-24 两条总动机与状态证据层级修订

- 将原有三类并列问题收敛为两条总动机，并与两项研究内容一一对应：记录数不能代表分阶段 AI 工作量；单一静态量或局部指标不能代表当前可调度状态。
- 动机二进一步分为配置层、模型服务层和 Job 层。容量上限、实际在途工作、吞吐与尾延迟、多 Job 进度和服务结果分别承担不同子证据，不再被写成多个平级研究任务。
- 第 4.4 节拆分为“核心动机证据”和“可行性与方法选择证据”。多 Job 共享图前移为图 3，用于补齐动机二的 Job 层；文本基线和数据组织图顺延为图 4、图 5。
- 重写工作量与状态动机图的父标题和 panel 标题，保留原始数据、坐标、误差线和统计口径；同步权威图源、开题图集副本、报告副本、图表审计、QA 和图片映射。
- 在根目录与开题目录规则中新增“总动机、子问题、证据实验、研究内容、实现任务”的层级检查，以及父项覆盖、兄弟同类和证据收敛三项自检。
- 同步更新 Claim Matrix、答辩内容大纲和本地状态说明；未生成或修改 PPTX、Word、飞书或 Wiki。

## 2026-08-24 Abacus 精读配图状态同步

- Abacus 精读目录和主笔记按精读合同统一为 `abacus_pvldb2026`，正式 PVLDB 版 Figure 1–8 的 8 个既有裁剪件已从错误的 `assets/` 引用接入对应正文；来源页码、SHA256、视觉检查和证据边界保存在 `docs/figures/audit/abacus_deep_reading_figures_audit_20260824.md`。
- `docs/thesis/README.md` 和 `docs/thesis/literature/reading_list.md` 的现行精读统计同步为 11 篇主笔记、89 张论文原图裁剪件。本次只同步导航与文献清单，未修改开题报告正文、PPT、Word、飞书或 Top 15 历史快照。

## 2026-08-24 Sema 精读配图状态同步

- Sema 精读主笔记已规范为 `sema_vldb2026/sema_vldb2026.md`，本地 arXiv v1 正文 Figure 1–8 的 8 个原图裁剪件已接入对应章节；来源版本、页码、SHA256、视觉检查和证据边界保存在 `docs/figures/audit/sema_deep_reading_figures_audit_20260824.md`。
- `docs/thesis/README.md` 和 `docs/thesis/literature/reading_list.md` 的现行精读统计同步为 12 篇主笔记、97 张论文原图裁剪件，并按当前工作区实体修正参考 PDF 状态。本次只同步导航与文献清单，未修改开题报告正文、PPT、Word、飞书或 Top 15 历史快照。
...
- `docs/thesis/AGENTS.md` 与 `docs/thesis/work_rules.md` 增加进度写作规则。本轮未改变研究问题、研究内容、实验结论、图片、PPTX、Word、飞书或 Wiki。
...
- 第二章按新增精读结果补充 Palimpzest、Abacus 和 Sema，分别说明其研究问题、核心方法、实验依据、适用条件及与本课题的衔接。
- 新增 Sema 为参考文献 [52]，修正 Abacus [21] 作者顺序；论文名称统一使用正式英文原题。
- 代价估计改为正面说明数据库计划比较、研究内容一的数据组织增强和研究内容二的调度增强；正文先描述角色和作用位置，再说明验证方式。
- 同步更新写作规则、报告 QA、开题导航和项目索引；PPTX、Word、飞书与 Wiki 未修改。

## 2026-08-24 开题报告参考文献正式版本复核

- 将报告 52 条参考文献按 [1]—[20]、[21]—[42]、[43]—[52] 分组逐条核对，优先采用会议/期刊官网、正式 proceedings、DOI 和官方技术文档。
- Sema 已由 VLDB 2026 官方程序确认为 Research Track 常规研究论文；报告改写为已录用状态，并明确正式摘要的 26 个查询与本地 arXiv v1 的 20 个查询属于不同版本。PVLDB 卷期、页码和 DOI 尚未公布，未提前补造。
- Autellix 的正式版本已更名为 Agentix，并发表于 NSDI 2026（2443—2459）；正文、参考文献和当前文献索引均切换到正式名称与版本。FairServe、DLPM、Ray Data Streaming Batch Model 和 CONCERTO 继续按预印本引用。
- 修正 GaussML 第三作者、Galois 首作者、NeuStream 首作者和一篇多租户数据库论文的正式题名，并为可核实的正式论文补充页码、文章号或 DOI。参考文献仍为 52 条，编号与正文引用关系不变。
- 新增“正式版本优先、发表状态与实际阅读版本分开记录”的写作规则。本轮未修改实验数据、图片、PPTX、Word、飞书或 Wiki。
...
## 2026-08-24 开题报告术语与实验口径收口

- 保留七部分结构和两项研究内容，只在现有段落中区分语义工作项、批次请求和模型服务请求；明确批次请求是交给调度模块的数据组织结果，不等同于外部模型服务实际接收的一次调用。
- 固定容量继续表示每个模型服务实例上的请求位置和预计工作量两个上限，并补充四作业静态分配中每个作业 `32 + 16,384`、四个作业合计 `128 + 65,536` 的对应关系。
- 研究目标、总体方案、进度和预期成果同步项目当前方向：PostgreSQL 18.3 扩展注册查询计划可识别的 AI 语义算子，文本场景复用 LOTUS v1.2.4 `sem_map` 语义；现有 `AI_COMPLETE` / `AI_EMBED` 外部路径仍作为物理方法前期证据。
- 将“完整数据库路径”改为“数据库读写端到端路径”。SQuAD 三臂因项目路径额外计入指标采集、运行记录和作业结束处理，不再按不到 1% 的差异作性能排名；结果完整性和答案质量仍可核对。
- 图 10 同步删除“SQuAD 结果可直接比较”的图内结论，改为“质量可核对，性能暂不排名”；未改变柱形、误差线或实验数值。图 4c 的作用改为承接容量与运行状态问题，图 14 的小规模三路径检查和 12 万规模两路径比较分别说明。
- 收紧数据组织实验的因果解释和两作业共享实验的收益归因；评价标准前移到实验设计，进度中删除研究内容二阶段与后续组合实验之间的重复事项。PPTX、Word、飞书和源实验数据未修改。

## 2026-08-25 相关工作图来源标注

- 相关工作图按最新精读内容更新代表系统，并在名称后以小号深灰字标出会议与年份；不使用文献编号和括号，两个过长条目将出处换到下一行。
- Ray Data 对应论文经作者公开论文列表核对后标为 NSDI 2027。Draw.io、SVG 与 4000×2250 PNG 已同步，供用户手工替换 PPT 第 5 页；PPT 和报告正文未修改。
