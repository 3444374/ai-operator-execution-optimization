...
- **Authority checked:** `docs/thesis/claim_matrix.md` and the project-level scope in `AGENTS.md`.

## Reference and style tokens

- Reference: `docs/figures/audit/reference_opening_editable_20260811/01_research_gap.png` only for three-column composition and visual grammar.
...
- 数据库侧第二张卡片标题缩为“输入、输出与写回”，避免长标题贴近右边界。
- “数据库通常不可见”卡片将队列、KV 缓存和模型完成速度拆为三行，使上下留白与右侧卡片一致。
- SVG、Draw.io、图集 PNG/SVG/Draw.io 与报告 PNG 已同步；1600×900 原尺寸复核无文字越界。
