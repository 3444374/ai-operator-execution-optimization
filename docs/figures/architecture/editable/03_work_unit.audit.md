...
- 参考：`docs/figures/audit/reference_opening_editable_20260811/03_work_unit.png`。
...
## 2026-08-23 工作描述与估计结果分离

- `Work Estimation` 改为“分阶段工作描述”，避免把运行前已知的工作特征全部写成预测值。
- 结构化工作描述（WorkDescriptor）只保留身份、阶段工作、数据局部性和服务信息；原来的 `confidence / uncertainty / calibration` 已从框内删除。
- “可选代价估计结果”使用独立橙色虚线卡片，位于 WorkDescriptor 外部，内容为预计时间、误差范围和校准状态。
- 图题、固定行数分批的局限、候选批次组织方式、批次工作量上限和评价条件均改为直接中文说明。1600×900 预览确认估计卡片没有遮挡主箭头，所有文字均位于卡片内。
