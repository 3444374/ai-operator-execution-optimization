...
- 参考：`docs/figures/audit/reference_opening_editable_20260811/02_system_architecture.png`。
- 当前口径来源：`docs/thesis/claim_matrix.md`、`docs/thesis/opening_defense_outline_20260808.md` 第 11 页及多模态段落、`docs/figures/audit/opening_editable_diagrams_manifest_20260811.md`。
...
- Cost Estimator 由“WorkDescriptor + 代价估计”（易被误读为研究内容一内部字段生成器）改为独立共享组件：estimate 卡还原为纯 WorkDescriptor（复用原 descriptor 图标，删除计算器 estimate 图标），新增淡紫底虚线 `Shared Cost Estimator` 横条横跨两项研究内容，表达“共同使能”。遵循 docs/figures/AGENTS “能不用箭头就不用”，未加指向箭头，靠位置与颜色表达共享关系；所有其余图标原样保留，Daft 官方标识未改。
...
- 渲染：本机 Windows 环境曾尝试 playwright-core 驱动在线 draw.io 编辑器导出，但 headless 下编辑器用 Canvas 而非 SVG 渲染，icon 与文字丢失（见下条）；最终改用 draw.io desktop CLI + Python 后处理 + headless Chrome 的管线（详见下条）。同步刷新 `docs/figures/opening_figure_set/` 的 P11 drawio/PNG/SVG 副本。
...
- 同步：权威源 `02_system_architecture.{drawio,svg,png}` 与 `docs/figures/opening_figure_set/` 的 P11 drawio/PNG/SVG 副本均已更新为最终版。
...
## 10. 代价估计与运行状态分离（2026-08-23）

- 总图改为 1600×900 的清晰结构：数据源与数据引擎、AI 数据执行层、模型执行服务、结果收集与写回、运行状态记录五个区域。
- 工作描述只保存阶段工作、数据局部性、作业身份和服务信息。AI 算子代价估计读取工作描述与历史测量，输出预计时间和资源需求，并只连接数据库优化器 / 多 SQL 调度。
- 运行状态记录保存实际排队、运行、KV 缓存、完成速度和作业进度，虚线只返回提交控制和路由模块。图内明确说明运行时事实不等同于代价估计结果。
- 所有可见的 `contract`、`frozen-static`、`safe capacity` 和“状态感知调度闭环”等旧标签已经删除。预览确认主路径、数据库计划路径、结果写回和状态返回没有相互穿越或遮挡。
