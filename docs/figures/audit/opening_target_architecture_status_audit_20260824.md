...
- 权威矢量图：`docs/figures/architecture/opening_target_architecture_status.svg`
- 权威位图：`docs/figures/architecture/opening_target_architecture_status.png`
- 历史 / 内部候选副本：`docs/thesis/report/figures/target_architecture_status.png`、`docs/thesis/report/figures/fig05_system_architecture.png`
- 生成脚本：`docs/figures/scripts/generate_opening_target_architecture_status.py`
...
该图为概念架构与实现状态候选图，不包含新增实验数据。内容来源是项目总纲、`docs/thesis/claim_matrix.md` 和现有实现记录。2026-08-24 的候选修订不在图内写具体语义运行时名称，只区分计划中的数据库内 AI 语义算子入口与当前已经运行的外部实验执行路径。用户复核后决定当前报告不引用该图，资产仅作历史 / 内部候选保留。
