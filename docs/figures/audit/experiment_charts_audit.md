...
本记录对应项目级图资产库 `docs/figures/`。旧的 `docs/thesis/assets/charts/` 仅作为历史生成路径，不再作为正式引用目录。

## 当前正式图目录

- 报告和 PPT 正文主图：`docs/figures/data/report_main/`
- PPT 备份页、飞书补充和后续讨论图：`docs/figures/data/backup/`
- 生成脚本：`docs/figures/scripts/`
- 图表筛选说明：`docs/figures/data/selected_motivation_figures.md`
...
- 图文一致性：通过。报告正文、飞书本地稿和图表筛选说明已统一引用 `docs/figures/` 下的正式图。
...
- 正式材料优先引用 `docs/figures/architecture/` 和 `docs/figures/data/report_main/`。
- 解释实验设计来源、答辩追问或 PPT 备份页可引用 `docs/figures/data/backup/`。
- 中间生成目录可以删除；再次需要时用 `docs/figures/scripts/` 从 CSV 重新生成。
- 旧路径 `docs/thesis/assets/charts/`、`docs/thesis/assets/figures/` 和 `learning/figures/` 不再维护正式图副本。
