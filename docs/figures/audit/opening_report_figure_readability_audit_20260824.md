...
`docs/thesis/report/opening_report.md`；未修改 PPT。结论如下：
...
|---:|---|---|---|---|
| 图 1 | `docs/thesis/report/figures/fig01_external_ai_operator_assumptions.png` | `docs/figures/architecture/editable/opening_background_20260812/07_traditional_vs_external_ai_operator.drawio` 与同名 SVG | Draw.io 为结构源；本轮同步修改 SVG，并用本机无头 Chrome 从 SVG 重导 1600×900 PNG | 改术语并同步开题图集副本 |
| 图 6 | `docs/thesis/report/figures/fig07c_state_fingerprint.png` | `docs/figures/data/report_main/opening_native_single_job_state_fingerprint.{png,svg}` | `docs/figures/scripts/generate_opening_story_figures_20260808.py --figures F` | 改轴名与底部说明，不改数据 |
| 图 9 | `docs/thesis/report/figures/fig04_state_aware_scheduling.png` | `docs/figures/architecture/editable/04_state_aware_scheduling.{drawio,svg,png}` | Draw.io / SVG 可编辑源；审计见同目录 `04_state_aware_scheduling.audit.md`；无头 Chrome 重导 PNG | 可见框标题、说明和状态字段中文化 |
| 图 10 | `docs/thesis/report/figures/fig06_text_baseline_boundaries.png` | `docs/figures/scripts/generate_opening_story_figures_20260808.py` 中 `figure_text_baseline_evidence_map` | `--figures T` 生成 `opening_text_baseline_evidence_map.{png,svg,pdf}` | 收紧左图比较说明，不改数据 |
| 图 13 | `docs/thesis/report/figures/fig14a_cost_prediction_time.png`、`fig14b_cost_ranking_decision_loss.png` | `docs/figures/scripts/generate_opening_story_figures_20260808.py` 中 `figure_cost_decision_v4` | `--figures E` 生成原合成图以及两张拆分图 | 原合成图备用，正文使用图 13a / 13b |
| 图 14 | `docs/thesis/report/figures/fig12_image_baseline_boundaries.png` | 同一脚本中的 `figure_image_baseline_evidence_map` | `--figures I` 生成 `opening_image_baseline_evidence_map.{png,svg}` | 只改可见文字，不改数据 |
...
- 同步修改 `docs/figures/architecture/editable/01_research_gap.drawio` 与 `01_research_gap.svg`，并重新导出 `docs/thesis/report/figures/fig02_ai_data_execution_gap.png`。报告 PNG 保持 1600×900。
