...
> `docs/figures/audit/opening_required_data_figures_20260810.md` 第一性原理重审并冻结为八张：
> work/state 动机、文本 baseline 分轨、原生四 Job 归一化干扰、组织 regime、Project 四 Job
> 机制与公平权衡、图像 staged-work 动机、图像 baseline 分层、代价估计决策质量。原生单 Job 状态指纹降为报告/答辩备份；图像与
> DuckDB 多 Job、同上限 phase-change 因无正式结果不画。后续选图以该审计为准。

> **2026-08-24 叙事层级修订**：开题动机按三个问题组织，而不是把 work、状态观测、动态调度
> 和代价估计列为四个平级动机。问题一是记录数不能代表分阶段 AI 工作量；问题二是单一静态量
> 或局部指标不能代表当前运行状态；问题三是多个数据库作业共享模型服务时会相互影响。问题一
> 对应研究内容一，问题二与问题三共同对应研究内容二；代价估计是两项内容的共同支撑，图像还
> 用于跨模态验证。共享额度的结果属于研究内容二的初步方法结果，不能代替问题三的原生多作业
> 干扰证据。实验数据与统计口径没有变化。
...
后续维护资产时，以根目录 `docs/figures/` 为正式图资产库。旧的 `docs/thesis/assets/charts/python/`、`docs/thesis/assets/charts/all_meaningful/` 和 ECharts 根目录图已经不再作为正式材料引用；如需重新生成候选图，使用 `docs/figures/scripts/` 中的 Python 脚本和原始 CSV。
...
```text
docs/figures/architecture/system_architecture_ai_data_execution.png
docs/figures/architecture/system_architecture_ai_data_execution.svg
```
...
```text
docs/figures/architecture/research_gap_three_islands.png
docs/figures/architecture/research_gap_three_islands.svg
```
...
```text
docs/figures/data/report_main/02_gpu_stage_latency_stack.png
docs/figures/data/report_main/02_gpu_stage_latency_stack.svg
```
...
```text
docs/figures/data/report_main/03_invocation_granularity.png
docs/figures/data/report_main/03_invocation_granularity.svg
```
...
```text
docs/figures/data/report_main/04_executor_endpoint_comparison.png
docs/figures/data/report_main/04_executor_endpoint_comparison.svg
```
...
```text
docs/figures/data/report_main/05_actor_endpoint_scaling_writeback.png
docs/figures/data/report_main/05_actor_endpoint_scaling_writeback.svg
```
...
```text
docs/figures/data/backup/b01_workload_matrix_speedup.png
```
...
```text
docs/figures/data/backup/b02_granularity_attribution.png
```
...
```text
docs/figures/data/backup/b03_backpressure_queue_pressure.png
```
...
```text
docs/figures/data/backup/b04_writeback_batching.png
```
...
```text
docs/figures/data/backup/b05_ray_arrow_fanout_fanin.png
```
...
```text
docs/figures/data/backup/b06_stage_share.png
docs/figures/data/backup/b06_stage_share.svg
```
...
|---|---|---|
| `docs/figures/data/report_main/06_gpu_pgai_rerun_granularity_20260714.png` / `.svg` | Shows that fine-grained 1024-row AI_EMBED calls are 37.5x slower than coalesced calls in the GPU-backed local chain. | PG18.4 local rehearsal, no writeback, one formal run. |
| `docs/figures/data/report_main/07_gpu_pgai_rerun_stage_writeback_20260714.png` / `.svg` | Shows model request wall time and JSON writeback time in the same full-chain timing view. | JSON text writeback, not pgvector vector(384). |
| `docs/figures/data/report_main/08_gpu_pgai_rerun_endpoint_comparison_20260714.png` / `.svg` | Shows dual local endpoints reduce model/operator wall time while writeback remains similar. | Two endpoint replicas on one RTX 5070, not multi-GPU. |
