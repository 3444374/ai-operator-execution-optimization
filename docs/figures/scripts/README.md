...
```bash
python3 docs/figures/scripts/generate_opening_target_architecture_status.py
```

脚本同时生成 `docs/figures/architecture/opening_target_architecture_status.{png,svg}` 和
`docs/thesis/report/figures/target_architecture_status.png`。图中上层是计划实现的 PostgreSQL
...
`docs/figures/audit/opening_target_architecture_status_audit_20260824.md`。

## 开题图集 SVG 便携化

```bash
python3 docs/figures/scripts/embed_svg_assets.py docs/figures/opening_figure_set/main_svg/P05_研究空白_AI数据执行层.svg
```
...
```bash
python docs/figures/scripts/generate_opening_core_evidence_figures.py
```
...
输出为 `docs/figures/data/report_main/opening_*.{svg,png}`。脚本排除 warm-up，保留 formal 重复或折级分布；图像 matched-resource headline 冻结为约 13–15%，代价估计明确标注 14.72% max regret 的 marginal-pass 风险。完整设计合同与视觉 QA 见 `docs/figures/audit/opening_core_evidence_figures_contract_20260807.md`。
...
```text
docs/figures/data/generated_gpu/
```
...
- `docs/figures/data/generated_gpu/` 是重新生成时的中间输出目录；正式材料只引用 `docs/figures/data/report_main/` 或 `docs/figures/data/backup/` 下筛选后的副本。
...
```text
docs/figures/data/generated_all_meaningful/
```
...
- `docs/figures/data/generated_all_meaningful/` 是重新生成时的中间输出目录；未进入 `report_main/` 或 `backup/` 的 PNG / SVG 不长期保留。
