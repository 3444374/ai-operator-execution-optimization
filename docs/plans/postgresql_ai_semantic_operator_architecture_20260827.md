...
|---|---|---|
| 扩展SQL中的算子调用 | 调用出现、输入依赖、求值范围、结果消费者与合法计划位置 | 共同调用/列绑定、一个 Filter→一个生成 Map、两个 Filter AND 已验证；多个 Map/嵌套与更宽 SQL 形状未验证 |
| 同一算子的不同方法 | 语义要求约束可选方法；方法产生任务并解释完成值 | 同步 reference 与外部 MethodDriver 已有；PG 方法桥接与真实第二路径未实现 |
| 有界数据执行 | 取数、表示转换、组织、准备、提交、结果消费分别持有资源 | 生成型 Map 的 PG 多在途（v6）与单算子总量留存预算已验证；accepted-prefix、乱序 completion 与更广组合未实现 |
| 多查询、多算子流共享资源 | 查询执行属于调度组，组内含多个流；连接数量不决定公平份额 | 共享 Engine、可信 Job 登记与查询级 Job 归属已验证；Job 份额策略与动态借用未接入 |
| 正确性与可靠性 | 可复制计划、单次执行、节点状态与远端任务的寿命分开 | 同步受限路径与生成型 Map 多在途已验证；rescan/EPQ、乱序 completion 与重连需新合同 |
| 后端与模态替换 | 语义能力、数据表示、工作量和部署配置分别适配 | 文本 vLLM 与图像 CLIP 已接通（embed 三路径数值一致）；GPU 计算中故障、跨模态 held-out 与统一 sink 未验证 |
| 估计、质量与反馈 | 观测共享，计划选择、运行调度、质量授权各有决策者 | 离线估计器与 planner-only artifact 机制已验证（deterministic fixture）；真实 matched 校准与在线反馈接线未完成 |
...
2026-09-14 研究优先级：先按[设计假设与强对照](data_organization_batching.md#design-hypotheses)
检验等待位置及有限候选/全局元数据的成本，再按证据补阶段或多查询适配。已有受限 Map v6、共享 Engine、
