...
- **embedding endpoint**：BGE **不是 vLLM 生成式模型**，走独立 HTTP endpoint（FastAPI 跑 BGE）——复用项目现有 Ray→HTTP→endpoint→trace 机械（与 AI_EMBED 文本预研的 custom endpoint 同模式，`results/motivation/gpu/`）。`@daft.cls` Native 作 baseline 不作主路径。
