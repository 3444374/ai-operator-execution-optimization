...
**已有实验基础**：
- `results/motivation/gpu/ai_embed_chain_breakdown_20260712.md`：coalesced vs fine = 13.4×
- `results/motivation/fake_cpu/analysis.md`：fine/coalesced e2e 比值约 4.01-4.37×
...
**已有实验基础**：
- `results/motivation/gpu/ai_embed_chain_breakdown_20260712.md`：16384 行下 `writeback_s = 6.586s`（占总时间 50%）
- `results/motivation/gpu/pgvector_writeback_20260714.md`：JSON text (1.567s) vs pgvector(384) (0.897s) 的对比
...
- 这是 B 系列实验的必做项。在进入 研究内容三 方法设计之前，必须跑完 B1-B3 确认：
  ...
  - B3: 在线索引维护 vs 事后建索引
- 结果写入 `results/`，不在动机测试目录中放写回 engineering baseline 数据。
