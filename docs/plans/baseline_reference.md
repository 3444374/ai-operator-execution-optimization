...
| `../results/` 与 `../../results/motivation/` | 原始数据、七步分析和结论的权威来源 |
...
<a id="data-execution-design-controls"></a>
### 0.4 数据执行设计假设：信息与物化分别比较（2026-09-14）

适用于[当前设计假设实验](data_organization_batching.md#design-hypotheses)，尚未运行，不恢复旧外部执行矩阵。
完整实验单元与运行授权由专项计划维护，本节只规定对照关系。

| 比较目的 | 必须保留的对照 | 共同条件与代价 |
|---|---|---|
| token控制是否超出强请求数控制 | 独立调优请求数FIFO；同C、W不生效的token表征FIFO；同排序、W生效的token控制 | 输入、消息/生成、物化预算、返回序和观测相同；token化准备/常驻成本单列，JCT含查询专属准备 |
| 局部观察与全局信息的取舍 | 流式FIFO；同物化预算有限候选组织；全局轻量ID/work信息规划后有界执行 | selector归因时相同可见信息；完整方案比较允许信息不同，但扫描/统计/排序/额外读取/metadata存储都付费 |
| 查询消费信息是否有增量 | 同候选、同请求/work/字节范围下的FIFO、简单长度排序与消费进度候选 | 未提交工作才可重排；必须实际改变读取/提交/释放，不能只改组标识；节点交付和客户端收到分别报告 |
| 多查询是否受过早提交影响 | 相同到达下的共享FIFO、既有公平控制与明确命名的候选；同资源matched solo | 报各查询JCT/slowdown、长期无服务区间和长查询代价；已提交不可抢占work独立计数 |

这些内部对照由SemLoom拥有执行/调度，不能命名为原生Ray Data、VTC、Kalypso或BlendServe。
原生系统对照仍让原实现拥有调度；无法匹配语义或资源时只作机制参照。
全局元数据可占额外内存或合理落盘，但必须在同一可用总资源范围内分域报告，不能只约束payload后免费增加巨大元数据。
只降低提交后P99、只报告成功子集、或只把准备移到计时外，都不能证明完整查询改善。
...
项目写回 baseline 为 PostgreSQL + pgvector 的 COPY + 延迟建索引（先 COPY 到 unlogged table，再 `CREATE INDEX HNSW`），已在动机 GPU 实验中建立（pgvector writeback 0.897s vs JSON 1.567s，见 `results/motivation/gpu/`）。按根 `AGENTS.md`“项目范围”和 `PROJECT_OUTLINE.md`，写回作为工程 baseline 处理，不作为独立实验阶段；本节 W1 仅作 baseline 出处登记，不再设为待跑门禁实验。
