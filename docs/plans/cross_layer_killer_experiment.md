...
> **当前状态（2026-08-27）**：本计划是条件性耦合验证，不是当前运行项。已有本地联合候选没有
> 显著优于独立最优拼接；只有在 PostgreSQL 中立语义算子/provider 资格项完成、两项策略各自冻结可复现配置后，
> 才允许按本文恢复系统级联合比较。

整理日期：2026-07-16

> **2026-07-17 口径更新**：本文中的"跨层""RC3"等旧术语已统一。当前耦合验证范围为研究内容一（数据组织策略）× 研究内容二（提交控制策略）× 引擎级参数（Daft `into_batches`/`batch_size`/`max_concurrency`）。写回已降为实验设置。最新定义以根 `AGENTS.md`“项目范围/当前资格顺序”和 `PROJECT_OUTLINE.md` 为准。
...
```
历史 P1c 曾把写回列为独立研究内容；该分类已经取消，写回现为统一工程 baseline。

2026-07-16 时上述前置项尚未完成。之后本地 18 单元联合筛选与候选重复已经完成，结果是
独立拼接和联合候选不可分辨；当前不重跑。若以后恢复系统级联合比较，只能使用文件顶部列出的
现行前置条件，不能沿用本段历史分类。

**为什么这个顺序不能乱**：独立拼接必须来自数据组织与提交控制各自已经验证的静态/候选点；
任一侧没有可复现配置时，就无法把联合候选与“独立最优拼接”作有效比较。写回路径在比较中保持
相同，只作为工程 baseline，不再参与研究内容计数或联合搜索。

---

## 1. 研究问题

**当前主线问题**：把数据组织与提交控制分别调优后，它们的独立拼接是否已经足够，还是必须联合选择配置？写回路径保持不变。
...
- [ ] 所有结果 CSV 保存在 `results/cross_layer/`
