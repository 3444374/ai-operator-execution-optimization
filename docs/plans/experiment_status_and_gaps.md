...
M1旧常驻实验、输入划分及本轮真实筛查已完成，未取得平台候选。M2实验层全局元数据适配与固定C4诊断已完成，生产SemMap接入尚缺。
本轮有限验证已结束，不自动追加模型调用；M3/M4没有新增额度。下方历史矩阵继续只供追溯。
...
`results/scheduling/saor_capacity_development_20260811/README.md`。
...
`results/scheduling/state_aware_work_unit/saor_bounded_ready_gate_20260813/README.md`。
...
`results/scheduling/state_aware_work_unit/saor_matched_ready_selector_rehearsal_20260813/`。
...
- 权威结果分别归档在 `results/image_execution/opening_image_native_fourjob_formal_20260810/`
  与 `results/image_execution/opening_image_project_fourjob_observe_only_formal_20260810/`。
...
当前 correctness 护栏目录为 `results/system_e2e/opening_database_e2e_text_refeed_20260808/`：
...
校准见 `results/system_e2e/opening_bounded_saturation_calibration_20260808/`。
...
实现边界复测见 `results/motivation/gpu/image_clip_preprocess_variants_20260801/`，
operator-E2E 原始数据和七步报告见
`results/motivation/gpu/image_clip_native_baseline_20260801/`。
...
`results/image_execution/image_ai_embed_operator_formal_20260803/`；Daft 60K 容量失败单列，
...
详见 `results/cost_estimation/operator_cost_estimation_20260726/README.md`。
...
- cache-ON batching 消融（07-30，`results/data_organization/prefix_cache_data_org_20260730/`）：上游 batching 顺序中性（within 1.2%，CV ≤0.5%）；
...
- cache-ON prefix-affinity routing 消融，4-ep/1.5B（07-31，`results/data_organization/prefix_cache_routing_4ep_1.5b_20260731/`）：prefix_affinity 相对 least_queued **+5.9%**（46,943 vs 44,317 tok/s，raw 不重叠、CV≤0.9%）、SLO −6.3pp、P95 −3.15s，**跨过 5% 门禁**。
...
结果见 `results/scheduling/accelerated_arrival_flush_20260725/`。旧实验仍需在
...
`results/scheduling/accelerated_arrival_flush_20260725/README.md`。
...
`results/scheduling/adaptive_flush_window_20260725/README.md`。
...
原始数据和七步解释见
`results/scheduling/request_lifecycle_gate_20260725/README.md`。
...
- `results/scheduling/adaptive_flush_randomized_20260726/README.md`
- `results/data_organization/joint_batching_submission_512_20260726/README.md`
- `results/scheduling/shared_vllm_adaptive_admission_20260726/README.md`
- `results/scheduling/adaptive_admission_controller_20260726/README.md`
- `results/scheduling/dual_gpu_shared_vllm_formal_20260729_1135/README.md`
