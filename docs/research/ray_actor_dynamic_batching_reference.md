...
> **全文核验校正（2026-07-23，历史文献笔记见 `docs/research/reading_notes/saber_2025.md`）**：SABER **不推导聚合 K_max**——batch_size 设大，USL 仅作 per-request 准入预测器；K_max = √((1−α)/β) 上界推导是本课题的扩展。USL 单瓶颈 + 平滑退化假设与 vLLM 连续批处理的非连续 KV-cache preempt 退化曲线不匹配（双瓶颈：算力 + KV 内存）；R²=0.99 为 in-sample，迁移到本课题 RTX 5070 + Qwen2.5-1.5B 前需 out-of-sample 残差审计。
