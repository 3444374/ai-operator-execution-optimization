...
历史术语说明：`RC1/RC2/RC4` 与 `P0/P1/P2` 只表示旧研究项或实验优先级；“冻结”表示运行前
选定、运行期间不改变的配置或判定标准；“门禁/晋级”表示候选被采用前必须满足的预设正确性、
资源和性能条件；“闭环”在旧段落中指读取、执行、结果核对和证据记录均已完成。当前架构与读者
表述以 §7 和 Sema-like 架构审计为准。
...
**详细手册**：`docs/research/vllm_continuous_batching_reference.md`
...
**对上游策略的含义**：上游组织策略改变工作项的准备方式、提交顺序或提交时机，但不直接决定推理引擎每次迭代处理哪些 token。组织批次（执行层把哪些工作项组织在一起）、服务请求（模型服务实际接收的一次调用）与模型迭代批次（引擎在一次迭代中实际安排处理的序列和 token）是三个不同对象：`max_num_seqs` 与 `max_num_batched_tokens` 约束的是第三种，上游只能间接影响。组织批次、在途请求数与引擎内部批处理之间的关系，需要根据实际提交行为和服务端观测判断；具体默认值和行为按项目实际部署版本核对，不拿当前官方页面替代历史运行配置。
...
**详细手册**：`docs/research/ray_actor_dynamic_batching_reference.md`
...
涵盖 65 篇论文 + 产业系统，分为四个研究岛。完整清单见 `docs/research/ai_operator_literature_inventory.md`。
...
**Ray 调度思想到策略变量的映射**（来自 `docs/research/gpu_scheduler_data_placement_supplement_20260715.md`）：
...
1. Sema 已把 semantic operator 放入 DuckDB 的 query interface、plan、optimizer 和 executor；LOTUS、
   Cortex AISQL、Palimpzest 和 Abacus 也已覆盖 semantic operator 的质量、成本、调用数和执行计划优化；
...
以下 6 篇 2025-2026 年论文为项目文献搜索发现的新增来源，与 RC1（数据组织）和 RC2（提交控制）直接相关。详细内容见 `docs/research/ray_actor_dynamic_batching_reference.md` §6.7-§6.12。

**从 CONCUR (2025) 提取**（历史文献笔记见 `docs/research/reading_notes/concur_2025.md`）：
...
**从 CoLoRA (2026, ASP-DAC, CCF-C) 提取**（历史文献笔记见 `docs/research/reading_notes/colora_2026.md`）：
...
经过对 Ray Core/Data/Serve 各层机制的详细审查，确认以下边界（详见 `docs/research/ray_actor_dynamic_batching_reference.md` §3.7）：
...
可选 compatibility 与 full-system baseline
  → LOTUS v1.2.4 compatibility profile（不定义默认语义）
  → Sema / LOTUS / Daft / Ray Data native paths（各自拥有 execution）
...
**2026-08-27 新增**：
- `docs/research/精读文献笔记/kalypso_arxiv2026/kalypso_arxiv2026.md` — Kalypso 全文精读；
  作为 arXiv 核心补充登记，直接收窄 semantic query plan 与 LLM serving 接口处的研究空白；
  不进入当前 Top 15、十五篇速览或已定稿开题正文。

**2026-08-01 更新**：
- `docs/research/existing_ai_operator_execution_chains.md` — 将数据库 AI 执行链归纳为
...
**2026-07-31 新增**：
- `docs/research/evaluation_metrics_survey_20260731.md` — 评估指标体系调研：7 簇精读笔记（49 篇）+ 8 个数据库厂商/标准基准 web 调研，按 10 类归目并对照项目指标做 gap 分析。P0 缺口：TTFT 分位、ITL/TBT 分布、prefix cache hit rate（均已核实为 vLLM 已暴露但采集端未落字段，见该文件 §6）。附录 B 含 7 家厂商 AI 算子测试方法 + PolarDB Lakebase 同栈专项。
- `docs/research/daft_db_gpu_bridge_direction_scope_20260731.md` — 方向 reframe scope：保留
  Daft 三痛点、offline-batch foreknowledge 与 workload 讨论；08-01 已撤回“数据搬运
  必然是瓶颈/执行层结构性空白”的预设，新增 staged baseline 前置条件。

**2026-07-21 更新**：
- `docs/research/ray_actor_dynamic_batching_reference.md` — 新增 §1.6-§1.8（Ray Serve 准入控制与队列自适应）、§3.7 大幅扩展（7 种反压机制详述 + ConcurrencyCap 废弃分析）、§6.7-§6.12（6 篇 2025-2026 新论文）
- `docs/research/knowledge_hub.md` — 新增 §5.5（6 篇新论文设计原则提取）、§5.6（Ray 现存机制能力边界）、§8 知识缺口更新

**2026-07-17 新增**：
- `docs/research/knowledge_hub.md` — 本文件，新增 §10
- `docs/research/daft_ray_multimodal_reference.md` — Daft+Ray 多模态技术手册与具身智能连接分析

**2026-07-16 新增**：
- `docs/research/knowledge_hub.md` — 本文件
- `docs/research/vllm_continuous_batching_reference.md` — vLLM 技术手册
- `docs/research/ray_actor_dynamic_batching_reference.md` — Ray 架构模式手册
- `docs/research/inference_pipeline_interaction_literature.md` — 28 篇推理管线文献综述

**已有文献与设计文件**：
- `docs/research/literature_and_evidence_review.md` — Ray/Daft/Lance/Snowflake 综合证据
- `docs/research/existing_ai_operator_execution_chains.md` — 现有 AI 算子执行链路对比
- `docs/research/ai_operator_literature_inventory.md` — 65 篇 CCF-A 文献清单
- `docs/research/gpu_scheduler_data_placement_supplement_20260715.md` — GPU 调度补充调研 + Ray 思想映射
- `docs/research/direction_assessment_20260715.md` — 方向评估 + 三岛模型 + 不能声称的结论
...
**详细手册**：`docs/research/daft_ray_multimodal_reference.md`
