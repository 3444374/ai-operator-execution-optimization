...
| **链接** | arXiv:2506.19677v2 / 本地 PDF：`docs/research/reference/saber_batching_2025.pdf` / ar5iv：https://ar5iv.labs.arxiv.org/html/2506.19677 |
| **阅读日期** | 2026-07-23 |
| **状态** | 精读完成 |
...
3. **复杂度**：SABER >> CONCUR > 两档切换 ≈ EWMA ≈ Clipper AIMD。项目 `code/AGENTS.md`“代码质量”要求先实现最小、可解释的控制规则，再由实验决定是否增加状态和参数；SABER 全套（离线采样脚本 + USL 拟合 + 两层队列 + 双重检查）不适合作为首版实现，只能作为后续方法论参考。
