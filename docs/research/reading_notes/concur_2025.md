...
> **证据纪律**：本文所有数字、§位置、图号均来自本地 PDF `docs/research/reference/concur_2025.pdf`（11 页）的精读。论文为 arXiv 预印本（2026-02-02），尚未经同行评审——所有结论均标注此约束。
...
| **链接** | arXiv:2601.22705 / 本地 PDF：`docs/research/reference/concur_2025.pdf` / HuggingFace Papers: `huggingface.co/papers/2601.22705` |
| **阅读日期** | 2026-07-23 |
| **状态** | 精读完成 |
...
4. **与 Ray ConcurrencyCapBackpressurePolicy 的关键对照**：CONCUR 的 AIMD 结构（proactive 信号 + reactive 信号 + 死区 + 加性增/乘性减）与本课题 seed `docs/research/ray_actor_dynamic_batching_reference.md` §3.7 记录的 Ray 废弃策略 ConcurrencyCapBackpressurePolicy **概念上高度相似**（后者也用 EWMA 队列水平 + deadband + backoff/rampup），但有关键差异——见第四层 §2。
...
- [ ] [[saber_2025]] — **已精读**。前瞻性准入（USL 建模 + SLA 预测），比 CONCUR 的反应式 AIMD 更接近本课题需要的预测式控制。CONCUR 是反应式，SABER 是预测式——两者对照可定位本课题控制器的设计点。
...
> ![图 4 · CONCUR System Overview（agent 级准入控制）](figs/concur_fig4.png)
>
> **图 4 · CONCUR System Overview（agent 级准入控制）**。对应方法拆解的 agent-level admission control：把 GPU-resident KV cache 类比为有限共享带宽，controller 用 AIMD 在 agent 粒度（admit/pause/resume）调节并发，pause 保留执行状态、不打断 in-flight 计算。读图重点：控制信号作用在 agent 而非 request，与 LRU 驱逐的区别。
