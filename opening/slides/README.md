...
数据库成为 AI 任务入口
  ...
  → 数据库与模型服务之间仍缺跨层执行闭环
  → 动机实验分别导出工作单元、状态感知提交、多作业调度和代价估计
  → 两项研究内容独立验证后拼接，并检验联合优化是否必要
```

全稿面向外部评审清理了项目管理术语：正文和交付图中不使用 RC/P0、Project、W65K、
“门禁”“冻结”“晋升”“正式点”“失效边界”等内部表达。数据页统一写清实验现象、系统含义与
设计对应；每张主图下均有解释文字和本页结论。31/31 页已完成渲染、画布溢出、空占位符、
speaker notes 和逐页视觉检查；详见 `opening_defense_20260812_v9_qa.md`。
