...
const LIT = (name) => path.join(ROOT, "opening/literature/top15_reading_notes/figs", name);
...
const slides = [
  ...
  },
  {
    ...
    kind: "sideFigure",
    section: "01",
    title: "研究现状｜数据库 AI 优化算子语义",
    body: "代表工作\n• LOTUS：语义算子与质量约束优化\n• Galois：基于大模型的 SQL 逻辑与物理计划\n• GaussML：数据库内机器学习算子与代价估计\n• Cortex AISQL：生产级 AI SQL 与模型调用优化\n\n已解决：算子语义、计划与调用成本。\n仍缺少：外部数据阶段、实时服务压力和多作业共享状态。",
    caption: "图示说明：Cortex AISQL 把多类 AI 算子接入 SQL 执行，但模型请求到达服务前的 work 成形与状态反馈仍需单独处理。\n下一问题：通用数据执行框架是否已经拥有这段跨层闭环？",
    image: LIT("cortex_fig1.png"),
    notes: ["research/top15_ranked_papers.md", "opening/literature/top15_reading_notes/lotus_pvldb2025.md", "opening/literature/top15_reading_notes/galois_sigmod2025.md"],
  },
  {
    source: 7,
    kind: "sideFigure",
    section: "01",
    title: "研究现状｜数据执行系统提供异构流水线机制",
    body: "代表工作与系统\n• Ray：任务/执行器与分布式资源调度\n• Ray Data Streaming Batch：CPU/GPU 流水线\n• Daft：Arrow/Rust、分区与批次抽象\n• NeuStream：流处理与深度学习服务流水线\n\n已解决：数据分区、批执行、异步与背压。\n仍缺少：统一分阶段工作量、模型状态与数据库结果语义。",
    caption: "图示说明：Ray 通过任务、对象存储和局部性调度组织分布式执行，说明通用框架已经提供异构执行的基础机制。\n本页结论：框架机制不会自动获得数据库任务语义和模型运行状态。\n下一问题：模型服务内部调度能否反向管理数据库作业？",
    image: LIT("ray_fig8.png"),
    notes: ["research/top15_ranked_papers.md", "research/reading_notes/ray_data_streaming_batch_2025.md", "opening/report/opening_report.md#23-分布式数据执行与异构流水线"],
  },
  {
    source: 7,
    kind: "sideFigure",
    section: "01",
    title: "研究现状｜模型服务优化已到达请求",
    body: "代表工作\n• Orca：迭代级调度\n• vLLM：PagedAttention 与连续批处理\n• Sarathi-Serve：分块预填充\n• VTC：资源有效利用的多租户公平\n\n已解决：KV、批处理、预填充/解码与服务内公平。\n仍缺少：数据源积压、作业剩余工作量、局部性与写回语义。",
    caption: "图示说明：vLLM 以服务端请求和 KV block 为基本对象。\n本页结论：本课题不重复服务内部 batching，而是控制请求到达服务前的组织、提交与路由。\n下一问题：这些控制动作怎样获得可比较的代价信号？",
    image: LIT("vllm_fig12.png"),
    notes: ["research/top15_ranked_papers.md", "research/reading_notes/vllm_sosp2023.md", "research/reading_notes/orca_osdi2022.md", "research/reading_notes/vtc_osdi2024.md"],
  },
  {
    source: 5,
    kind: "wideFigure",
    section: "01",
    title: "研究现状｜代价研究开始支撑 AI 配置选择",
    body: "GRACEFUL 估计用户自定义函数的服务代价，COSTREAM 学习跨资源放置成本，Abacus 面向语义算子做多目标计划选择。\n图示说明：数据库 AI、数据执行、模型服务与代价决策各有强工作，但三类关键信息仍分散在不同层。\n本页结论：预测误差不是终点，代价信号还要能支撑组织、容量与路由选择。\n下一问题：谁同时拥有数据库作业、分阶段工作量和模型服务状态？",
    image: FIG("P04_相关工作_跨层执行闭环.png"),
    notes: ["research/top15_ranked_papers.md", "research/reading_notes/graceful_udf_cost_icde2025.md", "research/reading_notes/costream_icde2024.md", "research/reading_notes/abacus_pvldb2026.md"],
  },
  {
    source: 5,
    kind: "wideFigure",
  ...
  },
  {
    ...
    body: "前向链路：数据库/数据帧 → 工作描述与代价 → 数据组织 → 调度 → GPU 执行 → 结果写回。\n图示说明：组织器决定一个工作单元包含什么；调度器决定何时、向哪里、以多少在途工作量提交；请求完成后更新状态与剩余工作量。\n本页结论：两项研究内容可独立消融，但共享工作描述、运行状态快照和代价信号。\n下一步：先定义组织与调度共同消费的数据结构。",
  ...
  },
  {
    ...
    kind: "wideFigure",
    section: "03",
    title: "研究内容一｜工作描述连接数据组织与调度",
    body: "公共字段包括记录/作业标识、数据源/准备/模型/结果工作量、局部性、到达时间/期限、不确定区间与校准签名。\n图示说明：文本映射输入/输出 token、前缀局部性与结果字节；图像映射编码字节、解码/缩放、张量/模型工作量与向量结果字节。\n本页结论：工作描述是研究内容一的输出，也是研究内容二的输入。\n边界：字段先采用可解释的最小集合，只有决策收益证明需要时才增加复杂度。",
  ...
  },
  {
    ...
    kind: "wideFigure",
    section: "03",
    title: "研究内容二｜固定总容量下动态释放工作额度",
    body: "离线按机器、模型、协议和负载校准最小饱和容量边界；在线观测每作业待准备、在途、剩余、已完成工作量，以及运行/等待请求与 KV 占用。\n图示说明：空闲份额可以借用；新作业到达后不抢占已进入服务的请求，只在请求完成、归还额度时回收未来份额。\n回退：状态过期、校准签名不符或额度账本异常时，退回先到先服务或公平队列，并保持总上限不变。\n本页结论：动态控制对象是释放顺序与份额，不是持续猜测新的总并发。",
  ...
  },
  {
    ...
    body: "实验现象：单作业全容量与四分之一容量控制组先分离额度损失；同一总上限下，共享额度相对静态分区提高作业组吞吐与 GPU 计算利用率，但不同作业收益不均，Jain 公平性下降。\n系统含义：提高资源有效利用并不保证每个作业同时改善，吞吐、隔离和公平是并列目标。\n设计对应：空闲份额借用提高资源利用；每作业保底/上限、工作量欠账和服务目标保护约束隔离。",
  ...
  },
  {
    ...
    kind: "wideFigure",
    section: "03",
    title: "共同支撑｜代价估计以配置排序和决策损失评价",
    body: "实验现象：429 个正式观测覆盖 20 组负载与服务配置，每组比较四个在途工作量候选；混合模型排序准确率 0.808，平均决策损失 2.90%，最坏损失 14.72%。\n系统含义：单点执行时间误差更低，不保证能选对组织、容量或路由配置；当前结果只说明初步可行。\n设计对应：采用解析工作量特征、少量剖析校准和残差修正，同时报告相对排序准确率与平均、中位、最坏决策损失。",
  ...
  },
  {
    ...
    body: "实验现象：Daft 内置 AI 函数、Ray Data 与本课题执行路径均出现作业级干扰，但短作业和长作业的退化形态不同。\n系统含义：图像同样需要每作业分阶段工作量、活跃/剩余状态、共享额度与隔离约束；本课题现有状态数据仅用于观测，尚未驱动控制动作。\n设计对应：工作描述、状态快照与额度接口跨模态复用，只有数据源、准备和模型阶段的字段映射不同。\n比较边界：每条路径只与自身单作业隔离运行比较，不做跨框架绝对排名。",
  ...
  },
  {
    ...
    kind: "twoProblem",
    section: "03",
    title: "验证方案｜策略独立搜索后拼接，再检验联合收益",
    intro: "研究内容一与二需要各自可归因；共同接口不意味着所有参数必须一次联合搜索。",
    leftLabel: "分层路线",
    leftTitle: "独立搜索 → 选定 → 拼接",
    left1: "先固定调度，搜索工作量预算、均衡与局部性的最小有效组合。",
    left2: "再保持组织方法不变，比较先到先服务、静态分区、公平队列、状态感知准入、路由与共享份额。",
    rightLabel: "交互检验",
    rightTitle: "拼接方案 ↔ 联合参数搜索",
    right1: "联合显著优于拼接，说明组织状态与调度动作需要共同优化。",
    right2: "两者接近则采用分层方案；相对固定配置没有达到预设的最小改善幅度时，说明不适用场景并停止增加参数组合。",
  ...
  },
  {
    ...
    body: "已完成文本 vLLM、Daft、Ray 执行链路，工作描述、共享额度、完成即归还和可追踪运行记录；图像 CLIP 流水线与同资源原生基线也已闭环。\n图示说明：已有结果承担动机、机制与可行性证据，不提前替代后续同总上限动态对照。\n本页结论：工程链路可运行，关键缺口是状态感知有序释放、未参与校准场景的代价验证与跨模态动态增量。\n下一步：按统一实验合同逐层把机制验证为可归因结论。",
  ...
  },
  {
    ...
    kind: "twoProblem",
    section: "04",
    title: "验证标准｜保持资源与最大在途工作量一致",
    intro: "每个设计先检查结果正确、模型服务供给充足且重复实验稳定，再解释策略效果；不满足这些条件的实验不用于支持结论。",
    leftLabel: "研究内容一",
    leftTitle: "只改变数据组织",
    left1: "固定行数/图片数 → 标量工作量预算 → 分阶段工作量 → 均衡/局部性。",
    left2: "固定资源、模型、调度与最大工作量；报告正确吞吐、作业完成时间、请求尾延迟、局部性与阶段计时。",
    rightLabel: "研究内容二",
    rightTitle: "只改变释放与份额",
    right1: "全局先到先服务 → 静态分区 → 公平队列 → 状态感知准入、路由与共享。",
    right2: "固定总并发和在途工作量；报告有效吞吐、模型计算利用率与能耗、隔离、公平性和状态—动作—结果轨迹。",
    notes: ["opening/opening_defense_outline_20260808.md#第-19-页主实验用同上限强静态-ab-逐层验证", "AGENTS.md#7-5-实验执行与结果记录流程"],
  },
  {
    source: 10,
    kind: "twoProblem",
    section: "04",
    title: "研究计划｜优先补动态证据并保留负结果路径",
    intro: "开题后不继续横向增加数据库产品或追求更多正结果，重点在相同资源与最大在途工作量下比较方法，并验证能否推广到新场景。",
    leftLabel: "阶段计划",
    leftTitle: "逐层完成论文证据",
    left1: "2026.08–10：选定数据组织策略；在相同资源和最大在途工作量下，比较先到先服务、公平队列与状态感知有序释放。",
    left2: "2026.11–2027.03：未参与校准场景的代价验证、图像动态与联合检验；2027.04–06：整理、复现、论文。",
    rightLabel: "风险控制",
    rightTitle: "简单策略与阴性结论优先",
    right1: "若先到先服务或公平队列已达到同一效率—公平前沿，则采用简单策略，不增加控制器复杂度。",
    right2: "若图像持续由 CPU 数据准备主导，则把结论限定为异构流水线组织；动态方法没有达到预设改善幅度时，报告容量测量结果和不适用场景。",
    notes: ["opening/report/opening_report.md#62-后续进度安排", "opening/claim_matrix.md"],
  },
  {
    source: 10,
    kind: "twoProblem",
    section: "04",
    title: "预期贡献｜两项方法、共同支撑与跨模态验证",
    intro: "目标不是让一次模型调用更快，而是让数据库 AI 任务形成可比较的工作量，并稳定、公平地使用固定 GPU 容量。",
    leftLabel: "预期创新",
    leftTitle: "组织与调度方法",
    left1: "统一 token/图像帧、分阶段工作量、局部性、期限和不确定性的工作描述。",
    left2: "在预先标定容量内，依据新鲜运行状态控制提交、路由与多作业份额，并约束服务目标、隔离和公平。",
    rightLabel: "共同支撑与产出",
    rightTitle: "决策评价与可复现验证",
    right1: "轻量代价估计以配置排序和决策损失验收；文本与图像复用同一策略接口。",
    right2: "交付可复现实验系统、原始运行轨迹、结果报告和论文；明确适用条件与不适用场景。",
    notes: ["opening/report/opening_report.md#7-预期成果创新点与风险控制"],
  },
  {
    source: 1,
    kind: "cover",
    texts: ["华中科技大学", "谢谢，请各位老师批评指正", "数据库 AI 负载的执行优化与调度研究"],
  ...
  return {
    outputSlides: slides.map((spec, index) => ({
      ...
      editTargets: recordsFor(sourceRecords, spec.source, new Set(["textbox", "shape", "image"])).map((record) => ({
        shapeId: record.id,
        action: record.placeholder ? "delete" : record.kind === "textbox" ? "rewrite" : record.kind === "image" && spec.image ? "replace" : "keep",
        reason: record.placeholder ? "remove inherited empty page-number placeholder" : `output slide ${index + 1}: template-preserving edit`,
      })),
    })),
...
const audienceFigureReplacements = [
  ...
  ["产品 / database-E2E 轨：仅 SQuAD 可排名", "数据库端到端轨：仅 SQuAD 可在轨内比较"],
  ...
  ["数据库端到端轨：仅 SQuAD 可在轨内比较", "数据库端到端设置：SQuAD 只在相同任务与计时口径内比较"],
...
function notesText(index, spec) {
  ...
  return [
    `汇报讲稿：本页围绕“${title}”推进。先说明页面中的事实或代表工作，再解释它如何提出下一页要回答的问题。`,
    "",
    "答辩备注：实验数字只在各自比较合同内解读；已实现但未完成正式对照的机制写为研究计划，不写成已证明收益。",
    "",
    "[Sources]",
...
function applyWideFigure(deck, slideNumber, records, spec) {
  ...
  setText(bodyShape, spec.body, { fontSize: 14.5, color: "#2E2E2E" });
  setFrame(bodyShape, 64, 575, 1152, 102);
}

function applySideFigure(deck, slideNumber, records, spec) {
  const texts = sourceTextRecords(records, slideNumber);
  const section = texts.find((record) => (record.bbox?.[0] ?? 0) < 100 && (record.bbox?.[1] ?? 0) < 180);
  const title = texts.find((record) => (record.bbox?.[0] ?? 0) > 100 && (record.bbox?.[1] ?? 0) < 180);
  const body = texts.find((record) => (record.bbox?.[0] ?? 0) < 100 && (record.bbox?.[1] ?? 0) > 180 && (record.bbox?.[1] ?? 0) < 550);
  const caption = texts.find((record) => record.id !== section?.id && record.id !== title?.id && record.id !== body?.id);
  setText(deck.resolve(section.id), spec.section);
  setText(deck.resolve(title.id), spec.title);
  const bodyShape = deck.resolve(body.id);
  setText(bodyShape, spec.body, { fontSize: 16, color: "#2E2E2E" });
  setFrame(bodyShape, 28, 195, 485, 450);
  const captionShape = deck.resolve(caption.id);
  setText(captionShape, spec.caption, { fontSize: 14.5, color: "#2E2E2E" });
  setFrame(captionShape, 535, 555, 700, 105);
}
...
function deletePlaceholders(deck, slideNumber, records) {
  for (const record of recordsFor(records, slideNumber, new Set(["shape"])).filter((record) => record.placeholder)) {
...
async function main() {
  ...
  await fs.writeFile(path.join(TMP, "template-audit.txt"), [
    ...
    "Reused source frames: cover 1, agenda 2, wide figure 5, side figure 7, two-problem comparison 10.",
  ...
  await fs.writeFile(path.join(TMP, "deviation-log.txt"), [
    "The output narrative expands to 31 slides; page count follows communication density rather than the source count.",
    "Inherited figure and text frames are repositioned inside the source safe area to add figure captions and takeaways.",
    "The four related-work layers are split across four pages to avoid a literature inventory.",
  ...
  for (let index = 0; index < slides.length; index += 1) {
    ...
    else if (spec.kind === "sideFigure") applySideFigure(deck, slideNumber, deckRecords, spec);
    ...
    if (spec.image) {
      const position = spec.kind === "sideFigure"
        ? { left: 535, top: 190, width: 700, height: 350 }
        : { left: 64, top: 190, width: 1152, height: 370 };
