...
> ![图 1 · The Clipper Architecture（分层 serving 中间件）](figs/clipper_fig1.png)
>
> **图 1 · The Clipper Architecture（分层 serving 中间件）**。对应核心论点的两层架构：上层 Model Selection Layer（bandit 选模型 + straggler 缓解），下层 Model Abstraction Layer（把任意框架封装为统一 batch prediction 接口的 Docker 容器 + per-model AIMD 自适应 batching）。读图重点：AIMD 作用在 Abstraction Layer 的每个 container，与 Selection Layer 解耦。
