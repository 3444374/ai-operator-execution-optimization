...
> ![图 1 · Logical dataflow graphs（异构 ML 管线）](figs/raydata_fig1.png)
>
> **图 1 · Logical dataflow graphs（异构 ML 管线）**。对应核心论点'partition 作为弹性调度最小单元'：dataflow graph 每个节点是一个 operator，partition 在运行时动态创建并流式传输，把 batch 的吞吐优势与 streaming 的弹性统一起来。读图重点：异构集群上 operator 如何映射、partition 如何流动。
