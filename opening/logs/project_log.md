...
- 将 `opening/README.md` 底部 2026-07-15 飞书同步说明明确标为历史记录，避免旧链接和旧“三层”
  表述被误读为当前发布状态；当前报告、PPT 与飞书入口不变，未修改任何对外正文或图件。
- 更新 `opening/practice_plan.md`：专业实践计划统一为两项研究内容、共同代价估计和多模态验证，
  写回改为统一工程 baseline；阶段时间和学校表格结构不变。
- 精读库登记 Kalypso 后，将 `opening/README.md` 与 `literature/reading_list.md` 的物理计数校准为
...
- Galois 精读主笔记新增 Figure 1–4 和 Figure 7–11 共 9 个论文原图裁剪件；选图、页码、SHA256 与视觉检查保存在 `figures/audit/galois_deep_reading_figures_audit_20260824.md`。
- `opening/README.md` 和 `opening/literature/reading_list.md` 的现行精读统计同步为 9 篇主笔记、74 张论文原图裁剪件。本次只同步导航与文献清单，未修改开题报告正文、PPT、飞书或 Top 15 历史快照。

## 2026-08-24 开题报告按评审意见重构

- 依据用户提供的逐章评审意见重写 `opening/report/opening_report.md`，保留学校模板七部分，但把研究结构固定为两项：分阶段工作描述与数据组织、固定准入范围内的请求释放/服务实例路由/多 Job 协调。算子代价估计改为共同使能组件，图像改为跨模态验证。
...
- 新增 `figures/architecture/opening_target_architecture_status.{png,svg}`，分开显示目标 PostgreSQL planner-visible 算子路径和当前外部可运行证据链；报告图片由 11 张压缩为 6 张。生成脚本、报告副本和图件审计已同步。
- 新增 `opening/report/opening_report_20260824_qa.md`，完成结构、术语、主张、图片、参考文献和外部事实复核。本轮未修改实验数据、Word、PPT、飞书和 Wiki；这些发布面后续恢复时必须以本地报告为源重新生成。
...
- Palimpzest 精读目录和主笔记名已按系统正确拼写更正为 `palimpzest_cidr2025`，并新增附件 arXiv v2 Figure 1–7 共 7 个论文原图裁剪件；选择、版本边界、页码、SHA256 与视觉检查保存在 `figures/audit/palimpzest_deep_reading_figures_audit_20260824.md`。
- `opening/README.md` 和 `opening/literature/reading_list.md` 的现行精读统计同步为 10 篇主笔记、81 张论文原图裁剪件。本次只同步导航与文献清单，未修改开题报告正文、PPT、Word、飞书或 Top 15 历史快照。
...
- Abacus 精读目录和主笔记按精读合同统一为 `abacus_pvldb2026`，正式 PVLDB 版 Figure 1–8 的 8 个既有裁剪件已从错误的 `assets/` 引用接入对应正文；来源页码、SHA256、视觉检查和证据边界保存在 `figures/audit/abacus_deep_reading_figures_audit_20260824.md`。
- `opening/README.md` 和 `opening/literature/reading_list.md` 的现行精读统计同步为 11 篇主笔记、89 张论文原图裁剪件。本次只同步导航与文献清单，未修改开题报告正文、PPT、Word、飞书或 Top 15 历史快照。

## 2026-08-24 Sema 精读配图状态同步

- Sema 精读主笔记已规范为 `sema_vldb2026/sema_vldb2026.md`，本地 arXiv v1 正文 Figure 1–8 的 8 个原图裁剪件已接入对应章节；来源版本、页码、SHA256、视觉检查和证据边界保存在 `figures/audit/sema_deep_reading_figures_audit_20260824.md`。
- `opening/README.md` 和 `opening/literature/reading_list.md` 的现行精读统计同步为 12 篇主笔记、97 张论文原图裁剪件，并按当前工作区实体修正参考 PDF 状态。本次只同步导航与文献清单，未修改开题报告正文、PPT、Word、飞书或 Top 15 历史快照。
