...
| 固定总并发上限下的状态感知有序释放优于简单调度 | 未证明（formal 有效、未晋级） | 双卡两作业同上限 formal 的 resolution-aware v2 validation passed；SAOR 在共享额度臂中前台最好，但静态分区隔离更强。strict-priority 两轮短测达到 11,791 tok/s、fg P99 14.27s/SLO 0%，说明 release-only 上界可达 | 只能写成“soft fairness release 未越过静态隔离；hard release ordering 有方向性潜力”；下一步做有界 priority-window/service-lag guard，不继续扫软权重 | hard priority 可能饿死 bulk，短测不是 formal；若有界版本不能在静态前台非劣、吞吐+5%且 bulk lag/SLO 不越界，则淘汰动态候选 |
