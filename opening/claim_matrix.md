...
| 四 Job 下 quota、真实竞争和共享调度可以被分离；效率、完成体验与服务份额需分别评价 | 已证明（受控文本） | `experiments/results/opening_fourjob_interference_20260809/README.md`：full→quarter short JCT +180.38%，quarter→static 真实竞争再 +60.40%；按三次 formal 均值，shared 相对 static 使 short/long1/2/3 JCT −72.23%/−8.28%/−20.24%/−52.66%、group tok/s +8.68%，即在效率/JCT 子向量上构成 baseline-relative empirical Pareto 改善；raw-work Jain 0.960→0.923 表示收益更不均。shared/quarter-solo 为 0.45/1.29/1.14/0.68，long1/2 未达到经验性保留份额非劣。三条原生路径全部 Job 相对自身 single 退化 | 动态调度需同时实现 idle borrowing、per-job floor/cap、work-fair deficit 与 SLO guard；评价固定 full-solo、reserved-solo、static-multi 三种反事实，并在新 formal 记录共同积压 service lag/starvation | 只有一个 workload/offset、equal weight；历史 compact CSV 不能还原 event-level GPS lag/最长连续无服务。Jain 不是正式保证，效率/JCT 子向量的经验性 Pareto 也不是完整多目标或 DRF Pareto efficiency；不能称 sharing incentive、Themis/VTC 性质、最终算法或跨框架胜出 |
