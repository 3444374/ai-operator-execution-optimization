...
| 固定总并发上限下的状态感知有序释放优于简单调度 | 未证明（旧 formal 未晋级；新 bounded-ready 仅 development candidate） | 旧双卡两作业 formal 中 SAOR 未越过 project frozen-static；strict-priority 证明 release-only 可达。bounded-ready $0.125W_e$ 两轮约 12.36K tok/s、fg P99 17.58–18.15s/SLO 0%、bulk miss 65.8%–66.6% 通过开发门，$0.25W_e$ 被 bulk guard 拒绝 | 只能写成“有限 concrete-ready exposure + guarded release 值得继续验证”；ready-window 与 selector 同时变化，先做 project bounded-ready + FIFO/DRR/VTC-style/strict-priority 内部归因门，通过后才 formal；原生 baseline 继续使用自身调度 | development 不是 winner；bounded-ready 是项目机制，不能注入原生 baseline；bulk 30s 只作相对 guard；若项目简单消融已在同一 Pareto 前沿，收敛为 observation contract 或淘汰复杂 selector，不换 workload 追正 |
