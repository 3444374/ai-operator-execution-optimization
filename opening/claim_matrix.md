...
| 固定总并发上限下的状态感知有序释放优于简单调度 | 未证明；已定位为效率—隔离—tail 非支配权衡 | matched-ready 下 DRR/VTC-style≈12.90K tok/s、fg SLO 0；SAOR 12.28K、fg P99 17.85s，换取更低 tail 但吞吐/bulk JCT 较差。bridge 中 shared capacity +25.96% tok/s 却使 fg P99 +99.17%；bounded-ready 在同 FIFO 下 +7.30% tok/s、fg P99 −33.62%，仍约 40% fg violation | 可写成“固定 K/W 下，Job 份额借用、ready exposure 与 release order 分别影响效率/隔离/SLO”；FIFO/DRR/VTC 是已有算法的 Project 实现，不是原生系统。下一步只做 Daft/Ray native-system matched comparison，不授权 selector formal | development 不是 winner；bounded-ready 不能注入原生 baseline；不称 dynamic-K 必要；若简单 selector 已在同 Pareto 前沿，则 SAOR 定位为可配置折中或淘汰复杂 selector |
