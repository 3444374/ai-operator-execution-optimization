...
| 固定总并发上限下的状态感知有序释放优于简单调度 | 未证明（方向性证据） | 双卡两作业同上限对照已完成：有序释放在共享额度臂中前台尾延迟最好，但静态分区以更低吞吐获得显著更强的前台隔离；原始完整机制 validation 保留 fail-closed，采样分辨率 compact 回放不改变性能排序 | 只能写成“有序释放改善无保护共享，但未越过静态隔离点”；下一步只做非抢占前台优先上界与有限保护余量，不继续扫权重 | 当前时限权重为零、阶段状态未驱动动作；若有限保护余量不能在静态前台非劣时保住至少 5% 吞吐增量，则淘汰动态候选 |
