...
```text
总动机一：记录数不能代表分阶段 AI 工作量
  ├─ 文本工作量层：同行数的 token work 可差 14.3×
  ├─ 图像阶段层：相同张数仍有 prepare / transfer / model 差异
  └─ 对应研究内容一：分阶段工作描述与数据组织

总动机二：单一静态量或局部指标不能代表当前可调度状态
  ├─ 配置层：容量上限不等于实际在途工作
  ├─ 服务层：吞吐、排队、KV、利用率和尾延迟需联合解释
  ├─ Job 层：全局服务状态不能代表各 Job 的剩余工作与已获服务
  └─ 对应研究内容二：固定准入范围内的提交、路由与多 Job 调度

共同使能：代价估计帮助比较配置和计划，不增加第三条总动机
跨模态验证：图像检验两项抽象能否复用，不增加第三项研究内容
```

这套层级不降低任何证据的严谨性。每个子实验仍要说明验证哪个子问题、排除了哪一种简单解释、能支持到哪里以及尚未证明什么。动态调度属于对动机二的动作设计，不再与状态问题并列成另一条动机；代价估计与图像实验分别承担共同使能和跨模态验证角色。
...
6. 第 7 页结尾：文本和图像共同说明记录数不能代表分阶段 AI 工作量，由此形成第一项研究内容。
7. 第 8 页结尾：配置边界和模型服务指标仍不足以描述多 Job 场景，下一页进入 Job 层。
8. 第 9 页结尾：动机二的三个状态层次已经完整，下一页把两条总动机收敛为两项研究内容。
...
#### 第 7 页：动机一，记录数不能代表分阶段 AI 工作量

- **本页回答**：为什么行数或图片张数不足以定义可比较的 AI 工作。
- **文本工作量层**：固定 16 行时 batch token 最小/最大为 474/6,793，相差 14.3×。
- **图像阶段层**：准备、传输和模型计算的相对成本随批大小与输入表示变化，固定张数也不能
  描述各阶段压力。
- **页面结论**：动机一导出分阶段工作描述与数据组织，不同时导出调度方案。
- **转场**：有了工作描述以后，还需要判断运行时什么时候、向哪里释放这些工作。

#### 第 8 页：动机二，单一指标不能代表当前可调度状态

- **本页回答**：为什么运行状态不能被容量上限或某一个服务指标替代。
- **配置层**：同为每实例 65,536 预计词元上限，输入到达较慢时实际峰值在途工作只有约 29%。
- **服务层**：约 65K 已达到最高已测吞吐的 97.8%；继续增加上限时吞吐只小幅增加，P99 继续上升。
- **读图要求**：配置上限、实际在途工作、吞吐、排队、缓存、利用率和尾延迟分别回答不同问题，
  不能选择其中一个作为完整状态。
- **页面结论**：动机二首先需要配置层与模型服务层的联合状态描述。
- **转场**：即使全局服务状态描述完整，多个 Job 的进度与服务结果仍可能被总体指标掩盖。
...
- **现象**：Daft Native、Daft Ray、Ray Data 中 Short 均受到影响，三个 Long 也全部退化，
  但退化幅度和服务压力形态不同。
- **Job 层含义**：只看全局吞吐或 Short 不足以描述系统；还需每个 Job 的已完成工作、
  剩余工作、已获服务、完成时间和最长未获服务时间。
- **页面结论**：动机二由配置层、模型服务层和 Job 层共同构成，多 Job 不是第三条平级动机。
- **转场**：至此，两条总动机都已有分层证据，可以收敛为两项研究内容。

#### 第 10 页：两条总动机对应两项研究内容

- **本页回答**：前面多组实验如何收敛，而不是继续扩张研究任务。
- **动机一 → 研究内容一**：记录数不能表示分阶段工作量 → 工作描述与数据组织。
- **动机二 → 研究内容二**：单一指标不能表示当前可调度状态 → 固定准入范围内的提交、
  服务实例路由与多 Job 调度。
- **动机二内部关系**：状态观测提供配置层、服务层和 Job 层信息，动态调度使用这些信息采取动作；
  两者是“观察与动作”的上下游关系，不是两条平级动机。
- **辅助角色**：代价估计帮助比较配置和计划，图像检验接口复用，均不新增研究内容。
