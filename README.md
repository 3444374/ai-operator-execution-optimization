...
该题目不是单独的汇报包装，而是后续项目规划的当前方向锚点。后续实验和文档应围绕三类研究内容展开：AI workload 感知的数据组织与批处理构造、GPU 推理服务状态感知的 Ray 并行调度与反压控制、面向 AI 数据流的结果汇聚与 Lance / 数据库持久化协同。
...
│   └── pgai/               # pgai SQL 算子触发面隔离预演；不是 PG18.4 结果
├── learning/               # 面向初学者的实验讲解和术语学习材料
│   ├── AGENTS.md
│   ├── README.md
│   └── experiment_walkthrough.md
├── figures/                # 项目级图资产库，供 learning、开题、中期和论文复用
│   ├── AGENTS.md
│   ├── README.md
│   ├── architecture/
│   ├── data/
...
2026-07-14 已补充 PG18.4 本地 GPU-backed Ray actor 链路中的
pgvector `vector(384)` 写回对比：
...
该结果比较 no writeback、JSON text 和 pgvector(384) 三种 sink，仍属于
PG18.4 本地预演事实，不代表 PostgreSQL 18.3 内部平台性能。
...
2026-07-13 新增 `deploy/pgai/` 隔离预演环境，用于验证 pgai / 等价
PostgreSQL AI SQL surface，例如 `CREATE EXTENSION ai` 和
`ai.ollama_embed(...)`。该环境独立于现有 PostgreSQL 18.4 容器，当前基于
pgai 官方 Docker 示例路线的 PG17 镜像，只用于跑通真实 SQL 触发面的功能验证；
不能写成 PostgreSQL 18.4、PostgreSQL 18.3 或性能结果。

2026-07-14 已启动 `deploy/pgai/` 环境并完成 SQL embedding 冒烟验证：
PostgreSQL 17.10 中 `ai 0.11.2`、`vector 0.8.4` 可用，Ollama
`all-minilm:latest` 可由 SQL `ai.ollama_embed(...)` 调用，3 条 smoke 文本均写入
384 维 pgvector。完整记录见 `feasibility/results/pgai_sql_smoke_20260714.md`。
该结果只证明真实 SQL 触发面可用，不是 PG18.4/PG18.3 或性能结论。

同日已补充三步触发面验证：PG18.4 job-table 256 行健康检查、pgai SQL
1024 行 profile，以及 job-table vs pgai_sql 的小规模可运行性对照。记录见
`feasibility/results/trigger_surface_validation_20260714.md`。该对照不能用来声称
pgai SQL 更快或更慢，因为 PostgreSQL 版本、写回格式和执行路径不同。
...
7. 后续把稳定工程代码迁移到 `code/`。
8. 先在 `deploy/pgai/` 跑通 pgai SQL embedding 冒烟验证，再决定是否把
   `postgres_ai_operator_profile.py` 扩展为 `job_table|pgai_sql` 两种触发面。
...
## 2026-07-14 最新 GPU-backed 关键复测

pgai SQL trigger surface 集成并验证后，已重新跑一组本地 GPU-backed
AI_EMBED 关键复测：
...
这组结果使用 PostgreSQL 18.4 本地预演环境、`local_embedding_server.py`
CUDA endpoint、384 维 embedding 和 JSON text 写回，覆盖 batch 粒度、全链路
写回时间、单双 endpoint 和数据规模。它可以作为当前 GPU-backed 动机画像的最新
本地结果，但不能写成 PostgreSQL 18.3 内部平台结果，也不能写成 pgai SQL 性能结论。
