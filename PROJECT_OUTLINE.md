...
提交 `47407751` 已在第二 path 之前独立修正 reference 的 semantic-input rows、NULL rate、
output selectivity、calls、prompt/output usage、model role 和 AI-work cost，并将估计与
provider 返回的实际 usage 分列显示。该工程启发式还没有校准为性能模型。
下一步实现 LOTUS/Cortex-like 第二 path。随后用
...
编译完成的 sealed tasks。
当前状态是 `exact-semfilter-reference-cost-cardinality-validated`：受限 recording `SemMap`/`SemFilter` 与三参
...
reference `CustomPath` 另有不进入 semantic digest 的 planner cost metadata，可说明输入/输出行数、
NULL-adjusted calls、estimated prompt/output work 和实际 usage。第二 physical path、载体反例审查、accepted-prefix 和
...
项目达到数据库优化资格还需要同一逻辑语义下可由 PostgreSQL 区分和选择的第二 physical path。
其前置的 reference rows/selectivity/AI-work cost 显式合同已完成，但还不是经校准的质量或性能模型。
...
5. **下一条 semantic path**：SemFilter input rows、selectivity、calls/tokens/model-role 与
   AI-work cost 已独立完成功能资格；下一步以确定性 golden/calibration evidence 建立一条可辨认的
