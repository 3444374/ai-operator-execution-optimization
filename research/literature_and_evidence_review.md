...
> 基于 Ray/Daft/Lance 类外部执行链路的数据库 AI 算子批处理与 GPU-backed 模型服务链路调优。

当前最强主动机不应来自单独的 object/fan-in microbenchmark，而应来自生产式 GPU-backed E2E profile：数据库/SQL 触发 AI 算子，外部执行链路调用 GPU-backed 模型服务，再写回数据库，并显示外部链路、模型服务队列、fan-in 或 writeback 的损耗足够明显。Ray object、Daft shuffle、Arrow/Lance 等资料用于解释这些损耗为什么可能出现，以及后续应该调哪些变量。
...
2. 系统：Daft + Ray + Lance；
3. 问题：生产式 GPU-backed 数据库 AI 算子链路中，固定 partition / batch / actor / routing / writeback 策略在不同 workload 下失效；
...
6. 不能说“跨层调度一定是最终贡献”；
   - 当前只确认了 Ray Data/Core/Serve 有相关接口；还没有 GPU-backed E2E 主动机画像、真实 actor pool、模型服务队列、资源配比或 backpressure 实验。
