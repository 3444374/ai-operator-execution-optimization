...
- **320-run 完成 + 归档 + CE LOO 评估（commits `b3edd01` 数据 / LOO 结果待提交）**：**320/320 有效、0 incident、§4 全 11 门禁过**（含 gate 10 = 0 local-Ray、cache hit∈[0,0.988]、exactly-once、两 endpoint）。CE 信号：20/20 context 0 退化（e2e spread 12–86%），最优 active-work context-dependent（98304 胜 11/20）。**CE0–CE6 context-LOO（plan §5）已跑**（harness `compare_cost_estimators_contextloo.py`，用 wrapper 把 `load_rows` 数据源指向本 runs.csv；240 行/20 context/4 candidate）：**CE3_ridge 与 CE5_hybrid 决策 regret 3.70%（过 5% 门槛，pick_rate 0.55、rank 1.7），但行级 pairwise <0.75 → 无估计器过完整 promotion contract**；CE0/CE2 退化（全 tie，49.7% regret）、CE1 解析 12.1%、CE4 LightGBM skipped（依赖未装）。CE3≈CE5（残差校正未在 Ridge 之上再增益）。**结论**：候选"选对最优"已近 oracle（3.7%），但"对所有候选对稳定排序"还差——按 §6 不接管计划选择。下一步：补 CE4（装 lightgbm）+ 诊断 pairwise + harness 改 `--data-csv` 参数化（去硬编码 REF_JSON）。raw 66MB 留服务器，SHA256 `a4f9cd52...4e8f`。
