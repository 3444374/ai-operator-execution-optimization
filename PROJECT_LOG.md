...
## 2026-09-21：文档内容级整理（分支 codex/repo-reorg，不合 main）

- 用户给出内容整理指引：修正漂移的事实、提炼被过程记录淹没的知识、改变以后产生文档的方式；
  本轮只整理文档与已有验证记录，没有重新执行测试。
- `code/INFRA_STATUS.md` 从按日期追加改为按能力组织：每项回答“现在有什么、验证到哪里、仍缺什么”，
  逐轮请求数/分支/清理细节留在 `results/` 记录，本文只链接；主架构文档的长期能力表同步修正状态漂移。
- `PROJECT_OUTLINE.md` 压缩为方向、研究内容、证据等级与当前优先级；逐轮验证流水账移出，
  开题 E2E 段只保留提炼后的认识并链接记录。
- `docs/research/knowledge_hub.md` 修正“上游 batch 超过 max_num_batched_tokens”的混用表述，
  区分组织批次、服务请求与模型迭代批次；`CONTEXT.md` 增加五组常用区分（行/工作项/请求/Job、
  三种批次、请求上限/在途/排队、资源计费/进程内存/设备内存、取消/确认终止/资源释放）。
- `docs/plans/data_organization_batching.md` 去过程化：17 份已执行的运行前声明/实施规格按原文迁入
  对应 `results/` 记录的 `plan.md`，计划正文只留当前问题、已知情况、下一步判断、完成条件与指针。
- 五份关键结果 README 改为结论先行（等待位置常驻、查询共享 E、图像 F、M1 完整复查、M1/M2/F 真实验证），
  开头写清“验证了什么、支持什么判断、还有什么没验证”，原始数据与失败记录保留。
- `code/README.md` 开头删除重复状态、指向能力状态入口；根 `AGENTS.md` §7 同步表改为按变化类型给出
  默认更新与不需要机械更新的内容，并写入“新信息更新所属主题、整理时删除被替代的现行说明”两条长期要求。
- 链接检查 0 断链；新增/改写文字过 §6 禁词表。
...
- 正确性16次与32个512行查询全部完成；账本、服务访问和success counter均16,400，无HTTP错误或未确认项；原故障根因仍待确定。
...
- 用户要求继续处理两条未确认请求。原错误统一为MODEL_UNAVAILABLE，缺具体类型；传输层新增有限异常类型/原因链/代码位置事件，不记录消息和正文，仍抛出原异常，无重试。
- 诊断事件缺失的受控检查先失败后通过；最终11项聚焦检查通过，Linux provider77项中68通过、9项环境跳过。测试调用与Mock构造错误原输出亦保留。
...
- 用户授权继续M1测试；先用固定25毫秒HTTP fixture复现PG/direct差距，128行约22–26秒对1.2–1.4秒，64行也可重现，32行不触发诊断信号。
- profile定位逐候选重复汇总全任务表。三处只读扫描改为同次复用派生用量；任务表仍为唯一责任来源，提交/完成/取消/释放后重新读取。
- owner回归先出现745次汇总，补入delivery后中间修复仍有379次；最终≤12且释放即允许下一FIFO成员。128行同fixture恢复到约2.1–2.25秒。
- Linux调度386、图像81通过，provider76项中9项环境跳过；另2项实际PG/HTTP集成通过。本地pyarrow缺失保留，Linux补齐，不安装新依赖。
- 按52,000次/60分钟新清单启动真实模型，512行调参、1024行评价、C4/16/64/128预先固定。906份运行文件和模型身份核对通过。
...
- 按用户新授权执行预先声明的小规模真实清单，文本最多17,000次/60分钟，图像最多160次/20分钟；依次单卡运行，没有下载或修改服务参数。
- 文本实际8,248次，客户端账本、服务日志与counter一致；M1筛查24查询未取得平台候选，W调参、独立评价及观测开销对照未运行。
- 新增M2实验层PG源信息适配，五组共享有界DirectMap及结果目标；首次全局扫描/分词/排序/再次读取付费，复用组显式追溯生产者。
- 离线审计发现M2换批短暂保留两窗口，原69行声明更正为保守133行；本次payload保守42,823字节低于16MiB。释放修复经对象存活回归验证，运行SHA和原结果保留，没有真实重跑。
- M2完成五组各1+3轮，固定C4仅作功能参照，未支持稳定收益；评价误判35–37/256和逐值变化保留，尚未接入SemMap全局预扫。
...
- 按用户附件修改研究问题、比较对象与选点依据，旧 M1 作为已完成测量反例保留；44,544 次固定运行表退出当前方案。
- 修正 C64/32 MiB 不可达点的实验装配：运行前分别计算 Core/PG/输入的支持项数，各扫描点统一资源。
- v2 实验编排分容量筛查、W 调参、独立评价和代表性日志对照，复用常驻 gateway、PG-source direct 与持久账本。
...
- 保留已选数据、脚本、受控检查与原始证据，测试资源已清理；恢复实际调用需用户新的明确指示。
...
- 根规则保留研究范围、验证前提、持续执行、禁词与隐私要求；精简读取说明、环境命令重复和文档维护步骤。子目录保留自身要求，目录清单、命令与历史入口引用已有 README。
- 运行行为变化才触发相应环境检查；新图或论证变化才重写设计说明。可恢复的本次临时导出可随任务清理，既有素材、人工编辑、原始实验与审计证据继续保留。
- 保留 PG/算子语义、完整模型请求、旧路径验证、实验预算与失败记录、数据盘与备份、容器数据和人工 PPT 保护；各类结果继续按真实运行身份表述。
- 31 份规则由 1,144 行精简至 668 行；字符数约减少 29%。这是文本规模统计，未运行 agent 行为对照，不代表已证明速度或质量改善。
- CLAUDE.md 继续只导入根规则，无需复制新内容；此次不改运行代码、实验事实或全局 skill。规则检查覆盖禁词、文档引用、重要要求保留、格式及隐私。

## 2026-09-14：统一禁词要求并精简文字规则

- 按用户要求，根 AGENTS 第 6 节改为统一禁词清单与直接改写要求，删除受众分类、技术术语例外、改写示例和重复代号说明。
- 验收只检查本次新增或改写内容；图形规则引用根清单，运行环境规则改用具体动作描述。原始实验数据和历史日志保持原样。
- 保留研究对象、论证层次、术语解释和初步结果的表达要求；本次只修改规则与日志。

## 2026-09-14：M1常驻服务与C4/C8重复

- 按用户“继续”完成有限常驻实验适配器；PG拥有各查询，group拥有gateway/tokenizer和持久POST额度。
  复用生产调度与已有评价；Job归零、session结束和transport关闭分别记录，不伪造每查询关闭。
- 源码4f06ab1e完成24查询/768受控POST；旧入口另1查询/120次通过。最终e480bc30补强失败汇总，本地/Linux各16项行为检查，761份code文件一致。
- C8五次均快于对应C4；紧W相对宽W的HTTP P99降低42.23%，完整查询时间增加233.06%。
  tokenizer/gateway一次启动约5.5秒，常驻后每查询准备约14–15毫秒；固定合成输入不能证明真实性能。
- 离线复核器第一次触发write-once评价拒绝，失败保留；改用读取已记录结果与纯函数重算，无查询重跑/新请求。
  所有PG/HTTP/任务进程/socket均关闭，GPU空闲、ACL恢复，原始私有证据保留。
- [完整结果与全部重复](experiments/results/postgresql/waiting_positions_persistent_20260914/README.md)。下一步仅准备新的真实输入、服务签名和有限请求计划；M2与低扰动消融尚未开始。

## 2026-09-14：M1首轮等待位置检查

- E完成后按用户要求启动M1；复用请求FIFO、token工作描述和work限制，没有改变生产调度选择。
- query_runner记录调用入口，waiting_positions复用独立PG绑定和flow事件，补齐提交、权威终态、PG就绪/节点/客户端五段时间并检查守恒。
- 首轮6次调优+9次比较共384受控POST；C4/C8中位数近乎打平，按预定规则选C8，未称稳健最优。
- 保留宽token两次约1.06秒HTTP异常；实测默认backlog5后，用backlog32和handler时序观测做固定C/W的9次/288POST跟进，没有重新调优或替换首轮数据。
- 跟进紧W相比宽W，HTTP P99中位数-39.03%，SQL至EOF+181.29%；查询前准备约5秒来自当前每查询新gateway/tokenizer生命周期，不能称生产查询的必付成本。
- 两轮共24查询/672受控HTTP、真实模型0次，12项相关检查与独立读回复核通过，840份代码一致；PG/HTTP/进程/端口/ACL清理。
- [完整报告与全部重复](experiments/results/postgresql/waiting_positions_pilot_20260914/README.md)。下一步继续M1常驻服务对照与并发选择稳定性，M2尚未开始。

## 2026-09-14：查询依赖与共享计算接入

- 按用户顺序先完成E，再准备M1/M2；不启动F或任意SQL组合扩展。
- 优先核对已有SessionCapacity、JobRegistry、轮转与FairEndpointCreditCoordinator；只增加计算上限组装策略，未重复实现额度账本。
- query-job生成Map新增可选管理员窗口配置，默认与child安全预读规则保持原状；CLI和接口说明同步。
- 通用Filter核验从Movie查询模块迁到filter_bindings.py，语义指令显式传入；保留Movie旧入口和测试。
...
- 按用户提供的研究审查，在33次补跑完成并将3f1f5540推送main后更新文档；不新增模型运行或生产机制。
- 当前数据执行计划加入五类设计选择/证据/反事实/否定条件，分开历史外部图像、C/D和本轮PG真实/fixture证据。
- 明确资格、提交、权威完成观察、PG就绪/节点/客户端终点，以及查询专属准备；不把Core终态称为GPU完成或把推迟接纳当时延改善。
- 写出有作用域的资源归纳、有限样本面积、串行按序FIFO反例及有限动作2ε+δ推导；完整代码不变量/进展和一致误差保证仍待证明。
- M1等待位置、M2全局元数据优先，仅准备测量和对照；M3表示膨胀与M4过早承诺按需接E/F，不将六工作包当贡献或串行前置项。
- 知识库补迁移条件卡和消费进度候选；撤回早期“混合必优”及无条件输出一致表述，超长行处理回到语义规范。
- 正式题名/摘要与既有精读分别标注；新增三项基础理论题录，不调整Top15或声称全文已读。
- 同步研究入口、总纲、索引、baseline对照与状态，修正旧“当前单Map切片”失效锚点；下方原始实验与历史设计保持原值。
- 文档链接、数学/证据核对和隐私检查通过后提交；此轮无新GPU请求。

## 2026-09-14：剩余33次真实模型补跑完成

- 用户明确批准先完成剩余33次；使用修正并预检通过的work4096，不重跑前24次，不退款或修改旧记录。
- 四个8行flow单元和1个真实响应加慢写入期限单元全部符合预期，账本/服务日志均33次，另5次cache-reset管理请求。
- 四组8行输出均与正常PG一致，6/8分类正确，关联/资源审计通过；暂停时队首均未就绪，未区分ready-first目标分支。
- 末次写入在5秒期限后51.928ms发现过期，真实1条结果保留完整SHA但execution仍失败、评分被拒；HTTP责任归零。
- 补跑228.211秒内清理，833份代码/配置一致，PG/模型/任务/GPU无残留、ACL恢复；累计57次实际请求，原失败8次预留保留。
- [报告及独立补跑证据](experiments/results/postgresql/async_deadline_flow_20260914/README.md)。
...
- 组织审计从原始输入选择取得预期数量；零任务lazy-open可无drain，非空及已开流仍要求完整证据。
- 本地定向27通过；调度371通过/1项缺pyarrow，实验482通过/15环境跳过，provider53通过/2跳过，PG Python116通过。
- 21轨迹1,380任务重放结果不变；原始模型输出、性能值和失败记录保留。
- 用户明确服务器未开机、不进行服务器测试。未连接服务器、未起模型，PG空选择与发送缓冲诊断脚本已准备但未执行。
- 修订在codex/capacity-wait-empty-map，服务器集成pending，暂未合并main。
- [范围、证据和后续验证](experiments/results/scheduling/capacity_wait_empty_map_20260912/README.md)。

## 2026-09-11：C/D审计修订与新真实预算对照

- 对照旧c4354cd6审查，复用已合入d2f9cc13的A/B公共修订；独立重建work/request责任、核验原始提交顺序和生成预算。
- Core容量拒绝观察默认关闭；组织adapter明确启用，两种拒绝来源分列，不改变调度/归还、不增加整组屏障。
- Linux调度370/provider55/PG合同116/审计19、实际PG组织1与旧入口3项通过；本地493实验工具14项跳过，调度缺pyarrow的失败保留。
- 用户允许加入真实模型测试；新上限1152 POST/20分钟，16单元全部有效，服务端成功数与账本一致，模型准备至退出319.11秒。
- C同配置equal/total约3.75/3.81秒；D紧work预算三组均更慢，但工作量不足的真实拒绝明确出现。rows紧预算有1行模型输出差异，不改写成全部预测一致。
- 733份代码一致；模型/PG/socket/任务进程和原ACL已清理，私有输入/预测与失败保留；E/F未执行。
- [完整报告、全部预热/验证值和原始摘要](experiments/results/postgresql/cd_validation_20260911/README.md)。

## 2026-09-11：A/B审查后的查询验证补充

- 复用共同记录器完整性校验；补direct/Ray独立行关联与重复payload反例，模型答错仍进入质量分母。
- 原生查询延迟至迭代，显式记录进入阶段首错；Direct慢源读取与完成结果同时推进，三类责任共用窗口。
- 抽取全过程逻辑资源核验；Ray CPU/actor/读取/HTTP分开，动态进程与初始读取RSS可追溯，未知峰值不填零。
- 保留A2正常容量点约8.44%较慢的观察；不改写旧结果，不继承真实模型额度，不推进E/F。
- 本地483项（14跳过）、Linux相关60项、完整查询两次各7项及兼容3+4项通过；4组集成2720受控HTTP、模型0请求。
- Linux扩大测试10项缺少快照外历史依赖/Git元数据的失败保留；745份code源码一致，PG/socket/进程/ACL已清理。
- [完整检查、来源及后续事项](experiments/results/postgresql/ab_validation_20260911/README.md)。
...
- 旧执行1dd6d060与新执行e42b43b8共用后者测量代码；固定资源，仅交叉L与结果预留，完整两次重复保留。
- L64/结果32MiB的offer由23836/24042降为862/858，JCT约17→10秒；其他配置无一致改善。
- 全部绑定/请求集合/资源检查通过，非强D0或组织算法结论；模型和两套诊断PG正常关闭，两卡1MiB，其他受控PG及ACL留待后续任务清理。
- [完整诊断与原始指纹](experiments/results/postgresql/execution_repairs_20260910/README.md#同配置的真实模型修复前后诊断)。

## 2026-09-10：A诊断启动失败与子进程源码路径修复

- 用户授权8,448模型POST/20分钟；共同测量的旧/新执行先通过零模型检查，但正式预热gateway因缺PYTHONPATH未能启动。
- 实际模型POST为0，128预留未领取、不退款；204.31秒内正常停止模型和两套诊断PG，未自动重试。
- runner显式传递其绝对源码目录；新反例及移除调用者PYTHONPATH后的实际PG/HTTP与相关37项通过。
- [原始失败与修复证据](experiments/results/postgresql/execution_repairs_20260910/README.md#真实诊断的零请求启动失败与修复)保留；匹配诊断尚未完成。

## 2026-09-10：执行修复工作包 A 的代码与受控验证

- 按四份审查及用户修订路线分 A–F 独立交付；严格按需 SQL、多Job足量独立存储已确定。
- 修复执行错误被慢清理/producer采集覆盖、永久拒绝误作反压、PG重复offer及全局预算耗尽后的空等。
- recorder保留旧schema并增加版本化时点/错误；观测内容与写入方式独立，完整事件只写私有文件。
- PG32行受控offer660→62、提交32→32，相关责任未释放时重复探测598→0；PG回归1/1、TAP2043项通过。
- Linux调度369、provider51、PG静态116、受影响工具78项通过；实际PG＋本地HTTP检查两臂/观测模式/超时。
- 历史归档只导出计数24708/512，修正非单变量预算对照、旧direct紧凑同步日志和失败计时解释；原始记录保留。
- KEN加入核心补充与知识库，题录/摘要已核验，全文与artifact仍待核查；Top15不变。
- 真实模型请求0；专用PG保留供后续受控验证，最终服务/ACL清理另记。详见[报告](experiments/results/postgresql/execution_repairs_20260910/README.md)。

## 2026-09-10：容量工具主干集成

- `13d349096c8e66d2986115d86df21c5d607f93a2` 已推送功能分支并快进合入 main；合并后683个执行代码文件与服务器版本一致。
- 仅补充集成事实；实验数据、失败、预算和强D0尚未成立的结论保持不变。

## 2026-09-10：Map 容量工具修复与单卡画像

- 按两份审查实现独立PG输入绑定、v1账本容量保护/单元预留、分段记录、缓冲事件、自然/分层选样和配置runner。
- 本地/Linux各214项、PG回归1/1与TAP2032项通过；真实PG fixture验证重复文本、乱序与跨查询从零编号。
- 单卡96601次真实出站、99808次预留；62个完整查询和1个8MiB配置拒绝。失败756行和未用3207额度均保留，未退款。
- 五次4000行观测得到C32/L64候选，强D0因缺少高并发同规模平台仍未成立；独立样本各2000行验证后未调参。
- 旧10次验收的序号独立性表述已补充限定，原始证据未改。测试服务停止、两卡1MiB、临时ACL恢复，77.94min内清理完成。
- 结果、完整重复值与后续工作：[容量报告](experiments/results/postgresql/map_capacity_20260910/README.md)。
...
- 按用户三份附件与当前 main@0ba12bfa 核对，近期顺序改为真实单 Map 任务、静态容量画像、有限窗口
  组织，再进入多 Job；在既有数据组织计划中维护具体切片，不重建 benchmark 平台或共享调度算法。
- 隔离分支 codex/data-execution-pilot 新增 SQuAD 双消息清单/离线评价入口，复用 importer 与 EM/F1；
  补实验观测时间戳，复现并修复缺失预测在归一化空答案上误得满分。本地/Linux各125项相关测试通过。
- 服务器只使用一张 GPU 进行正确性检查，48/68次请求后停止。SQuAD同步/增量各16行答案完全匹配；
  原增量检查误用完成事件顺序，离线按序号与payload核对通过，不重跑、不补造未保存的查询时间。
- ShareGPT在同步第12行length停止，另发现摘要任务未被遵从、临时准备脚本误改一行文本和tokenizer
  计数错误；失败与修正后的离线审计均保留，不视为合格原样数据或性能证据。剩余额度未使用。
...
- 按用户授权，从c2bf4c5e快进至1138bee7，包含25c0f090与接纳/生命周期修复，无冲突和源码重写。
- 合并后654份源码/测试及归档校验均匹配；最终26次真实模型请求、521项Linux检查与PG回归1项/TAP2006项通过，测试服务清理完毕。
- 本次仅登记主干集成，未因文档更新重复模型运行；详见[集成记录](experiments/results/scheduling/query_lifecycle_20260909/README.md#主干集成)。
...
- 动态借用明确为可选研究策略；“GPU资源管理”收敛为服务容量和工作量驱动的提交、路由、多Job调度，不扩展为显存分配器或模型内部调度。
- 同步多Job详细设计、主架构入口、代码README和状态摘要；代码、实验结果与既有资格均未变化，本次没有运行服务器或模型。

## 2026-09-08 三项会话接口清理合入main

- 按用户授权将main从2ad67c06快进到61fa6967，无分叉、冲突或源码修改。
- 合并前重新校验644份源码/配置及74份归档材料，最终单Job12次、多Job18次真实请求记录均通过；此前Linux519项和PG回归1项/TAP1963项对应同一实现。
- 本次仅登记主干集成，不重启服务器或模型。三项整理、接口迁移及Bug记录见[验证报告](experiments/results/scheduling/session_api_cleanup_20260908/README.md)。

## 2026-09-08 会话公共接口、Job选择与旧适配器清理

- 基线2ad67c06，在独立工作区codex/session-api-cleanup完成三项非阻断整理；公开fail、派发控制及消费者关闭操作，网关不再调用私有方法或改会话状态。
- choose_flow接收不可变就绪身份与历史，默认保持Job优先轮转；Engine校验候选并保留账本和所有资源检查。
- 迁移原单流适配器测试到统一生产网关；删除旧runtime/adapter、无消费者错误回调与全局字段，生产源码净减70行，同步参考保留。
- [验证与Bug记录](experiments/results/scheduling/session_api_cleanup_20260908/README.md)：Linux519项、PG回归1项/TAP1963项；最终单Job12次、多Job18次真实请求通过。首轮12次也保留，累计42次，无模型请求重试。
- 新close_consumer初稿释放容量后漏发wake，新增断言先失败再通过；最终68项本地相关测试通过。保留测试字段名和环境准备错误的原始记录，74份材料脱敏扫描后压缩。
- 最终643份code文件与根pyproject共644项一致，模型/PG/gateway服务已清理。主工作区45个字节相同副本、1个与f6cd2392一致的历史副本经授权删除，原件均可恢复。

## 2026-09-08 增量执行与多Job基础合入main

- 本地与远端main原为`5fbcc754`；核对干净工作区后快进到已审查的`f6cd2392`，完整包含该分支15个提交，无冲突、无代码改写。
- 合并前重新核对110份归档材料及644份源码/测试文件hash，全部一致；已有Linux515项、PG回归1项/TAP1963项及最终12/18次真实模型证据适用于此次源码。
- 本次集成不重新启动服务器或模型；仅补记主干状态与后续整理项。结果与失败修复见[验证报告](experiments/results/scheduling/multijob_20260908/README.md)。
- 同查询多算子可信Job归属、动态借用、方法状态总预算和GPU资源管理仍待完成；公共关闭/回收API、Job选择策略接口及嵌入式旧适配器退役按后续设计推进。

## 2026-09-08 多Job共享执行与网关复用

- 一个Engine登记多个Job/流，统一任务账本检查服务/Job/session额度；Job能力句柄禁止按字符串冒认归属。
- 按Job再按流轮转；资源份额策略移入执行组装入口，受控非均分3/1验证接口可替换。
- 有界命令/结果槽与独立取消通知隔离慢连接；结果lease发送结束才释放，未知远端请求保留额度。
- 删除生产专用单会话增量循环；增量支持1/N Job，同步协议与增量共用连接生命周期。保留仍使用的同步Filter/Map、recording/golden及嵌入式兼容API。
- [验证](experiments/results/scheduling/multijob_20260908/README.md)：Linux515项、PG回归1项/TAP1963项；真实单Job12次、多Job18次均通过，跨Job HTTP峰值2，14增量任务与9个Job资源归零。
- 644份源码及9项模型文件核对一致，测试服务清理完成。保留lint/兼容回归/检查目录与端口启动失败记录；补齐accept计数和关闭自唤醒修正，原慢关闭用例50次通过。前后两组真实模型累计60 POST，最终两组各12/18次通过；端口失败为0 POST。110份材料明文脱敏扫描后压缩。
- PG仍只管本地缓冲与查询生命周期；同查询多算子可信Job归属、动态份额借用、方法状态总预算和GPU资源管理后续在现有执行层接口继续。

## 2026-09-08 消费期限修复与v5窗口1桥接退役

- 修复新交付结果漏报next_deadline；保留交付资源转移顺序，回归覆盖分阶段/旧超时及关闭消费期限。
- 先以受控、PG和10次真实请求对照旧桥接与v6窗口1，再删除旧适配器、CLI、PG分支和专属身份，迁移原生命周期测试。
- 相关实现净减101行，清除孤立序号字段；旧名称明确拒绝，同步参考、wire v5及历史证据保留。
- [验证](experiments/results/scheduling/bridge_retirement_20260908/README.md)：最终Linux503项、本地52项、PG回归1项/TAP1963项；删除后窗口1再跑10次、窗口2再跑12次真实请求，均通过。
- 三份独立账本共32次POST，无模型重试；最终640份源码一致，全部测试服务和资源已清理。62份材料脱敏扫描后压缩保存，包含原始失败复现。
- 多session/多Job、方法状态总内存预算和其它兼容入口迁移仍是后续工作。

## 2026-09-08 阶段超时与执行组装整理

- SessionTimeouts分开排队、执行和消费期限；生产网关不再用单次模型超时限制排队，原调用方默认行为保留。
- 会话模块移出HTTP及默认策略/工作量组装，执行工厂复用现有核心与策略；合成长度与末项优先策略通过实际v6适配器验证。
- 修正PG绑定设计中过期的1–64描述，CLI文档以v6为首选入口；v5窗口1兼容路径保留并记录迁移步骤。
- [验证](experiments/results/scheduling/phase_policy_20260908/README.md)：本地50项、Linux501项、PG回归1项/TAP1961项、独立12次真实请求全部通过。
- 641份源码一致，HTTP迁移AST核对通过，使用已核对的原C扩展；服务和资源已清理，25份本轮材料脱敏扫描后压缩保存。
- 多Job资源管理、v5桥接退役和PG窗口模块提取仍是后续任务；没有删除仍在使用的同步调度器和wire版本。

## 2026-09-08 增量容量与完成结果整理

- 接纳任务数、输入/结果字节和后端并发分别配置；PG按窗口分配数组并检查字节预算，删除固定64项限制。
- v5/v6共用完成解析；修复v6非法输出和终止路径遗留已完成结果配额的问题，未确认远端工作仍保留配额。
- SessionLimits使用具名字段；等待使用同一session配置；非法预算在线程启动前拒绝。
- [验证](experiments/results/scheduling/capacity_cleanup_20260908/README.md)：本地14项、Linux146项、PG回归1项/TAP1961项，65行窗口与2请求并发的受控测试通过。
- 两轮独立账本各12次真实请求通过；最终639份源码一致。第二轮覆盖预算校验前移后的最终源码，测试服务已停止。
- 仍保留v5窗口1桥接与同步参考，不将本次变化表述为多Job资源管理完成；35份本轮材料脱敏扫描后压缩保存。
...
- 复用现有提交、路由、传输与结果lease，不复制LOTUS执行循环；算法、解析和校准留给后续方法适配。
- 澄清code/AGENTS的完整请求规则适用于单个生成阶段，多阶段由方法明确授权；没有改变prompt拆分限制。
- [验证记录](experiments/results/scheduling/method_framework_20260908/README.md)：本地44、Linux325项通过，621份源码一致。
- 真实两阶段测试共4次请求；首轮脚本delivery bound错误在0次POST停止，保留失败后修正脚本，复用同一预算。
- 所有资源归零、传输/模型退出、端口关闭、GPU空闲；没有新增优化算法、生产PG桥接或性能结论。
- 吸收结构审查，将单流数据组织/批次成员映射、统一表示预算、多session共享及旧runner迁移终点登记在现行设计§14。
...
- 按用户授权，将`codex/semfilter-and`从`1d83c975`快进合并至`1b8fa3b0`，包含已验证的
  双Filter/有界gateway实现及三次设计文档提交；合并无冲突。
- 与66887463相比生产代码无变化，沿用该版本Linux138项、PG18.3回归及1808项TAP证据；
  本次文档检查通过，不重复模型测试，也不将设计标成实现。
- 同步现役入口的集成状态，包括扩展README和结果索引/证据台账；下一步在独立分支开展A1行为保持提取。

## 2026-09-07 近期规格补充审查定稿

- 吸收用户补充资料并定点修订两份现役详细设计：明确Delivery交付提交点、唯一终态结算、
  可立即推进状态、按任务阶段计时、Engine残余归属、UNKNOWN处理及连续session作用域lease。
- PG新Map选用显式PG权限设施；A2a先验证setrefs后的权限/依赖与OFFSET/LIMIT、普通VOLATILE
  输出的求值位置。风险来自静态审查，尚未写成现有路径漏洞或已验证修复。
- 明确FIFO、bytes、单成员提交、最坏结果预留时机和FINAL_MAP是首版选择；保留类型、阶段资源、
  策略选择和工作单元的扩展位置，不提前实现图像/视频或多Job。下一步从行为保持提取开始。
- 本次文档修订不改变66887463的生产代码或扩大原有实验结论。
...
- PG首批限定A1行为保持及A2a一个Filter→一个生成Map；多个/依赖Map后续单列。
  Core先定内部状态/所有权，不借wire待定回避设计；B4拆为单流窗口1、扩大窗口、多节点共享三步依赖。
- 修正主文query-fixed歧义、gateway已选线程方案时态和旧Map待引入表述。
  本轮只完成设计文档与源码静态核对，没有编写实现、动态表征、运行服务器或请求模型。
- 本地文档链接/代码围栏、既有锚点、读者型术语与git diff检查通过，8份文档隐私扫描通过。

## 2026-09-07 总体能力与接口设计收敛

- 按用户要求吸收补充设计，基于已推送`66887463`更新同一架构主文档，未创建并行权威设计。
- 以长期能力组织PG计划适配、算子语义/方法、SemLoom Core；分开调用、方法、任务和工作单元，
  补充合法求值范围、值与关系语义、查询/流身份、计算与结果资源分账及单一业务接纳者。
- 当前逐行请求和同步reference继续保留；跨行语义batch、OR/CASE载体、融合、重扫及多阶段方法
  明确为待定义/验证或另行授权能力，不因设计存在而开放。
- 将实施统一为数据库、增量Core和共同支撑三线，给出消费者、依赖、反例和停止条件。
  缩短已完成目录/AND切片叙事并保留证据链接与66887463历史恢复身份，工程参考版本与专项合同不变。
- 同步总纲、根入口、项目索引、计划索引和代码状态说明；仅文档修改，没有连接服务器、调用模型
  或运行代码实验，main与生产代码不变。
- 文档本地链接、既有引用锚点、术语检查、git diff --check和7文件隐私扫描通过；未重复运行代码测试。

## 2026-09-07 两个Filter AND与gateway多会话实现

- 在`1d83c975`上建立`codex/semfilter-and`，按用户要求重审单会话假设。分开CLI装配、连接生命周期、
  协议分发与模型请求名额；保留同步wire，无待执行队列，容量与超时可配置。
- PG调用分析与路径构造分开，复用现有CustomScan/pump/runtime。真实反例促成中间投影隔离和
  原生函数权限/依赖保留，避免两个VOLATILE输入被只计算一次。没有增加通用组合执行器。
- 两项独立审查发现并修正HTTP超限/提前EOF误退容量、连接拒绝误占容量、首帧超时观测时序与
  相同payload并发请求归因；旧错误语义和其他路径通过回归。
- [验证记录](experiments/results/postgresql/semfilter_and_20260907/README.md)：本地137通过/1项Linux跳过；
  Linux138/138，PG18.3严格构建、regression1/1、8个TAP1808/1808，584源码哈希一致。保留7轮PG结果，
  包含失败而非只留最终绿色。40个顺序会话后存活gateway的FD/线程回到初值，未作RSS或性能结论。
- 只传必要自有代码增量；未改服务器原工作树或已有服务。测试进程已清理、真实模型请求0。
  本分支尚未合并/推送；Filter→Map、单节点多在途、正式资源与质量实验仍分别推进。

## 2026-09-07 扩展目录重构合并到本地main

- 用户在服务器验证完成后授权合并。获取远端main确认仍为41e103f2，本地main无并发改动，
  已从41e103f2快进至103e2715，无冲突；当前未推送。
- 合并前核对生产代码/测试与已验证20b22a55一致，全部公开证据哈希匹配，92项变更文件隐私扫描通过。
  沿用同源码的Linux112+4项、PG18.3严格构建、regression1/1和全部TAP1758/1758，不重复运行服务器。
- 同步现行入口、状态和计划；原始JSON及历史日志保留各自运行身份。没有新增组合、异步或模型结果。
...
- 合并提交为 `a29f5d04`；后续文档复核移除合同残留的“当前不能执行生成请求”，并将旧 Filter
  constrained-decoding 诊断写明为历史状态。当前 Map 的 PG＋golden 可执行不等于真实模型已验证。
...
- 同步精读库 README、research README、文献清单和项目索引：当前为 18 篇主笔记、160 张论文原图
  裁剪件。本次未新增 PDF 或论文原图，不调整 Top 15、十五篇横向速览或开题正文。
- 本轮只做登记、文件与提交检查，没有重新执行全文事实核验、PG 测试或模型实验。
- 首次仅大小写改名后文件消失，暂停提交并从仓库外原始副本恢复；恢复文件与该副本逐字节一致，
  为 1,478 行、56,454 bytes，SHA-256 为 `529e98b42a7abe979ba01bc9cf039063c55dfa3099a0043d6d4c5d62486bb110`。
  确认恢复后才重新应用上述出处、措辞与空白整理，原始副本未修改。
- 登记检查通过：107 个本地链接、15 个章节引用、读者措辞、差异空白和 6 个本次文件的隐私扫描。
...
- 32 次按本轮逻辑推理请求全程累计，包含直接探测、主动预热、失败/超时/取消及结果不明；先记账后派发，
  重启不重置，不必用满。4096 包含 template 后 prompt 与输出，保留 max_tokens=128 的常规样例额度。
- gateway 以同一预热且存活的进程为基线，峰值≤32 MiB、关闭后 60 秒内结束增量≤16 MiB并回收会话/请求/FD；
  不能通过重启/杀进程或重置跨轮基线证明恢复。PG 原阈值不变，压力测试只用 fixture；异常/超限/预算耗尽即停。
- 只更新计划和本日志；未改源码、访问服务器、启动模型或运行测试。实际 revision、具体样例和精确请求超时
  在完整 Map/golden、模拟 HTTP、PG18.3 前置验证通过后、首个真实请求前登记。本次批准不提供新的运行证据。
...
- 复核固定范围 `c494e1b2...390f666a` 的 6 个提交，工程规范和计划符合性两项审查均未发现阻止合并的
  代码问题。生产修改仅在 `extension.c`；未引入对象缓存、跨会话 registry 或 core patch。
- 本轮独立复跑本地 94/94（PG/protocol 68、gateway 5、calibration 10、choice 工具 11），中立 C11
  语法检查通过。PG18.3 `-Werror`、regression 1/1、TAP 1022/1022 仅复核已归档证据，未重跑服务器测试。
- 核对四份公开 SHA 清单的 121/48/60/48 项及两轮各 82 个源码哈希。回归输出初次直接比较不一致，
  确认为公开副本去除了行尾空白；按运行器相同处理核对后完全一致。结果记录已区分服务器原始哈希
  与公开副本哈希，raw、失败记录和原验收提交身份保持不变。
- 已将 `390f666a` 快进合入 main，并同步当前计划、状态、README 与结果入口；先前独立分支日志保留
  原历史表述。仅成员 ADD/DROP 后仍须在受控维护中刷新所有相关物理连接，不承诺自动刷新或即时权限撤销。
- 推送前全仓隐私检查与 89 个新增文本 blob 版本扫描通过。未调用模型、使用 held-out 或重跑资源 smoke，
  未修改公司源码或服务器环境。下一步仍是四 D 最小生成型 Map 合同，而不是自动启动其实现。

## 2026-09-02 函数对象身份加固与双会话手动刷新

- 从干净 `c494e1b2` 建立独立 `codex/pg-function-identity`，未改 main。先按主计划只读核对公司
  函数成员解析及官方 PG18.3 API，来源和取舍只记工程计划；公司源码未修改、复制或上传。
- `aaa52967` 用真实 PG18.3 复现未安装扩展时普通同名 Map 被接管，2/2 反例失败。`792a0408`
  仅在 `extension.c` 增加扩展成员校验，反例转为通过；不改 SQL 属性、runtime/provider 或协议。
- 测试扩展时因同 OID 替换遗漏原参数名中止，失败保留；`5f144cec` 修正夹具后完整 990/990。
  额外双会话诊断发现仅改变成员关系时 generic plan 不自动刷新。用户确认临时使用要求：暂停查询、
  结束旧事务及游标，DDL 提交后刷新所有相关物理连接（包含连接池）再恢复；不只刷新 DDL 会话。
- `934f4f61` 保留函数替换/删除重建自动失效测试，新增四种调用形式的双 backend 仅成员 ADD/DROP
  检查。最终 PG18.3 warning-free `-Werror`、regression 1/1、TAP 1022/1022（身份 103 项）、本地/服务器
  各 94/94、C11 通过。记录刷新前后计划并核对 OID/函数定义未变；不是连接池实现或自动失效资格。
- [证据](experiments/results/postgresql/function_identity_20260902/README.md)保留两轮独立构建、48+48 个
  服务器公开文件及各轮 82 个源码 SHA、反例与失败记录。本地漏 PYTHONPATH 的工具输出无独立原始文件，
  沙箱 bind 失败日志另存；获准重跑通过。无新模型请求、held-out 或资源 smoke，旧四 C 证据不重新绑定。
- 仅成员变更的跨会话自动失效仍 pending；无法保证全连接刷新时不采用临时方案。不承诺在线无感 DDL，
  不将成员移除当即时权限撤销。未新增 registry/core patch，未实现真实 Map、多算子或 gateway 多会话。
  两轮测试节点均停止，历史安装未覆盖，服务器产物与测试工作树保留；主线合并/推送不在本轮执行。
- 收尾核对 140 个本地 Markdown 链接、全仓 7,760 个跟踪文件隐私扫描及差异检查通过；122 个原始证据
  文件（含总 SHA 清单）纳入 Git。读者用语命中仅为已解释历史术语及 SQLSTATE，未增加新的内部代号。

## 2026-09-02 choice 主线推送前独立复核与文档清理

- 审查范围为远端 `dd8cbf89` 到本地合并提交 `2820dcb5` 的 30 个提交。按工程规范和四 C 专项要求
  分别核对，未发现阻断推送的实现偏差；PG plan/profile 所有权、严格 v3/v4 分流、共享 codec/session、
  lazy provider 生命周期和受限 Filter INSERT 与计划一致。版本兼容入口有真实职责，不继续拆公共层。
- 清理主计划 §12、实验入口和结果摘要中仍称四 C 未合并或待验证的文字；旧切片只记录当时状态，
  当前 main 已包含 choice。已完成计划用于回查要求，真实 Map、多算子、独立核心及 Filter 质量/校准
  仍由主计划安排。补齐 SQL 第四个 option 与 wire v4 的说明，移除两个源码文件中过期的计划位置和阶段注释。
- 清理前后本地相关测试各 94/94；四个中立模块严格 C11 语法检查通过。C 头文件预处理前后相同，
  Python 差异仅为 docstring；没有修改执行逻辑、SQL 或协议，也未运行服务器、PG TAP 或真实模型。
- 离线逐项复核[真实服务记录](experiments/results/postgresql/choice_service_20260902/README.md)：7 组请求仅差 choice，
  14 次的 SQL 行数与 usage 一致，两个 NULL 对照零调用，累计 15/100。223 项公开 manifest、66 项原始
  源码及 2 项清理源码 SHA 匹配；8 份 SHA256SUMS 的 156 条也全部通过。原始证据不修改或重新绑定。
- 全仓隐私扫描 7,636 个文件无新增问题；待推送历史中的 637 个新增 blob 版本也通过相同规则检查。
  读者用语、本地文档链接与差异检查纳入本次收尾；按用户授权提交这组文档/注释修正，再正常推送 main。
...
- `7da954f9` 新增实验专用预算 ledger、实际 POST 观测与 PG18.3 资源 runner；没有改生产 PG、wire、
  gateway 或 HTTP Adapter。8 项预算/HTTP 测试通过，加既有 83 项，本地合计 91/91。
- 受控资源最终源码 `4464fe9b`，复用 `39007150` 已验证 PG18.3 二进制并核对 SHA。v3/v4 各 5,164 次
  65,536-byte fixture 调用、10 次取消、10 次阻塞 DNS/恢复通过；原始时间序列、失败与 manifest 保留。
  初轮等值断言因测试表 `_fsm` FD 减少而失败，诊断还遇到容器 proc 权限；对应 UID 诊断后，先完成表
  维护并仅关闭自有表的自动 vacuum，再独立重跑，不放宽阈值。未重跑构建/TAP，旧 919/919 不重绑定。
- 零任务探测复现串行 gateway：第二连接须等第一连接关闭。仅登记限制，未实现多会话、两个 Filter AND、
  真实 Map 或 SemLoom 增量调度。真实模型请求为 0，未创建真实累计 ledger、访问 held-out 或恢复校准。
- 按用户新说明吸收 main 未提交主计划 §8.7、§6.4 及 PG 基础检查/组合/Map 的安排，替换分支旧的局部
  参考表，仍只用一个工程计划；SQL 注册、对象身份、载体、请求/结果、生命周期与外部执行分别对照。
  main 计划读取时 SHA 为 `c0ab650c4207204688b459ac80f8708adc7c46712cfa80ea5d3c497539d60656`。
  不复制公司代码，来源与取舍只在文档；主工作区 16 份用户修改未覆盖，没有新增规则文件或通用框架。
- 用户最后将后续顺序明确为“完整工程对照 → 真实 Map / 必要公共整理 → 可组合执行与研究接口”，
  已据此更新分支计划与入口；两个 Filter AND 不再是 Map 前置。对照 demo 的函数成员身份、距离改写、
  Map 参数共用/重扫、Filter disabled cascade 和 batch/config 做定点复核，未运行或修改公司代码。
  对象身份仍是待反例验证项，rescan 未开放，三类 batching 不混用；没有据此新增生产抽象。
- [结果与证据](experiments/results/postgresql/choice_resources_20260902/README.md)已登记。归档初版排除了
  后来发现仍在写入的自身日志后重新生成，原版保留；正式原始/公开 SHA 已核对。下一项仍是四 C
  受限真实服务检查，不把资源通过写成整个工作包完成。本地分支供审核，未合并、未推送。
- 最终检查：公开证据 170 个文件与 62 个源码 SHA 一致，173 个 Markdown 本地链接、读者措辞和
  `git diff --check` 通过；全仓隐私扫描 7,407 文件无新增问题。四个本切片集群已停止、无对应
  Python/PG 进程或 UDS listener，服务器临时 worktree 已注销；源码压缩包、bundle 和原始产物保留。
...
- 按“源码/原始结果 → 领域权威入口 → 总纲 → README/速览 → 历史计划”的顺序复核当前状态文档，
  修正 `code/INFRA_STATUS.md`、实验状态入口、证据台账、总纲、研究知识入口、开题 Claim Matrix 与
  图资产说明中的过时进度；带日期的历史过程保留，并在容易误读的位置增加当前替代入口。
- 五臂系统对照统一登记为 `93271012` 的 5/5 rehearsal、exactly-once 与独立 archive validation
  已通过；同时保留“只有一次 rehearsal、formal 未运行、0s/5s isolation 仅部分可用”的证据限制。
  VTC-compatible 8-client 1+3 正式实验、固定 active-work 的 request-level/service-quantum 对照、
  数据组织双卡复测、两作业错峰、18-cell 联合搜索和图像静态/observe-only 状态也不再列作未运行。
- 清理结果导航中同一 active-work/request-replay 目录的重复登记；旧五档曲线的 49K 仅保留为当时
  candidate，当前同签名选择由八档扩展曲线确定为 65,536。C128 容量数字统一为“实测达到 C256
  的 98.22%，满足预先规定 97% 选择条件”，不再把实测比例和选择阈值混写。
- SQuAD 三路径统一改为“完成性和答案质量可核对，细微性能暂不排名”：Project 外层计时额外包含
  指标采集、记录写入和结束处理，不能根据不到 1% 的记录差异判断快慢。同步修正结果 README、
  总纲、实验状态、Claim Matrix、答辩问答、图资产/审计和 PPT v9 构建源；没有覆盖或重生成已有
  PPTX，当前 26 页 v9 与最新 16 图/53 参考文献报告之间仍需单独差异审查。
- 开题物理状态统一为 16 篇精读主笔记、146 张论文原图裁剪、当前 PPT v9 为 26 页；旧 v8、
  20 页大纲、14 图/31 参考文献的审查文件均明确标为历史快照。实验结果与历史日志未删除，正在
  进行的工程重构代码及测试不纳入本次文档提交。
- 对外答辩内容基线逐项改写“冻结、门禁、闭环、产品轨、晋级、失效边界”等内部管理词，换成
  实验期间保持不变的对象、纳入比较前的检查条件、实际执行步骤和方法有效/无效的具体运行条件；
  同时修正其中“尚无 PPT”、production descriptor 未接入和同观测调度对照未完成等过时状态。

## 2026-08-27 新文档生命周期规则

- 根 `AGENTS.md` 新增“文档生命周期”，把新文档出现后的处理收敛为定位、放置、登记、替代、
  移动/删除和验收六步；每一步都有可检查的完成条件，避免普通文档、历史记录和当前权威入口混放。
- 明确规则只进入能覆盖目标范围的最窄 `AGENTS.md`，目录内容进入 README，领域事实按既有同步表
  进入权威文件，一次性材料进入 `tmp/`；普通文档不再另建并列 rules/guide 或平行索引。
- 新文档只有在角色与当前/历史状态明确、已登记、没有竞争权威入口、引用和必要同步已完成，并通过
  对外语言、本地链接、diff 与入 Git 前隐私检查后才算完成。本次未新增规则文件或改变目录结构。
...
- 审计 30 份 `AGENTS.md` 与根 `CLAUDE.md`，统一为“根规则始终生效、从根到目标目录逐级追加局部
  规则、README 保存目录内容和当前状态”的继承模型。根 `AGENTS.md` 新增规则路由和事实冲突处理
  顺序；`README.md`、`PROJECT_INDEX.md` 同步说明读取路径。
- 根 `AGENTS.md` 从 280 行收敛为 208 行（含继承示例、文档受众规则和旧章节引用兼容表），只保留全局研究范围、当前资格顺序、证据纪律、环境路由、
  正式实验共性、对外语言、变更同步、Git/隐私和 Wiki 触发条件。具体 K/W、GPU 拓扑、历史实验
  数字和逐项结果模板改由现有计划、证据台账与 runbook 承担，不在规则文件缓存。
- 文档语言规则提升为全路径继承：修改任何 Markdown/报告/PPT/图注前先区分读者型/对外与内部操作
  文档；受众不明确时按读者型处理。“冻结、门禁、闭环、产品轨、框架轨、晋级”等词在对外材料中
  必须改写为具体对象、条件和动作，子目录规则不得放宽这一要求。根规则给出逐文件搜索作为完成
  条件；本轮已把根 README、overview 快速卡片和 communication notes 中的相关管理化表述改成实际
...
- 30 份 `AGENTS.md` + `CLAUDE.md` 的核心规则入口由 1,481 行降至 1,077 行；两份 opening 专项清单
  由 196 行降至 86 行，33 个规则/清单文件合计由 1,677 行降至 1,163 行。29 个子目录
...
- 最终规则审计：30 份 `AGENTS.md` 均有同目录 README，29 份子规则全部显式继承；含 CLAUDE 与两份 opening 专项
  ...
  0，`CLAUDE.md` 只导入根规则。全仓 491 份 Markdown、422 个显式本地链接无断链；读者型样本的
  内部管理词复扫为 0。environment CLI/help、secret scanner 与 redactor import 可用；实际 preflight
  在本机因仓库外 runtime env 未配置而按设计 fail closed，状态为 `pending runtime configuration`。
  tracked/untracked secret scan 与 `git diff --check` 均通过。
- 本轮未删除任何规则文件、历史证据或清场候选，也未修改实验数据、代码行为、开题正文或图件。
...
- 将 `experiments/plans/` 分为顶层当前计划、`completed/` 已完成合同、`reference/` 跨实验参考和
...
- 合并 `code/scripts/serving/` 与 `code/scripts/services/` 的重复职责：vLLM identity launcher 移到
  ...
  并删除 CLI、动态入口或公共模块。
- 审计确认 `PROJECT_LOG.md` 中第二份 6,937 行连续历史块与前文重复；删除重复副本并统一历史条目
...
- 登记 Kalypso 为 arXiv 核心补充；arXiv 官方页面核对题名、作者、query-plan-aware API、
  memory-aware scheduling 与论文自述最高 4.57×。清理时对尚未纳入 Git 的 1439 行原始笔记执行
  仅大小写变化的重命名，在大小写不敏感文件系统上意外丢失项目内副本；先生成过一份明确标记的
  临时重建稿。用户随后指出下载目录保留原件，现已将 `/Users/junshun/Downloads/Kalypso_arxiv2026.md`
  正文复制回项目小写路径；目标仍为 1439 行，只去掉前言两行末尾的 Markdown 双空格以通过仓库 whitespace 检查，
  文字内容不变；临时重建稿已被下载原稿替换，
  下载目录源文件未改动。Kalypso 不进入现有 Top 15、十五篇速览或已定稿开题正文；
  `knowledge_hub.md` 将研究增量收窄为 PostgreSQL query lifecycle、多 Job、多独立 endpoint 与不修改
  vLLM 的外部控制，停止使用“semantic query plan 与 LLM serving 接口缺少系统研究”的旧表述。
- 全仓 Markdown 本地链接审计覆盖 491 份文档、422 个显式本地链接，移动后 0 个断链；旧移动路径的
  非日志引用为 0。精读主笔记、论文原图与可解析 PDF 实体数分别复核为 16、134、6。
  Python compileall、全部 shell 与 Node/MJS 语法检查通过；secret scanner 对
  6,797 个 tracked 文件和 20 个 untracked 文件均通过。unittest 收集 1,318 项，沙箱内无断言失败：
  12 个 error 中 1 个 Ray 权限项在沙箱外重跑通过，其余 11 个由当前最小 `.venv` 缺少
  `psycopg`、`daft` 或 `aiohttp` 导致；未绕过 runtime preflight 安装依赖。`pytest` 与 `ruff`
  当前未安装，分别记为不适用/待完整依赖环境验证。`git diff --check` 在最终修改后重跑。
...
## 2026-08-25 Parrot 精读笔记配图补齐

- 根据正式 OSDI 2024 proceedings PDF 将正文全部 Figure 1–19 插入 Parrot 精读笔记，分别放在应用工作流、三类问题、系统设计、目标推导、baseline 校准和六类结果的对应段落；每张图都增加读图顺序与不能外推的结论。
- 本地 PDF 共 18 页，其中包含 1 页 USENIX 封面和 17 页论文正文；笔记原先按 arXiv v1 描述版本，本轮已校正为当前实际使用的正式版，同时保留对应预印本信息。图像裁剪、页码、SHA256 和视觉 QA 见 `figures/audit/parrot_deep_reading_figures_audit_20260825.md`。
- 精读库仍为 15 篇主笔记，论文原图裁剪件由 115 张增至 134 张；当前可解析参考 PDF 由 4 份增至 5 份。同步更新研究入口、开题文献清单、参考索引、图资产说明和项目索引；未修改开题报告正文、PPT、实验数据或结论。
...
- follow-up K160 对照（frozen DRR K160，config SHA `ee2c4297…`，从 4-arm 程序化派生、仅 K128→160、其余字节一致）：**frozen-K160 = 14830.5 过门（96.30%），数值上比 adaptive 高 +0.33%**。冻结 K160 已足以解释过门，当前 sustained-overload 数据不支持额外动态吞吐收益；但该 follow-up 在 4-arm 后顺序运行，且 +0.33% 小于 adaptive 重复波动，不能声称动态增量严格为零或静态显著胜出。
...
- 同步修正统一绘图 helper，并重绘单 Job 请求/状态补充图和图像四 Job 图，使所有同类多行
  标签采用相同规则；数据与统计值未改变，300 DPI 严格检查通过。
...
## 2026-08-12 开题答辩 PPT v9 叙事收敛

- 将 v9 从 31 页收敛为 26 页：合并逐篇论文截图、验证页以及计划/预期成果页，结尾恢复为学校
  模板的“后续工作与预期成果 → 谢谢”两页节奏。
- 保留 17 张开题专用主讲图，并逐项确认图像 baseline 与图像四作业分别承担能力边界和并发干扰
  证明义务；没有混用旧图，也没有新增未经审计的数据图。
- 对外术语采用英文专业名首次附中文解释；清理内部代称，修正标签裁切、底部说明裁切、图文数值
  和章节目录高亮。
- 26/26 页渲染、0 空占位符、26/26 notes 来源、画布溢出和逐页目视检查通过；本机 Microsoft
  PowerPoint 已成功打开；未同步飞书或 Wiki。
...
- 修复最终视觉复核发现的文字—柱条重叠：文本 baseline 删除 panel 内重复统计说明，只在
  页脚保留均值±SD 口径；图像 120K panel 显式标明 Daft Built-in 在 20K OutOfDisk、未形成
  120K formal cell，避免被误解为漏画 baseline。所有对外图中的 `Bounded HTTP` 统一改为
  “直接调用（容量参照）”；内部 arm 名 `bounded_http` 保持不变以维持数据可追溯性。
...
- 只读核对确认A/C生成脚本已预置正确新标签，但当前PNG/SVG仍是旧渲染：A含无严格数据
  定义的“安全工作区/过载尾部”，C仍写“低压力近似中性”。按用户暂停绘图要求不执行脚本。
- 在权威开题报告证据章节顶部增加可见状态护栏：正文/CSV/图合同已冻结，A/C旧图禁止
  作为最终开题图引用，恢复后只做标签级重绘；没有删除或覆盖现有图资产。
- 未生成图片、PPT、飞书或Wiki内容。
...
- 原生单 job matrix 也接入相同 cell instrumentation，逐 run 保存 GPU 利用率/功耗和 vLLM gauge/latency/estimated-FLOPs delta；后续报告必须据此计算并列出 MFU，不能只报吞吐。
...
- 将初版 31 页收敛为 26 页：三张逐篇论文截图合并为四层研究现状页，验证标准并入验证方案，
  研究计划与预期贡献合并；图像 baseline 与图像四作业因证明义务不同继续保留。
- 结尾恢复学校模板的“后续工作与预期成果”双栏页和独立致谢页；目录页分别高亮当前章节。
- 专业接口采用英文名首次附中文解释；清理内部代称，修正 Direct Call 标签、图下文字裁切及
  图像数据叙述与图中数值不一致问题。
- 26/26 页渲染、0 空占位符、26/26 notes 来源、画布溢出和逐页目视检查通过；未同步飞书或 Wiki。
...
## 2026-08-17 P05 箭头与可读性修订

- 修正 P05 三栏之间连接器的 marker 缩放问题：SVG marker 改用 `userSpaceOnUse` 固定像素尺寸；
  蓝色请求与紫色提交箭头均使用 60 px 路径、4 px 线宽和 14×12 px 头部，灰色反馈使用 60 px
  虚线路径、2.5 px 线宽和 11×10 px 头部。三个箭头都有完整可见线身，方向清晰。
...
- 开题报告将 AI 算子代价估计的主要用途明确为数据库侧的计划优化与工作负载安排：估计 AI 语义算子的执行时间和资源需求，供关系算子与 AI 算子的计划比较以及多条 SQL 的接收顺序、并发方式选择使用。
- 报告区分了研究方案与当前证据。后续将评价结果含义相同的 SQL 执行计划和多 SQL 编排；当前 429 次实验只验证同一文本 AI 算子在四种在途工作量上限下的执行时间差异，不作为完整 SQL 优化已经实现的证据。
- 报告补充当前混合模型的解析估计与 Ridge 残差修正方式，并将 Daft、Ray、vLLM 和项目方法的分工改为正面说明。本轮未改变实验数据、PPT 或图片资产。

## 2026-08-23 PPT 与报告代价估计证据图修订

- 新增 `opening_cost_model_decision_quality_v4.{png,svg,pdf}`，并同步到开题专用图集 P16 与报告图 10。
  原始实验 JSON 仍位于 `experiments/results/`，图资产权威版本、材料选图副本和报告兼容副本分层保存。
- 六种估计方法分别展示 80 组候选均值的真实时间、预测时间和逐候选差值，同时保留四种上限的两两排序
  与 20 个留出情境的选择损失。报告正文同步说明 80 组预测与 429 条原始重复运行的区别。
- 图中明确区分时间预测误差与决策质量：解析模型的中位绝对相对偏差较小，岭回归的候选均值平均绝对误差较小；
  混合模型的证据是两两排序准确率 0.808、所选上限平均额外耗时 2.90%、最差 14.72%，不称其单点时间预测最准确。
- WPS 整页渲染和独立视觉复核通过；生成的 PPT 副本保留 36 页全部正文和备注，只替换第 29 页图片。

## 2026-08-23 开题答辩 PPT 备注与后续计划修订

- 桌面版 `0819.pptx` 的 36 页备注整理为简短讲稿与详细讲解两部分，建议讲稿时间共 561 秒；补充页面过渡、指标解释、实验设置和现有证据能够支持的结论。
- 第 35 页后续工作改为从已有文本和图像原型继续推进工程实现，包括统一工作描述、状态采集和算子接口，完善可配置执行框架，将代价估计接入数据库计划比较与多 SQL 调度，以及完善读取、执行、写回和质量检查并扩展视频、音频任务。
- 预期成果相应改为统一接口、可配置原型系统、可接入数据库优化与调度过程的轻量代价估计组件，以及文本与多模态实验系统、实验数据和论文；未写成已经完成。
- 仅第 35 页两栏可见文字发生变化，其余页面可见文字与图片保持不变；WPS 导出检查和备注结构检查通过。

## 2026-08-23 开题背景与动机总结讲稿修订

- 扩写开题 PPT 第 3 至第 8 页的简短讲稿和详细讲解，使背景按数据库 AI 调用、外部执行差异、现有研究分工、研究对象、三类问题和评价方法连续展开。
- 第 16 页将三类动机现象与两项研究内容明确对应：固定行数不能代表实际工作量，对应数据组织；容量上限不能代表当前状态、请求级处理不能保证作业完成，共同对应状态感知提交与多作业调度。
- 第 16 页清理中英文混写并调整三个问题块的大小和位置，使其不再互相覆盖；WPS 多轮导出复核无截断或拥挤。
- 36 页简短讲稿建议时间调整为 585 秒，即 9 分 45 秒。

## 2026-08-23 开题报告代价估计增强关系设计

- 明确两项研究内容各自具有不依赖代价预测的基础方法：数据组织直接使用工作特征，提交与多作业调度直接使用预先测得的容量、完成通知和实际运行状态。
- 将 AI 算子代价估计设计为两类用途：服务数据库执行计划与多 SQL 安排，并作为可选信息增强数据组织、作业选择和服务实例选择。
- 规定后续报告按“常见做法、现有不足、本课题研究、基础方法、代价信息增强、评价方式”组织相关段落；现有 429 次运行仍只作为离线代价估计的前期依据。
- 段落级修改设计记录在 `docs/superpowers/specs/2026-08-23-opening-report-cost-estimation-enhancement-design.md`；本轮尚未修改开题报告正文。
...
- 第一章的实现链路收缩为 PostgreSQL 数据库内算子与外部物理执行的总体关系，具体 LOTUS/Daft/Ray/vLLM 组件留到第四章；详细容量数字和工作量/状态/容量图也从第一章移到第 4.4 节。
- 三类动机重新表述为工作表示失配、静态容量与运行状态失配、请求级执行与 Job 完成目标失配。第一类对应研究内容一，后两类在同一提交决策点共同构成研究内容二；代价估计仍是共同使能组件。
- 第一章术语表只保留五项核心概念，提交批次、模型请求、服务内部批次和估计工作量在第三章首次需要时解释；第二章各小节减少对本课题具体方案的提前展开，完整差异集中到第 2.4 节。
- 报告图号同步为目标架构图 1、工作量/状态/容量证据图 2，后续图号不变；同步更新报告图片映射、QA 和项目索引。实验数据和图片文件未改变，PPT、Word、飞书及 Wiki 未修改。
...
## 2026-08-24 开题正文图中文字与图量收口

- 按用户要求撤下报告第 4.1 节的目标路径/当前外部路径架构状态图，目标和当前实现状态继续由文字与模块状态表说明；正文由 6 张图调整为 5 张数据图并统一顺延图号。
- 在现有工作量/状态动机图和多 Job 图上改写标题、panel 标题与注释，使其直接说明比较条件和观察结果；未增加新图，未修改实验数值、坐标、误差线或统计口径。
- 根目录、开题目录和图目录规则新增图中文字检查：总标题写“对象或条件 + 观察结果”，内部结构名或英文指标必须解释具体作用，抽象分类词不能脱离对象单独承担结论。
- 同步更新报告图片映射、QA、开题导航、项目索引和图件审计；架构图文件保留为历史候选资产，当前报告不引用。PPTX、Word、飞书与 Wiki 未修改。

## 2026-08-24 开题动机定位与进度安排精简
...
- 只读核对用户提供的桌面开题 PPT：文本固定行数与图像分阶段证据归入问题一，容量边界与单作业状态指纹归入问题二，原生四作业并发相对独立运行的 JCT 退化归入问题三；共享空闲容量是研究内容二的初步方法结果，不再充当问题三的动机证据。
- 报告第三章统一为“研究动机与问题分析—研究目标—研究内容”。问题二用服务侧状态回答何时提交及发往哪里，问题三用作业侧状态回答下一份工作来自哪个作业；二者发生在同一次上游调度决策，共同构成提交控制、服务实例路由和多作业调度。
- AI 算子代价估计明确为数据库计划比较的共同支撑，并可在独立验证后选择性增强两项研究内容；它不是第三项研究内容，也不是基础方法的前置依赖。第四章研究方案只分总体执行、两项内容实现、共同支撑与实验设计三个小节，可行性分析保留四个小节。
- 第一章“传统数据库算子与外部 AI 算子的执行假设”改用已有高层图，不再显示 `work/state/SLO` 方案字段；研究缺口图修正数据库侧文字边界；单作业状态图标题改为“吞吐、排队、缓存与利用率需要联合观察”。正文现有 16 张连续编号图片和 51 条参考文献。
- 进度安排为五个多月时间范围，覆盖约一年半；第二章以后无 Markdown 表格。报告 QA、图片映射、图件审计、项目索引和开题写作规则同步更新。源 PPTX、Word、飞书与 Wiki 未修改。

## 2026-08-24 开题综述与代价估计表述补充

- 第二章按新增精读结果补充 Palimpzest、Abacus 和 Sema：分别说明声明式语义分析与采样计划选择、面向 Pareto 前沿的代价驱动优化、DuckDB 原生语义算子与运行期自适应执行，并交代各自实验依据、适用条件及与本课题的衔接。
- Sema 以 arXiv:2603.11622v1 题录新增为参考文献 [52]；Abacus [21] 修正作者顺序。正文现为 52 条参考文献，已有编号保持不变。
- 综述中的论文名统一使用正式英文原题。代价估计改为正面说明“数据库计划比较的共同支撑 + 两项研究内容的可选增强”，并明确计划阶段估计、运行时状态与两项研究内容之间的信息关系。
- 对外写作规则新增正面角色陈述要求：内部分类争议不主动带入正文，否定式表述主要用于实验结论的证据范围、文献适用条件和必要概念辨析。报告 QA、开题导航和项目索引已同步；PPTX、Word、飞书与 Wiki 未修改。
...
Linux 上 126 项相关检查通过；私有 PG18.3 按 -O2 -Werror 构建通过。首次1×100诊断绑定836448ab：
压力与取消/恢复有效，瞬时断连缺少采样，未开始后续场景。原运行保留。
下一修订 phase-lifecycle-2 预登记独立握手等待夹具，驱动在同批观测两端后释放，仅用于故障/恢复，
最长5s；不改压力负载、生产协议或资源阈值，不把等待时间作为性能结果。
...
真实复查先因误用历史初版预算校验器在0请求停止；完整SHA绑定成功版本后，新增3请求到28/32。
...
## 2026-09-06：真实模型复验通过，准备推送修复分支

用户要求真实模型验证无问题后提交推送。按Map合同§8.4.6，用新8次持久预算验证
`b7eeea53`的当前共享观测：一次PG预热，SELECT两次、INSERT一次、取消/拒绝及恢复各一次。
六个测量阶段全部valid/passed，INSERT由独立连接在cleanup后审计，模型调用/接受/输出各1，
prompt/output tokens为42/7，两个角色结束FD/线程增量均0。原32/32账本SHA不变。
第一次启动因PATH缺serving venv bin、找不到已安装的ninja在0请求失败，原脚本/日志保留；
在新目录修正启动环境后，沿同一空账本完成8/8，没有推理重试。4项无模型控制器反例在本地
允许ps的环境和Linux均通过，PG无模型EXPLAIN预检通过。1162项raw哈希全部匹配，PG/gateway/
模型均退出，端口关闭，两GPU恢复1MiB。完整结果见
...
清理提交 `fcd12373`：本地 233 项中 231 通过、2 项 Linux 专属跳过；Linux 233/233 通过，
