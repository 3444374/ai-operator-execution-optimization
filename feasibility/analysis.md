...
- `feasibility/results/`：组件级 benchmark、环境验证、PG18.4 连接验证、dry-run 和 smoke CSV。
- `motivation/results/`：数据库 AI 算子端到端动机测试、PG18.4 系统画像、瓶颈定位和可优化点分析。

因此，PG18.4 当前分成两类证据：

1. 连接与环境可用性：见 `feasibility/results/pg18_4_connection_validation.md`。
2. 系统画像与瓶颈定位：见 `motivation/results/pg18_4_fake/system_profile.md`。
...
> 先把 Ray/Daft/Lance 类外部执行链路当作候选系统和调优对象，补齐 Python、Ray task、Ray actor、不同 batch/actor 配置、pgvector 写回、真实 CPU 小模型或轻量模型服务等 baseline，再判断收益来源和优化空间。GPU / vLLM 可以作为外部模型服务的现实计算端点进入后续少量边界对照，但不把“算子迁移到 GPU”本身作为主工作量。
...
当前证据支持“粒度和 fan-in 敏感”这一局部判断；还不能直接证明 CPU/GPU-backed model-service 资源配比、actor routing、Daft shuffle 或 Lance 写入一定是主瓶颈。GPU 相关结论需要少量边界 baseline 单独验证。
...
| GPU 过早接入 | 无法判断 GPU 收益来自硬件还是链路调优 | 先完成外部链路 baseline、CPU/fake model baseline 和真实 CPU embedding；GPU 只在最后做少量边界对照 |
...
4. pgvector `vector(128)` 写回路径，对照 fake 写回、真实向量写入、主控 fan-in 后写回和多 worker 直接写回成本。
5. 真实 CPU embedding 小模型或轻量模型服务，对照 fake model 与真实前后处理/推理成本。
6. GPU / Ray Serve / vLLM 形态只做少量边界 baseline，对照模型服务路由、队列、backpressure 和硬件加速收益，不作为主要调优线。

每组实验都需要输出 CSV、运行命令、参数表、指标解释和结论边界。正式瓶颈定位和可优化点分析写入 `motivation/results/`；只证明环境和组件可用的结果写入 `feasibility/results/`。
