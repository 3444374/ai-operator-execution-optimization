...
> 判断该瓶颈是否只是 object/fan-in 问题，还是会进一步扩展为 task/actor 并行度、外部 worker 形态、CPU/GPU-backed model-service 流水线、模型服务队列、writeback 和 backpressure 问题。
...
- 不做传统 GPU 查询算子；
- 不做模型 kernel 优化；
- 不把主要时间投入“AI 算子搬到 GPU / 数据库内执行”的 baseline；
...
- Python batched worker vs Ray task vs Ray actor 外部链路 baseline；
- 主控进程 fan-in 后批量写回 vs 多 worker 各自写回；
...
| 多 worker 各自写回更慢 | 需要控制数据库连接并发、事务大小、失败重试 |
| 主控 fan-in 写回更慢 | 需要减少集中 fan-in、尝试分区写回或队列式 worker |
