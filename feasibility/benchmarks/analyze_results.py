...
def parse_args():
    ...
    parser.add_argument("--results-dir", default="feasibility/results")
    parser.add_argument("--output", default="feasibility/results/feasibility_report.md")
...
def choose_direction(analyses: dict) -> tuple[str, list[str]]:
    ...
    if fake_ai_embed_score >= 2:
        reasons.append("fake AI_EMBED(text) 端到端实验已显示 coalescing 对整体链路有收益，object/fan-in 可作为 组件可行性入口证据；下一步应拆分收益来源，并验证 task/actor/concurrency、模型服务队列和 backpressure 是否形成更高层瓶颈。")
