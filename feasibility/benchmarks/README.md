...
```bash
python feasibility/benchmarks/ray_small_task_benchmark.py \
  --tasks 100 1000 10000 \
  --repeats 3 \
  --output feasibility/results/ray_small_task.csv
```

```bash
python feasibility/benchmarks/ray_object_transfer_benchmark.py \
  --sizes-kb 1 10 100 1024 10240 \
  --types bytes numpy arrow \
  --repeats 3 \
  --output feasibility/results/ray_object_transfer.csv
```

```bash
python feasibility/benchmarks/arrow_recordbatch_serialization_benchmark.py \
  --rows 1000 10000 100000 \
  --cols 4 16 \
  --repeats 3 \
  --output feasibility/results/arrow_serialization.csv
```

```bash
python feasibility/benchmarks/shuffle_simulation_benchmark.py \
  --upstream 8 32 128 \
  --downstream 8 32 \
  --rows-per-partition 10000 \
  --strategies fine coalesced \
  --repeats 3 \
  --output feasibility/results/shuffle_simulation.csv
```
