...
| 2026-08-14 | `saor-v0.5.7-fail-closed-evidence` | 修正 avoidable-idle 的事件域 `hold_start`；matched-ready cell pass 强制 completion fairness evidence；runtime Job ID 必须非空/单 Job 一致/跨 Job 唯一；selector/bridge readiness 冻结 effective K/W 与 weights | 代码审核反例 + 211 项 SAOR/shared-credit/shared-vLLM 回归 + 旧服务器完整 artifact 生命周期复核 | 旧全矩阵 `validation=passed` 降为 diagnostic：五个 bounded-ready 臂各 Job 512/512 lifecycle，frozen-static 0/512，故性能/SLO 和 bounded-ready 臂内经验 lag 保留，跨 static 公平比较不可用。formal 继续锁定；新服务器 regression 通过前不运行正式矩阵 |
