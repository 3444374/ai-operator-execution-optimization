...
| 数据库产品原生 | OceanBase SQL `AI_COMPLETE` | 已有数据库 AI 算子在同 endpoint 下的 E2E | 是，但当前容器部署门禁阻塞 |
...
4. **数据库产品能力**：OceanBase `AI_COMPLETE` 能否在合适 VM/容器中使用同一 vLLM
   endpoint 完成相同 SQL 语义？若不能，只报告 capability/blocker，不伪造替代实现。
...
所有 arm 必须是一行对应一次 Chat 请求、相同 messages、temperature=0、输出上限、
模型、chat template、endpoint 和 immutable manifest。
