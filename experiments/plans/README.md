...
| `state_aware_work_unit_evaluation_20260808.md` | **开题最小方法证据 + 论文主实验总合同 + SAOR 唯一维护入口（§5.2）** | fixed-envelope 2-Job formal 已 40/40，resolution-aware v2 完整 validation passed。SAOR 未越过 static，为 `formal-valid/not-promoted`；strict-priority 两轮短测达到 fg P99 14.27s/SLO 0%，但仅是上界。`saor-v0.5.1` 的显式 priority/SLO budget + actual-work debt cap + 队首定向 reclaim barrier 已完成本地实现、无损事件账本、四臂 readiness 和两轮汇总器；服务器关机期间未跑 0.125K/0.25K GPU rehearsal，仍为 `development-unrun/not-formal-registered`；reservation 作通过后的鲁棒性消融 |
