...
| `state_aware_work_unit_evaluation_20260808.md` | **开题最小方法证据 + 论文主实验总合同 + SAOR 唯一维护入口（§5.2）** | fixed-envelope 2-Job formal 已 40/40，resolution-aware v2 完整 validation passed。SAOR 未越过 static，为 `formal-valid/not-promoted`；strict-priority 两轮短测达到 fg P99 14.27s/SLO 0%，但仅是上界。dynamic K parked；下一候选为有界 priority/lag guard，reservation 作鲁棒性消融 |
