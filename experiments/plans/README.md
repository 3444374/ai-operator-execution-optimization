...
| `state_aware_work_unit_evaluation_20260808.md` | **开题最小方法证据 + 论文主实验总合同 + SAOR 唯一维护入口（§5.2）** | fixed-envelope 2-Job formal 已 40/40 完成但总 gate fail-closed；SAOR 在 credit 臂内 fg 最好、未越过 static，当前为 `formal-evaluated/directional-only`。dynamic K parked；下一候选是经 strict-priority 可达性和 static 非劣门约束的 reservation-backed release；§5.2 记录数学/估计缺口和图像 HSE 两级 broker |
