...
增量核心表征与 fixture 测试，公司接口可只读核对。四 C 的值、PG plan 和 gateway v4 已实现，
C port/wire 执行仍待接入；独立核心与公司接口工作仍待实施，真实 PG 接入另行验收。
...
| [`postgresql_choice_profile_engineering.md`](postgresql_choice_profile_engineering.md) | 四 C 进行中：值、PG plan 和 gateway v4 已实现，C port/wire 待接入；字段、错误、兼容性、预算和资源验收的唯一详细入口 |
