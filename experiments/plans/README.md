...
| `state_aware_work_unit_evaluation_20260808.md` | **开题最小方法证据 + 论文主实验总合同 + SAOR 唯一维护入口（§5.2）** | 原生单/多 Job 证据已完成；`saor-v0.3` 将 fixed-envelope ordered release 定为主候选、capacity-only 降为 optional governor；后者真实四臂 development gate 未晋级，前者 fairness debt 尚未接 formal runner；§5.2 记录数学/估计缺口、独立 oracle 门和图像 HSE 两级 broker |
