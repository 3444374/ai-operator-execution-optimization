...
| `state_aware_work_unit_evaluation_20260808.md` | **开题最小方法证据 + 论文主实验总合同 + SAOR 唯一维护入口（§5.2）** | fixed-envelope 2-Job formal 已 40/40；原始完整 gate 保留 fail-closed，resolution-aware compact mechanism replay effective 12/12。SAOR 未越过 static，仍为 `formal-evaluated/directional-only`；strict-priority release-only 诊断已实现、GPU pending。dynamic K parked；下一候选仍由可达性和 static 非劣门约束 |
