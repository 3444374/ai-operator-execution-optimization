...
| `state_aware_work_unit_evaluation_20260808.md` | **开题最小方法证据 + 论文主实验总合同 + SAOR 唯一维护入口（§5.2）** | 原生单/多 Job 证据已完成；`saor-v0.4.1-runtime` 固定总 K，只动态调 active-set 份额/借用/回收/release order，dynamic K 已 parked；fixed-envelope runtime/direct control 已接但无 GPU formal，capacity-only 四臂未晋级，下一项必须先过 global FIFO/DRR killer baseline；§5.2 记录数学/估计缺口和图像 HSE 两级 broker |
