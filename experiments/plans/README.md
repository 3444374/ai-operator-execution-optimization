...
| 生成型 Map 的具体行为、数据表示与验收 | [`postgresql_semmap_generation_contract.md`](postgresql_semmap_generation_contract.md)：消息、纯值与 Python v5 已合入 main；独立分支 PG plan/权限已验证但不可执行，C v5、完整 golden/模型/资源检查按 §8 继续 |
...
| [`postgresql_semmap_generation_contract.md`](postgresql_semmap_generation_contract.md) | 四 D 定稿；消息、纯值/Python v5 已集成，独立 PG plan/权限分支已验证；C v5/完整 golden、资源与模型仍待验证 |
