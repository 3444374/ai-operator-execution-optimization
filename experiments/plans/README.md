...
| `state_aware_work_unit_evaluation_20260808.md` | **开题最小方法证据 + 论文主实验总合同** | 原生单/两 Job 与项目 short/long 两臂已完成；1-short+3-matched-long 四 Job 因果轨保留；公开 benchmark 泛化轨冻结为 VTC `on_off_overload`/`overload-multi` + BurstGPT v2.0 trace，明确 workload/指标兼容不等于复现服务内 VTC |
