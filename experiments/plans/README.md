...
| `state_aware_work_unit_evaluation_20260808.md` | **开题最小方法证据 + 论文主实验总合同 + SAOR 动态算法唯一维护入口（§5.2）** | 原生单/两 Job 与项目 short/long 两臂已完成；SAOR 已到 `trace-validated`，但 capacity-only 真实四臂 development gate 未晋级，ordered release/fairness debt 尚未接 formal runner；§5.2 已记录失败机理、估计误差证明缺口和图像 prepare→ready-tensor→model 两级 broker |
