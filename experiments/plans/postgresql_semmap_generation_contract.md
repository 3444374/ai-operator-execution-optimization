...
- `server.main` 显式接受 argv、adapter/session 包装函数；Map/Filter 观测入口通过该接口组合，
  ...
  当时保留旧 fault CLI 兼容转发；后续按用户的退役清理要求在§8.4.7删除。
...
### 8.4.7 合并前退役代码清理（2026-09-06，已验证）

用户要求清理过时、不会再使用的处理与实验观测代码。固定起点为已推送的
`d93e3f9b58b4ecfedd46b32754d69c82b4ed3dc6`；在修复分支实施，不自动合并main。
本次明确允许撤销退役的Python import/CLI别名，保留现役SQL、wire v2/v3/v4/v5和错误语义。

先完成实际调用方迁移，再删除旧extension gateway入口、v3 Python类型别名、golden session
转发函数、旧fault CLI和choice预算默认子类。默认预算数据仍由choice实验入口拥有，通用账本
继续要求明确预算。旧测试改为直接验证当前Interface；仅用于证明退役实现仍有旧错误的测试
不再作为现役回归保留，其必要反例由当前采集/判定测试覆盖。

退役的SemMap资源采集与单次真实运行脚本，先检查全部引用和唯一内容；源码由上述已推送Git
提交及逐文件SHA恢复，不再作为工作树中的可执行副本。原始采样、失败结果和历史报告保留，
本地链接改指向固定Git源码，当前公共哈希清单与退役索引分别说明可用文件和历史源码。
仍用于当前Filter质量/校准、现役协议测试或其他研究路径的代码不因日期较旧而删除。

验证包括引用清理、Python协议/资源/预算回归，以及四个PG TAP文件迁往公共gateway入口后的
Linux PG18.3验证。此次不调用模型；既有真实8次复验证据保持原提交身份，不被清理提交冒领。

实际清理提交 `fcd12373`：19 个退役文件已删除并核对 Git/blob/SHA；本地 233 项中 231 通过、
2 项 Linux 专属跳过，Linux 233/233 通过；PG18.3 `-O2 -Werror`、四个受影响 TAP 1297/1297
...
### 8.4.8 两算子完整回归与 main 合并（2026-09-06，验证通过）
...
`5771cef1` 实际通过Linux247/247、PG18.3严格构建、regression1/1和全部7个TAP1758/1758；
新9次真实请求全部完成，六个SELECT/INSERT场景通过，6会话关闭、9任务完成、服务全部退出。
旧8/8账本原字节不变。没有生产修复或推理重试。详见[两算子合并验证](../results/postgresql/semmap_resource_lifecycle_20260906/README.md#main-integration)。
