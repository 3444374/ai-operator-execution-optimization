...
> （`code/scripts/baselines/squad_database_e2e_runner.py`，DuckDB-ai 臂 + `direct_client` 臂）：一个计时墙包住持久表扫描 →
...
> 分段；runner 层算 `correct_rows_per_s`（主 headline）、`successful_rows_per_s`、failure rate，并对所有臂统一
> 报 `truncation_count`/`truncation_rate`；状态字段解耦为 `single_run_valid` / `formal_run_gate_passed`（单次
> runner 恒 false）/ `comparison_admission`（`pending_formal_repeat`）。DuckDB 扩展继续拥有
> batching/concurrency，runner 不注入项目 credit/actor pool/动态 backpressure。`project_static`
...
2. 三臂同 manifest、同 model、双 GPU、vLLM 同配置、prefix cache、**同 cap=64**、同计时边界，按 §2 五类指标
   + §3 两边界 + §4 请求等价门禁执行。
3. 项目最终优化方案确定后补第四臂。
4. **database-E2E 顶层 runner（已实现，DuckDB-ai + direct_client 臂；§3）**：`code/scripts/baselines/squad_database_e2e_runner.py`
   已覆盖 scan→construct→operator→unified sink 的 E2E 计时墙与 runner 层指标；`project_static`
