...
上述两轮后已完成下方常驻服务对照；下一步准备新的真实输入和模型额度，M2尚未开始。
...
### 真实输入与模型运行准备（已准备，真实调用暂缓）

[准备结果](../results/postgresql/waiting_positions_real_preparation_20260914/README.md)：512/1024行真实输入与模型身份已核对；
完整/摘要模式128次受控HTTP通过，真实模型请求仍为0。

用户于2026-09-14明确要求暂不启动真实模型；已准备的数据、测量开关和运行表保留，恢复运行需新的明确指示。具体真实调用总量
在启动前单独列明，不沿用此前已结束的额度。受控检查不因本节存在而变成真实模型结果。

- 数据：沿用已下载的Clapper Movie Reviews v4原始CSV，构造新的Movie-derived Map情感分类查询。
  排除旧SemBench 2000行涉及的全部电影、review ID及相同文本；按电影哈希分到调参与评价两组，
  再按评论哈希选512/1024行，不按标签或模型预测筛选。去掉重复review ID/完全相同文本的后续出现，
  保留原始文本、标签、源位置和排除统计。此工作负载不称为原始SemBench程序。
- 模型候选：已有Qwen2.5-7B-Instruct固定revision，单RTX4090、vLLM0.25.1；文件哈希、
  tokenizer、生成设置和服务参数均在机器上核对。最大输出128、temperature=0，沿用现有PG Map分类语义。
- 测量：full记录完整gateway事件与PG逐行阶段；compact保留实际请求摘要、行关联、输出摘要和资源，
  关闭PG逐行阶段追踪。compact不能重建组织候选/拒绝原因，必须明确列为缺项；匹配full查询承担这部分检查。
  两种模式都保留PG独立行绑定、结果原文/行数/哈希、HTTP计数和最终资源核验，不能称完全无观测。
- 拟定真实请求表：调参C4/8/16/32/64各1次预热+2次512行，共7680次；按调参调用至EOF
  中位数选达到已测最高完成吞吐97%的最小C。随后3种控制×2种观测×(1次预热+5次1024行)，
  共36864次。全部上限44544次真实POST，每查询90秒、含启动与清理总计30分钟，不重试或重置额度。
- 三种控制：请求数FIFO、同C同序且W不生效的token FIFO、同C同序且紧W的token FIFO。
  L128、Core输入/结果各32MiB、PG total32MiB/暂存4MiB固定。宽W为评价输入最高64项work之和；
  紧W4096遵循现有组织配置的最小值，逐条能放入服务context4096。若选定C项的最大work之和仍≤4096，
  则停止后续对照并报告无法触发该限制；实际未出现work-only拒绝时也报告未触发，不能调W补跑。
- 短查询的结果只说明该规模；未达到60秒稳态或最高C仍明显改善时，不称服务容量已校准完成。
  输出合法性、标签质量、预测差异和生成token分别报告；输出差异不删行，非法结果或资源/关联错误立即停止。
- 服务prefix caching关闭且全程一致；运行次序预先登记，预热独立报告。每组复用一个gateway；
  累积测量不是独立服务启动样本。结果报告保留全部单次值，完整JCT优先，HTTP尾延迟只解释等待位置。
...
E已按执行/资源范围完成，M1的调用起点与等待分解已实现；常驻服务成本和本fixture的C4/C8重复已验证，真实输入划分与独立模型校准仍待完成。
...
### 4. 四项设计假设实验（M1已做受控小实验）

M1—M4只是内部实验标识。正式报告用下列中文问题名；每项都保留负结果，不能通过反复改输入或阈值强求支持。

| 实验 | 对照与固定条件 | 必需观察与主要判定 | 最小实现缺口 / 下一步 |
|---|---|---|---|
| M1：控制是否改善查询，还是移动等待 | 相同输入、模型/生成、物化预算、输出序与服务配置；请求数FIFO独立调优、同C的token表征FIFO且W不生效、同排序且W生效；计算限制的可生效性先由输入work上界证明 | 查询专属准备至EOF的完整JCT；资格→提交及所有可测完成→消费区间；HTTP P99只作解释。若提交后P99下降、上游等待增加而JCT/其他查询无改善，则只有等待位置改变 | 不可变全扫描资格、调用起点与五段计时已通过672次受控检查；常驻追加768次已完成，C8五次快于C4；下一步真实输入划分与模型额度 |
| M2：局部候选是否比全局信息更划算 | 流式FIFO、同预算有限候选组织、全局ID/work元数据规划+有界payload执行；全局memory不足时合理外排，不能用故意OOM的无限提交作强参照 | 共同信息条件先比较selector，再比较付费获取全局信息的完整方案；计入扫描/统计/token化/排序/额外读取、metadata峰值和持久存储；固定快照与结果语义 | 全局元数据路线尚未实现；先做实验层适配，不改生产账本。首次构建和可复用metadata分列，不免费借用已经准备好的完整评价信息 |
...
| M4：过早提交是否损害后到查询 | 固定两查询到达与资源；长查询先开始、短查询按预定延迟到达；改变已承诺工作量，同信息比较共享FIFO和已有公平控制；matched solo另跑 | 每查询JCT、相对同服务资源/保留份额solo的slowdown、最长无服务时间、不可抢占已提交work及长查询代价；不能只报短查询改善 | E共享服务接入已具备；仅在M1或M2暴露查询竞争问题后增加先到/后到对照。既有shared-credit/公平算法先核对适配与默认装配，不重新实现 |
...
2. **下一步仅准备M1/M2**：核对现有时点与资格定义，形成输入划分、查询专属准备计时、全局元数据适配的最小差异和独立调优单元表。
