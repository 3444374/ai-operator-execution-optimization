...
## 当前实施：完整工作包E——依赖与共享服务（2026-09-14）
...
其中1条真实结果用于预期超时检查，不计成功查询。下面四项设计假设实验尚未运行，本文不新增模型额度。
当前只完成研究规格；用户随后选择先完成顶部工作包E，再准备M1/M2的观测、输入划分和独立调优方案。
...
|---|---|---|---|
| M1：控制是否改善查询，还是移动等待 | 相同输入、模型/生成、物化预算、输出序与服务配置；请求数FIFO独立调优、同C的token表征FIFO且W不生效、同排序且W生效；计算限制的可生效性先由输入work上界证明 | 查询专属准备至EOF的完整JCT；资格→提交及所有可测完成→消费区间；HTTP P99只作解释。若提交后P99下降、上游等待增加而JCT/其他查询无改善，则只有等待位置改变 | 复用已有事件；补足不受策略影响的资格定义及查询准备外层计时。先做无新模型请求的观测完整性检查，实际M1另列获批运行单元 |
...
| M4：过早提交是否损害后到查询 | 固定两查询到达与资源；长查询先开始、短查询按预定延迟到达；改变已承诺工作量，同信息比较共享FIFO和已有公平控制；matched solo另跑 | 每查询JCT、相对同服务资源/保留份额solo的slowdown、最长无服务时间、不可抢占已提交work及长查询代价；不能只报短查询改善 | 仅在M1或M2暴露查询竞争问题后补E最小共享服务接入；既有shared-credit/公平算法先核对适配与默认装配，不重新实现 |
