...
下一步仍是M1的常驻服务对照与C4/C8稳定性检查，之后再准备真实输入和模型额度；M2尚未开始。
...
E已按执行/资源范围完成，M1的调用起点与等待分解已实现；真实输入划分、稳定调优与常驻服务成本仍待验证。
...
|---|---|---|---|
| M1：控制是否改善查询，还是移动等待 | 相同输入、模型/生成、物化预算、输出序与服务配置；请求数FIFO独立调优、同C的token表征FIFO且W不生效、同排序且W生效；计算限制的可生效性先由输入work上界证明 | 查询专属准备至EOF的完整JCT；资格→提交及所有可测完成→消费区间；HTTP P99只作解释。若提交后P99下降、上游等待增加而JCT/其他查询无改善，则只有等待位置改变 | 不可变全扫描资格、调用起点与五段计时已通过672次受控检查；下一步常驻服务/C4-C8稳定性，再准备真实输入划分与模型额度 |
