...
方法续体保存的bytes及阶段数有界；[多行驱动V1](bounded_method_driver.md)以共享池固定配额限制
活跃行数、方法payload预留与最终结果占用。它不是RSS测量或动态Job预算策略。
...
在9f555a00版本的源码审查中，SessionPolicies仍构造row_count=1、空prefix_key的BatchRequest，
...
<a id="organized-window"></a>

## 15. 有界组织窗口与单成员物理提交接入（工程决策）

共同接口先区分行/调用/阶段身份、组织批次及物理请求。保留§8.7/§8.8的语义与carrier分工；
公开资料沿用knowledge_hub的Sema/LOTUS记录，本切片不复制其实现。具体复用当前
organization/service_quantum.py:slice_service_quanta与planning/work.py:WorkDescriptor：前者按work
切完整成员，后者提供带单位/校准身份的工作量与局部性。批次策略只看已经offer接受的有限窗口，
不借用会拉取/等待输入的ArrivalReplayBatcher，不创建独立缓存或第二套提交循环。

当前backend合同每次仅接收一个完整任务，因此组织批次明确展开成多个单成员物理请求；请求数按
实际展开计数，不能把组织分组误称为单HTTP批量。批次成员索引与TaskKey传递到backend和Delivery，
SQL行身份独立保存。一个未完成的组织计划可跨advance/背压保留，不重新组织已归属的成员；
无描述的旧调用保持FIFO路径。暂不实现一个请求返回多个成员结果，后续须增加独立终态/取消协议。

先用独立有限生产器和受控backend验证：同局部性/能力的工作量分组、部分接纳、多个请求在途、
逆序结果、成员失败、取消与迟到终态、拒绝非法成员计划，以及释放后恢复容量。生产代码的注释
说明身份与所有权，不承载研究来源。PG窗口扩大仍单独核对提前求值、LIMIT和错误时机。

用户要求真实模型测试：新增独立4次POST预算、两条合成输入各draft→revise两阶段；每一阶段组成
两成员组织批次，展开为两个同时在途的HTTP请求。固定2任务/2请求/2诊断work单位、输入16KiB、
每项输入8KiB/响应64KiB/metadata1KiB，状态64bytes、每行最多两阶段，每次最多64输出tokens、
temperature=0。work为诊断请求计数，不作为模型成本校准；同局部性只是显式合成标签，不宣称缓存收益。
沿用已核验7B资产和独立控制器，重新核对模型文件与源码、core/text preflight；不下载、不启动PG。
核对批次成员/行身份、实际并发2、真实阶段输入、未释放结果时背压、四次请求预算、账本归零和服务清理。
无额外warm-up生成、HTTP重试或性能结论；失败保留记录，停止并诊断，不扩大请求预算。

首轮组织路径4次真实请求通过后，复查新增重排发现原has_immediate_work只判断输入头部容量。
新增受控用例复现：头部work=3、后续work=1/1、容量3、单步预算1，派发第一项小任务后仍可派发
下一小任务，却错误报告无即时进展。保留修复前失败测试，改为检查就绪窗口中可放入容量的任务；
选中任务若被策略/额度阻止，仍使用原retry_at限制重试。生产代码变化后重跑受影响回归，并为最终
源码追加独立4次真实确认预算（两轮合计上限8次）；旧四次账本保留，不把两轮混作一次或隐藏重跑。


最终结果：本地54项、Linux335项回归通过，624份源码哈希一致；两轮各4次真实请求通过，
