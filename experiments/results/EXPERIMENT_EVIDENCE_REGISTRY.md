...
本文是正式方法实验的统一入口，回答三个问题：机制是否已经实现、是否只通过了功能测试、是否已有真实 GPU 性能证据。具体数字和逐次运行证据仍以各结果目录的 `README.md`、`manifest.json` 和 CSV 为准。当前工程主线已在 `REL_18_3` extension planner-visible `SemMap`、协议 v2 canonical digest 和同步单在途 UDS recording slice 上完成 PostgreSQL-private scan/pump、neutral provider port 与 recording/UDS adapter 的职责拆分和响应边界 hardening；下一步先固定 extension 级 PostgreSQL compatibility suite，再以 exact `SemFilter` 作为第二个真实消费者验证共同 lifecycle 代码，并在两个 reference path 通过后抽成 PostgreSQL-private runtime，之后增加最小第二 semantic path。accepted-prefix、多在途、增量 SemLoom session 与 IMLane-like batch placement 在数据库语义资格后推进，Kalypso-like lineage 只作后续参考。既有文本、图像和 SAOR 条目记录外部物理执行基座与历史证据，不得重标为已实现数据库内算子。执行顺序以 `experiments/plans/experiment_status_and_gaps.md` 顶部摘要为准。
...
|---|---|---|---|
| PostgreSQL planner-visible `SemMap` | `code/postgres/semloom_pg/`；9 项静态 contract、7 项 Python protocol、PGXS SQL regression 与 TAP lifecycle/fault tests | 双 RTX 4090 服务器隔离源码构建 `REL_18_3`（upstream `62d6c7d3…`）；提交 `0b9948ee` 以 `-Werror` 无警告 build、regression 1/1、TAP 150/150、Python/static 16/16 通过 | 功能测试：deterministic carrier 已验证 EXPLAIN/ordinary child、filter/projection、LIMIT/error recovery、preload、prepared statement、snapshot、cancel/recovery、direct `INSERT ... SELECT` rollback/commit；plain EXPLAIN、LIMIT 0、zero-row child 与 NULL-only 执行不连接 provider，`PROPAGATE_NULL` 由 PostgreSQL 完成。in-process 与 UDS adapters 只比较 SQL rows（含 NULL）和归一化 EXPLAIN；UDS-only fault tests 覆盖 Unicode、断连、坏 digest、response/connect wait 取消、恢复、非 UTF8、整数溢出、escaped/raw NUL、fractional integer 和清理。三个新增输入均返回精确脱敏 `08P01`，TAP 拒绝 task payload 出现在 stderr；static contract 另行验证 `CopyErrorData()` 前恢复稳定 context。尚无模型/GPU 性能证据，rescan/EPQ/parallel 和更宽 query shape 未支持。 |
| PostgreSQL-private pump + neutral provider seam | `code/postgres/semloom_pg/src/{ai_provider_port,sem_pump,provider,recording_provider,uds_provider,wire_v2}.{h,c}`、`gateway/` 与上述 contract/TAP | 精确 18.3 产物 ID `tap-run-0b9948ee` 中 `semloom_pg.so` SHA-256 为 `db6b7b7ba3bf4b9216251212957919490b62300434f34b99f88ec5576d141125`。前序提交 `d08eda38` 的仓库外 `resource-run-d08eda38/resource-smoke.txt` 对 2,000×100,000-byte task 在 warm relation VFD 后采样：RSS 起始/峰值/结束为 17,120/19,532/18,852 KiB（峰值/结束增量 2,412/1,732 KiB，预设上限 16,384/8,192 KiB），FD 为 10/12/10（增量 2/0，预设上限 4/2）；客户端、行数统计和 gateway 均为 0 退出，结果 2,000 行。本轮未重跑该资源 smoke | `ai_provider_port.h` 只含固定宽度值、byte slice、opaque session/provider 和 caller-owned error，不含 `Datum/Oid/MemoryContext` 等 PostgreSQL 类型；UDS input limit 由 adapter 通过固定宽度 `limit_bytes` 返回，pump 不再知道 wire 常量。`sem_pump.c` 当前拥有 child pull、next sequence、completion copy 与 terminal error mapping；provider/config 在 begin 固定，首个非 NULL task 才 open，query-context callback 与正常 close 共用无阻塞幂等本地清理。SQL-visible semantic spec、physical algorithm、provider execution profile 使用独立 digest，物理 mapped-column 不进入 wire identity。资源 smoke 只说明前序路径未观察到随累计 payload 近似线性增长或 FD 泄漏，不是新提交的性能或资源证明；accepted-prefix、多在途/乱序 completion、direct HTTP 与 SemLoom adapter 尚未实现。 |
...
1. 固定 extension 级 PostgreSQL compatibility suite，覆盖普通 SQL 非干扰、RLS/权限、snapshot/事务/savepoint、prepared/generic plan 与 invalidation、planner-hook coexistence、多 backend 隔离、cancel/ERROR/资源清理和无任务不连接。
2. 以 exact `SemFilter` 作为第二个真实消费者验证共同 lifecycle 代码，两个 reference path 通过后才抽成 PostgreSQL-private runtime；未来的独立 `FilterMachine` 验证三值/NULL/error policy、cardinality、tuple identity、LIMIT 与 transaction lifecycle，不复制当前 pump。
...
7. 代价估计仍需独立时间段或新 workload 校准、预测区间和在线决策增量；现有 429-run 结果只支持 marginal feasibility。
8. 文本 Shared-vLLM held-out、weighted/SLO、UCB reward 归因、多 endpoint 故障迁移和剩余 prefix 隔离统一为 `parked-conditional`，不阻塞当前主线。
