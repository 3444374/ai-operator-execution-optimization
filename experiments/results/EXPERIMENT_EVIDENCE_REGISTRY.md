...
reference 及其独立、未校准的 cost/cardinality metadata、同步单在途 UDS、shared runtime、neutral provider port、
...
|---|---|---|---|
| PostgreSQL planner-visible `SemMap` / exact `SemFilter` | `code/postgres/semloom_pg/`、`code/src/execution_provider/`；19 项 PostgreSQL static contract、8 项冻结 v2 protocol、9 项 v3 semantic protocol、8 项 fixed-model adapter、5 项 gateway migration contract、PGXS SQL regression 与 TAP lifecycle/fault tests | 双 RTX 4090 服务器隔离源码构建 `REL_18_3`（upstream `62d6c7d3…`）；历史 4A/4A.1 与更早结果继续绑定各自提交。4B `53cf3da8` 保留真实模型 capability；4B.1 `ef314618` 与 cost-observability `47407751` 证据继续绑定原提交。边界修正 `71a8ef7d` 通过 warning-free `-Werror`、regression 1/1、TAP 415/415、Python/static+migration 49/49 和 neutral/machine C11 compile；`postgresql_semfilter_gap_hardening_71a8ef7d_20260901` 保存源码哈希、原始日志、actual/expected、二进制与已校验 manifest | 功能测试：recording Map/Filter compatibility 与三参数 exact Filter 保持 ordinary child、三值/NULL/cardinality、`LIMIT` 前 placement、prepared/generic plan、RLS/权限、snapshot/savepoint、cancel/recovery、多 backend 与 direct `INSERT ... SELECT`。fixed adapter 拒绝常见 3xx、显式端口 0，且同一 adapter 至多一个 DNS resolver attempt；系统 resolver 不能取消，但调用按 deadline 返回。plain `EXPLAIN` 分列 uncalibrated rows/work estimate，`EXPLAIN ANALYZE` 分列实际 calls/usage；calibration 明确为 unavailable，不能用于第二 path 比较。真实模型 capability 不证明自然语言质量、性能或泛化。第二 path、rescan/EPQ/parallel、join/aggregate 和更宽 query shape 未支持。 |
| PostgreSQL-private plan/runtime + neutral provider seam | `code/postgres/semloom_pg/src/{sem_plan_spec,sem_filter_cost,semantic_filter_contract,recording_contract,pg_semantic_runtime,sem_pump,sem_operator_machine,sem_map_machine,sem_filter_machine,ai_provider_port,provider,recording_provider,uds_provider,wire_common,wire_v2,wire_v3}.{h,c}`；`code/src/execution_provider/{wire,adapters,server.py}`；旧 `gateway/` compatibility wrappers 与上述 contract/TAP | 历史 resource、4A/4B、4B.1 与 cost-observability 证据继续绑定原提交；`71a8ef7d` 追加 endpoint/resolver/cost-claim 边界复验，不替换资源 smoke | schema v1/v2、pump binding 和 shared runtime 保持；uncalibrated cost metadata 与 semantic digest 分离，runtime 计数实际 model calls 和 prompt/output usage。`wire_common.c` 拥有 framing/socket wait/JSON primitive，v2/v3 各自拥有版本 schema/digest/error。gateway 的 golden/fixed-model adapters 共享 `V3SessionRunner`；fixed adapter 不跟随重定向、拒绝端口 0，连续 DNS timeout 共享一个 in-flight attempt。endpoint/model/timeout/auth 只来自仓库外配置。matched reference calibration、accepted-prefix、多在途/乱序 completion 与增量 SemLoom adapter 尚未实现。 |
