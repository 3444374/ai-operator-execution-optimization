...
|---|---|---|---|
| PostgreSQL planner-visible `SemMap` / exact `SemFilter` | `code/postgres/semloom_pg/`、`code/src/execution_provider/`；17 项 PostgreSQL static contract、8 项冻结 v2 protocol、8 项 v3 semantic protocol、5 项 gateway migration contract、PGXS SQL regression 与 TAP lifecycle/fault tests | 双 RTX 4090 服务器隔离源码构建 `REL_18_3`（upstream `62d6c7d3…`）；历史 4A 与更早结果继续绑定各自提交。4A.1 `359ffdf3` 以 warning-free `-Werror`、regression 1/1、TAP 320/320、protocol/static 33/33、gateway migration 5/5 和 neutral/machine C11 compile 通过；仓库外证据包 `postgresql_semfilter_4a1_hardening_359ffdf3_20260831` 保留最终日志、同 hash regression actual/expected、完整 commit identity、使用显式 PostgreSQL 18.3 `pg_config` 的干净 `-O2 -Werror` build log/exit 0/扩展二进制和可复验 SHA-256 manifest | 功能测试：recording Map/Filter compatibility 与三参数 exact Filter 保持 ordinary child、三值/NULL/cardinality、`LIMIT` 前 placement、prepared/generic plan、RLS/权限、snapshot/savepoint、cancel/recovery、多 backend 与 direct `INSERT ... SELECT`。4A.1 新验证 C→Python Unicode instruction/input、空串、exact savepoint/recovery、合法 open/task error 与缺失/多余/错序/未知 code v3 error frames；v2 字段和 golden vectors 不变。尚无真实模型/GPU 性能证据，rescan/EPQ/parallel、join/aggregate 和更宽 query shape 未支持。 |
| PostgreSQL-private plan/runtime + neutral provider seam | `code/postgres/semloom_pg/src/{sem_plan_spec,semantic_filter_contract,recording_contract,pg_semantic_runtime,sem_pump,sem_operator_machine,sem_map_machine,sem_filter_machine,ai_provider_port,provider,recording_provider,uds_provider,wire_common,wire_v2,wire_v3}.{h,c}`；`code/src/execution_provider/{wire,adapters,server.py}`；旧 `gateway/` compatibility wrappers 与上述 contract/TAP | `3b2077e1` 仓库外 resource qualification 继续绑定原 Map/Filter RSS/FD 数字；`359ffdf3` 只追加精确 18.3 功能/生命周期复验，未重跑性能或资源 smoke | schema v1/v2、pump binding 和 shared runtime 保持。`wire_common.c` 现拥有 framing/socket wait/JSON primitive，v2/v3 只拥有版本 schema/digest/error；v3 error frame 已严格校验。query-fixed provider 公布中立 input limit，runtime 在 canonical-message 构造前拒绝超限输入，UDS 保留防御检查。gateway 双 adapter seam、query-fixed execution profile、fixed endpoint、accepted-prefix、多在途/乱序 completion 与 SemLoom adapter 尚未实现。 |
