...
| SAOR fixed-envelope active-set release | named shared-credit runtime、direct/project 统一 runner、matched solo、lifecycle/mechanism fail-closed 汇总 | `saor_active_set_release_formal_20260812_69affc7e/` | 40/40、0 incident、exactly-once；定位数据中 SAOR 12,393 tok/s、fg P99 50.3s，为 credit 臂 fg 最好；static 9,508 tok/s、fg P99 29.2s、fg SLO 0%。原始完整 gate 因 DRR/VTC rep2 无 post-drain 样本而 fail-closed；resolution-aware compact replay 将低于 250 ms 的两窗口判为不适用，effective 12/12，但不升级完整 validation。仍为 `directional-only`。 |
| Foreground strict-priority release-only upper bound | shared-credit `strict_priority`、Job lifecycle close、priority evidence、三臂 readiness/summary | 尚无 GPU 结果；模板 `deploy/autodl/saor_priority_reachability.example.json` | 本地实现与单测已通过；只回答非抢占 release 能否达到 fg P99≤30.7s、SLO≤1%，不代表 SAOR/reservation 胜出。远端运行前仍需 machine preflight、idle/lease/PG/Ray/endpoint gate。 |
...
| `saor_active_set_release_formal_20260812_69affc7e/` | fixed-envelope 2-Job active-set killer matrix | 40/40、0 incident、exactly-once；原始完整 validation 保留 fail-closed。compact mechanism replay 依据 250 ms trace resolution 得到 effective 12/12，显式不升级完整 validation。SAOR 未越过 static，状态仍为 `formal-evaluated/directional-only`。 |
