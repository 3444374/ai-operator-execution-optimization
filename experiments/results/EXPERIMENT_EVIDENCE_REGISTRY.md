...
reference 及其独立 cost/cardinality metadata、同步单在途 UDS、shared runtime、neutral provider port、
schema v1/v2 planner-owned plan spec、
wire v2/v3 和公共 compatibility suite 的功能与资源生命周期；小规模真实模型只证明纵切面可运行，
不支持语义质量、第二 physical path、
...
|---|---|---|---|
| PostgreSQL planner-visible `SemMap` / exact `SemFilter` | `code/postgres/semloom_pg/`、`code/src/execution_provider/`；19 项 PostgreSQL static contract、8 项冻结 v2 protocol、9 项 v3 semantic protocol、8 项 fixed-model adapter、5 项 gateway migration contract、PGXS SQL regression 与 TAP lifecycle/fault tests | 双 RTX 4090 服务器隔离源码构建 `REL_18_3`（upstream `62d6c7d3…`）；历史 4A/4A.1 与更早结果继续绑定各自提交。4B `53cf3da8` 以 warning-free `-Werror`、regression 1/1、TAP 404/404、Python/static 45/45 和 neutral/machine C11 compile 通过；仓库外证据包 `postgresql_semfilter_4b_fixed_model_53cf3da8_20260831` 保留 preflight、build/installcheck、失败尝试、真实模型 capability 与 SHA-256 manifest。4B.1 `ef314618` 的等价源码树通过 Python/static 48/48、同一 PostgreSQL 资格矩阵和 neutral/machine C11 compile；`postgresql_semfilter_4b1_http_hardening_ef314618_20260831` 保存 source/diff identity、原始日志、actual/expected、二进制与已校验 manifest。cost/cardinality `47407751` 通过 warning-free `-Werror`、regression 1/1、TAP 414/414、Python/static+migration 49/49 和 neutral/machine C11 compile；`postgresql_semfilter_cost_cardinality_47407751_20260831` 保存源码哈希、原始日志、actual/expected、二进制、status 与已校验 manifest | 功能测试：recording Map/Filter compatibility 与三参数 exact Filter 保持 ordinary child、三值/NULL/cardinality、`LIMIT` 前 placement、prepared/generic plan、RLS/权限、snapshot/savepoint、cancel/recovery、多 backend 与 direct `INSERT ... SELECT`。4B 增加 fixed-profile model identity、valid/invalid raw output、HTTP 4xx/5xx、invalid JSON、timeout、returned-model mismatch、savepoint、statement cancel、fresh-session recovery 与 no-task lazy open；4B.1 拒绝常见 3xx 且不访问目标或转发 bearer token，并使 DNS 与 slow-drip response 受单一 monotonic deadline 约束。`47407751` 让 plain `EXPLAIN` 分列 semantic input rows、output selectivity、estimated calls/tokens、model role 与 AI work cost，`EXPLAIN ANALYZE` 分列实际 calls/usage；该启发式未校准，不产生质量或性能结论。Qwen2.5-1.5B-Instruct/vLLM 0.25.1 对 `yes/no/NULL` 的小规模 SQL 只输出 `yes` 对应行；该单次 capability 不证明自然语言质量、性能或泛化。第二 path、rescan/EPQ/parallel、join/aggregate 和更宽 query shape 未支持。 |
| PostgreSQL-private plan/runtime + neutral provider seam | `code/postgres/semloom_pg/src/{sem_plan_spec,sem_filter_cost,semantic_filter_contract,recording_contract,pg_semantic_runtime,sem_pump,sem_operator_machine,sem_map_machine,sem_filter_machine,ai_provider_port,provider,recording_provider,uds_provider,wire_common,wire_v2,wire_v3}.{h,c}`；`code/src/execution_provider/{wire,adapters,server.py}`；旧 `gateway/` compatibility wrappers 与上述 contract/TAP | `3b2077e1` 仓库外 resource qualification 继续绑定原 Map/Filter RSS/FD 数字；`359ffdf3` 追加 4A.1 功能复验；`53cf3da8` 追加 4B 功能与小规模真实模型 capability；`ef314618` 追加 4B.1 HTTP/DNS boundary 功能复验；`47407751` 追加 reference cost/cardinality 功能复验，均不替换资源 smoke | schema v1/v2、pump binding 和 shared runtime 保持；cost metadata 与 semantic digest 分离，runtime 计数实际 model calls 和 prompt/output usage。`wire_common.c` 拥有 framing/socket wait/JSON primitive，v2/v3 各自拥有版本 schema/digest/error。gateway 的 golden/fixed-model adapters 共享 `V3SessionRunner`；PostgreSQL query-fixed profile 选择 distinct provider identity，neutral errors 映射稳定 SQLSTATE。endpoint/model/timeout/auth 只来自仓库外配置；fixed adapter 不跟随重定向，单一调用 deadline 覆盖 DNS 到响应体。accepted-prefix、多在途/乱序 completion 与增量 SemLoom adapter 尚未实现。 |
...
| `SemFilter` 关系语义 | `sem_filter_path.c`、`sem_filter_cost.c`、`sem_filter_machine.c`、公共 runtime、wire v2/v3 与 PGXS/TAP | 历史 recording qualification 与 `3b2077e1` golden/resource 结果继续保留；`53cf3da8` 追加 404/404 TAP 和一次真实模型 capability；`ef314618` 在相同 404/404 TAP 上追加 fixed HTTP/DNS boundary 复验；`47407751` 追加 414/414 TAP cost/cardinality 复验；无性能/质量结果 | 一参 recording 兼容不变；三参 path 严格解析 uppercase TRUE/FALSE/UNKNOWN，SQL NULL 不建 task，并已验证 duplicate identity、`LIMIT` 前 keep/drop、Unicode/空串、savepoint、invalid raw output/error frame、cancel/recovery、EXPLAIN plan identity/cost/actual usage 和 fixed endpoint。fixed endpoint 不跟随重定向，且 DNS 到响应体受单一调用 deadline 约束。真实模型单次结果只证明相同 parser/keep-drop 链路可运行；第二 proxy/oracle path、近似质量 policy 与异步乱序 completion 尚未实现。 |
