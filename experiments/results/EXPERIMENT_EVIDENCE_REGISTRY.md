...
|---|---|---|---|
| PostgreSQL planner-visible `SemMap` / exact `SemFilter` | `code/postgres/semloom_pg/`、`code/src/execution_provider/`；17 项 PostgreSQL static contract、8 项冻结 v2 protocol、8 项 v3 semantic protocol、5 项 gateway migration contract、PGXS SQL regression 与 TAP lifecycle/fault tests | 双 RTX 4090 服务器隔离源码构建 `REL_18_3`（upstream `62d6c7d3…`）；历史 4A 与更早结果继续绑定各自提交。4A.1 `359ffdf3` 以 warning-free `-Werror`、regression 1/1、TAP 320/320、protocol/static 33/33、gateway migration 5/5 和 neutral/machine C11 compile 通过 | 功能测试：recording Map/Filter compatibility 与三参数 exact Filter 保持 ordinary child、三值/NULL/cardinality、`LIMIT` 前 placement、prepared/generic plan、RLS/权限、snapshot/savepoint、cancel/recovery、多 backend 与 direct `INSERT ... SELECT`。4A.1 新验证 C→Python Unicode instruction/input、空串、exact savepoint/recovery、合法 open/task error 与缺失/多余/错序/未知 code v3 error frames；v2 字段和 golden vectors 不变。尚无真实模型/GPU 性能证据，rescan/EPQ/parallel、join/aggregate 和更宽 query shape 未支持。 |
