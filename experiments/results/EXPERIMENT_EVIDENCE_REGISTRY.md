...
| AIMD、EWMA、PID admission | `code/src/scheduling/adaptive_admission.py`、`pid_admission.py` 及测试 | 无正式 GPU 矩阵 | 代码与契约完成；尚不能声称端到端有效。 |
...
19 个目录均有各自的 `README.md`。失败、被替代或门禁目录仍保留，是为了记录事故和排除理由，不应从台账中删除。
...
1. AIMD、EWMA、PID 的同 workload 正式 GPU 对照。
