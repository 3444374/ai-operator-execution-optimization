...
本文是正式方法实验的统一入口，回答三个问题：机制是否已经实现、是否只通过了功能测试、是否已有真实 GPU 性能证据。具体数字和逐次运行证据仍以各结果目录的 `README.md`、`manifest.json` 和 CSV 为准。当前工程主线已在 `REL_18_3` extension planner-visible `SemMap`、协议 v2 canonical digest 和同步单在途 UDS recording slice 上完成 PostgreSQL-private scan/pump、neutral provider port 与 recording/UDS adapter 的职责拆分；下一步实现 exact `SemFilter` 和最小第二 semantic path。accepted-prefix、多在途、增量 SemLoom session 与 IMLane-like batch placement 在数据库语义资格后推进，Kalypso-like lineage 只作后续参考。既有文本、图像和 SAOR 条目记录外部物理执行基座与历史证据，不得重标为已实现数据库内算子。执行顺序以 `experiments/plans/experiment_status_and_gaps.md` 顶部摘要为准。
...
|---|---|---|---|
| PostgreSQL planner-visible `SemMap` | `code/postgres/semloom_pg/`；9 项静态 contract、7 项 Python protocol、PGXS SQL regression 与 TAP lifecycle/parity tests | 双 RTX 4090 服务器隔离源码构建 `REL_18_3`（upstream `62d6c7d3…`）；提交 `d08eda38` 以 `-Werror` 无警告 build、regression 1/1、TAP 129/129、Python/static 16/16 通过 | 功能测试：deterministic carrier 已验证 EXPLAIN/ordinary child、filter/projection、LIMIT/error recovery、preload、prepared statement、snapshot、cancel/recovery、direct `INSERT ... SELECT` rollback/commit；plain EXPLAIN、LIMIT 0、zero-row child 与 NULL-only 执行不连接 provider，`PROPAGATE_NULL` 由 PostgreSQL 完成。相同 pump 行为测试在 in-process 与 UDS adapters 上验证 SQL rows、sequence、NULL、EXPLAIN 和错误生命周期一致；外部 UDS 还覆盖 Unicode、断连、坏 digest、response/connect wait 取消、非 UTF8、整数溢出和清理。尚无模型/GPU 性能证据，rescan/EPQ/parallel 和更宽 query shape 未支持。 |
| PostgreSQL-private pump + neutral provider seam | `code/postgres/semloom_pg/src/{ai_provider_port,sem_pump,provider,recording_provider,uds_provider,wire_v2}.{h,c}`、`gateway/` 与上述 contract/TAP | 精确 18.3 产物 `tap-run-d08eda38/` 中 `semloom_pg.so` SHA-256 为 `5907afd9a749cfcb63332325ad357a9b0a311f3cb6d89a539a5aa12b3a1c7ae8`。仓库外 `resource-run-d08eda38/resource-smoke.txt` 对 2,000×100,000-byte task 在 warm relation VFD 后采样：RSS 起始/峰值/结束为 17,120/19,532/18,852 KiB（峰值/结束增量 2,412/1,732 KiB，预设上限 16,384/8,192 KiB），FD 为 10/12/10（增量 2/0，预设上限 4/2）；客户端、行数统计和 gateway 均为 0 退出，结果 2,000 行 | `ai_provider_port.h` 只含固定宽度值、byte slice、opaque session/provider 和 caller-owned error，不含 `Datum/Oid/MemoryContext` 等 PostgreSQL 类型。`sem_pump.c` 拥有 child pull、next sequence、completion copy 与 terminal error mapping；provider/config 在 begin 固定，首个非 NULL task 才 open，query-context callback 与正常 close 共用无阻塞幂等本地清理。SQL-visible semantic spec、physical algorithm、provider execution profile 使用独立 digest，物理 mapped-column 不进入 wire identity。资源 smoke 只说明当前路径未观察到随累计 payload 近似线性增长或 FD 泄漏，不是性能结论；accepted-prefix、多在途/乱序 completion、direct HTTP 与 SemLoom adapter 尚未实现。 |
