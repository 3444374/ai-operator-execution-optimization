...
reference、同步单在途 UDS、shared runtime、neutral provider port、schema v1/v2 planner-owned plan spec、
...
1. reference/optimized path identity、AI-work cost、quality policy/evidence 与 fallback 尚无实现资格证据；
   当前 schema v2 已通过 golden/fixed-model reference，但真实模型只有单次 capability，没有质量或性能验证。
