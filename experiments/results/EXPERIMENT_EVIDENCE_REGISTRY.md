...
|---|---|---|---|
| PostgreSQL planner-visible `SemMap` / exact `SemFilter` | `code/postgres/semloom_pg/`；10 项静态 contract、8 项 Python protocol、PGXS SQL regression 与 TAP lifecycle/fault tests | 双 RTX 4090 服务器隔离源码构建 `REL_18_3`（upstream `62d6c7d3…`）；提交 `d3a22dcf` 以 `-Werror` 无警告 build、regression 1/1、TAP 193/193、Python/static 18/18 通过 | 功能测试：两个 deterministic reference carriers 已验证 ordinary child、Filter 三值/NULL/cardinality、`LIMIT` 前 placement、preload、prepared/generic plan invalidation、RLS/权限、snapshot/savepoint、cancel/recovery、多 backend 与 direct `INSERT ... SELECT`。plain EXPLAIN、LIMIT 0、zero-row child 与 NULL-only 执行不连接 provider。in-process/UDS adapters 对 Map/Filter rows 和归一化 EXPLAIN 一致；UDS fault tests 保持既有 Unicode、断连、digest、取消、编码、整数、NUL 和脱敏错误边界。尚无模型/GPU 性能证据，rescan/EPQ/parallel、join/aggregate 和更宽 query shape 未支持。 |
| PostgreSQL-private shared runtime + neutral provider seam | `code/postgres/semloom_pg/src/{pg_semantic_runtime,sem_pump,sem_operator_machine,sem_map_machine,sem_filter_machine,ai_provider_port,provider,recording_provider,uds_provider,wire_v2}.{h,c}`、`gateway/` 与上述 contract/TAP | 精确 18.3 产物 ID `tap-run-d3a22dcf` 中 `semloom_pg.so` SHA-256 为 `e4048b9709228c18b335e3a90ca98c766a6d6c581b57eb53ac1e8affecae5f26`。仓库外 `resource-run-d3a22dcf/resource-smoke.txt`：Map 2,000×100,000-byte 输入产生 200,018,000 字节，RSS 起始/峰值/结束 21,048/21,688/21,688 KiB，FD 25/25/24；Filter 20,000 行含 15,000 个非 NULL task、输出 5,000 行，RSS 21,688/21,688/21,688 KiB，FD 26/26/24。历史 `0b9948ee` hardening 与 `d08eda38` seam/resource 结果继续绑定各自提交 | `ai_provider_port.h` 不含 PostgreSQL 类型；input limit 仍由 adapter 通过固定宽度参数返回。`PgSemanticRuntime` 独占 provider selection/lazy lifecycle、sequence、completion copy、query cleanup、中立错误映射和公共计数；thin pump 不持有 provider session，Map/Filter machines 不持有 socket/JSON/query cleanup。当前 smoke 未观察到随累计 payload 近似线性增长或 FD 泄漏，但不是性能结论。accepted-prefix、多在途/乱序 completion、direct HTTP 与 SemLoom adapter 尚未实现。 |
