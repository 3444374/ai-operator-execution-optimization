...
| 两 Job guaranteed-overlap 干扰 | exact-short full/half 控制、项目 static/shared、原生两 Job runner 与统一逐请求时间汇总器 | `opening_multijob_interference_20260809/` | 5s offset 下项目 matched-cap 因果链与三条原生 overlap 观察已闭合。项目 single JCT 的 93.87% 是 arrival span；long 令 static/shared 的 short service mean +59.74%/+88.17%，并放大上游 buffer。shared 总吞吐 +21.03%，但 short JCT +4.98%、Jain fairness 下降；不证明动态全面胜出。 |
...
| `opening_multijob_interference_20260809/` | short/long guaranteed-overlap 的 single/multi、project/native 统一证据与 project request-time 分解 | quota-only≈0；项目 71.24 s=66.875 s arrival span+4.367 s drain，单请求非 service 开销均值约81 ms。long 加入后 service、buffer/flush-submit 同时恶化。Daft 三臂只作同系统观察；完整 raw 留服务器，Git 保存紧凑派生数据。 |
