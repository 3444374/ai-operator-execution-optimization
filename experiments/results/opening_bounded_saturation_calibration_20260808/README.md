...
2. C128 实测达到 C256 的 98.22%，是第一个满足预先规定 97% 选择条件的并发点，且没有 C256 的持续 waiting/KV 顶格现象，因此采用为正式 bounded 对照。
...
只保留一张双面板候选图：左侧为并发—service tok/s/MFU，标出 C128 实测 98.22% 且满足 97% 选择条件；右侧为并发—waiting/KV/TTFT，显示 C256 的过量排队。正式原生矩阵完成后再决定它作为主图还是状态感知动机图的子面板。
