...
旧容量数字和所有失败保留，数据库查询、组织控制、多Job与图像接入独立继续。
...
## 共同调用与tuple绑定（2026-09-07）

[验证记录](postgresql/semantic_binding_20260907/README.md)：本地115、Linux138项、PG18.3回归1及
...
来源/权限保留，实现及复核修复纳入 main。该历史切片当时真实模型为 0/32、资源压力未运行；
后续真实链路通过而资源资格失败，见上方 2026-09-04 记录。
...
| `operator_cost_profile_pilot_20260804/` | 双 4090 四候选 cost-profile v1/v2 运行要求检查；完整 raw trace 已于 2026-09-21 按 RAW_ARCHIVAL 政策迁至两处镜像（见该目录 README 注记）。 | v2 8/8、0 incident、512 unique requests/cell、23 维四向量同 context；n=1 只验证采样要求与约 4 小时 formal 预算，不作配置排名。 |
