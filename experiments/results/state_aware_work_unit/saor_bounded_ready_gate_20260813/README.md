...
| 四臂 | static partition、原 `saor_release`、bounded-ready 0.125K、bounded-ready 0.25K |
| priority/guard | foreground priority=1、30s SLO/window；bulk priority=0；actual-work debt cap 为 0.125K 或 0.25K |
...
| 稳定性 | 两轮 tokens/s sample CV：static 0.159%、SAOR 0.300%、0.125K 0.066%、0.25K 0.428% |
...
| 1 | bounded-ready 0.125K | **12,355.00** | 69.11 | **24.75** | 61.59 | **18.15** | **0.658** | **0.000** | 0.940 | **pass** |
| 1 | bounded-ready 0.25K | 12,141.26 | 70.05 | 24.64 | 62.69 | 17.96 | **0.752** | 0.000 | 0.944 | fail: bulk |
...
| 2 | bounded-ready 0.125K | **12,366.59** | 68.80 | **24.35** | 61.24 | **17.58** | **0.666** | **0.000** | 0.942 | **pass** |
| 2 | bounded-ready 0.25K | 12,214.99 | 69.53 | 24.67 | 62.66 | 17.97 | **0.744** | 0.000 | 0.941 | fail: bulk |

两轮均值下，0.125K 为 12,360.79 tok/s、foreground P99 17.87s、foreground SLO
...
- 0.25K 虽保护 foreground，但 bulk SLO violation 两轮为 0.752/0.744，均越界，拒绝。
...
| 1/0.125K | 97.58% | 105.89/255 | 2.06/33 | 0.393/0.576 | 0.466 | 2.968/4.594 | 0.056/0.271 |
| 1/0.25K | 94.74% | 104.46/256 | 1.57/21 | 0.415/0.585 | 0.458 | 2.420/4.390 | 0.052/0.304 |
...
| 2/0.125K | 96.79% | 106.18/255 | 1.67/37 | 0.393/0.576 | 0.467 | 2.448/4.431 | 0.059/0.277 |
| 2/0.25K | 94.32% | 104.72/256 | 1.59/21 | 0.417/0.585 | 0.461 | 2.480/4.478 | 0.057/0.291 |

KV 是 0–1 分数；MFU 也是分数。vLLM histogram TTFT/ITL 是模型服务总体分布，不能替代上表
按 Job 的外部 request E2E P99。0.125K 的 vLLM TTFT P99 比 static 高，但 foreground 外部 P99
...
|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|
| 1 | 0.125K | 2,048 | 512 | 256 / 84,837 | 512 | 13 | 499 | 2 | 1 | 0 | 0 |
| 1 | 0.25K | 2,048 | 512 | 256 / 84,837 | 512 | 10 | 502 | 2 | 1 | 0 | 0 |
| 2 | 0.125K | 2,052 | 512 | 256 / 85,057 | 512 | 10 | 502 | 2 | 1 | 0 | 0 |
| 2 | 0.25K | 2,048 | 512 | 256 / 84,832 | 512 | 10 | 502 | 2 | 1 | 0 | 0 |
...
| 事实 | 0.125K 两轮同时通过 correctness、机制、foreground、bulk 与效率门；汇总器返回 `status=passed`、`formal_registration_candidate`。 |
| 事实 | 0.25K 两轮都只因 bulk SLO 越界失败；不能在 formal 中继续携带这一臂作为候选。 |
| 推断 | 旧 SAOR 的主要问题确实包含 observation contract，而不只是 cap 选错：暴露 concrete-ready set 后，0.125K 在保持 release 吞吐的同时把 foreground P99 从约 55.3s 降到 17.9s。 |
| 推断 | 0.125K 的 actual-work debt recovery 提供了足够 bulk 保护；更松的 0.25K 允许 foreground 优先持续更久，导致 bulk request-level SLO 越界。该解释仍需 formal event/service-lag 复验。 |
...
`development-gated/formal-registration-candidate`，并且只冻结 **bounded-ready 0.125K**。它
给出了当前最清楚的动态调度需要场景：bulk 已经占用共享包络、5s 后短 foreground 到达；static
保护前台但损失约 30% 吞吐，原 SAOR 保吞吐却严重伤前台，bounded-ready 0.125K 在两轮短测中
同时保持吞吐、前台 SLO、bulk guard 和较高的 achieved-service Jain。

下一步不是继续扫 cap、动态 K、reservation 或 4-Job，而是冻结 0.125K，建立 1 warm-up + 3
formal 的候选矩阵，并补齐：随机/交错运行顺序、full event ledger 的共同积压 service-lag、三个
JCT 反事实、starvation/work conservation、SLO goodput、能耗与重复 CV。formal 仍必须包含 static
与原 SAOR 定位臂；0.25K 只保留为开发期 rejected ablation，不进入 proposed 候选。
