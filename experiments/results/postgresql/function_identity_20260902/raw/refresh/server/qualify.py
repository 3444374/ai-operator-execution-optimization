"""Build and qualify the PG18.3 function-identity slice without model calls."""
import argparse
import hashlib
import json
import os
...
for option, expected in (("--bindir", args.prefix / "bin"), ("--pkglibdir", args.prefix / "lib")):
    assert subprocess.check_output([str(args.prefix / "bin/pg_config"), option], text=True).strip() == str(expected)
...
for cwd, label in ((extension, "extension"), (extension / "t/plan_contract", "plan-caller")):
    run(label + "-clean", ["make", "PG_CONFIG=" + str(args.prefix / "bin/pg_config"), "clean"], cwd=cwd)
...
save("qualification.json", dict(source_commit=commit, tests=tests, steps=steps,
    ...
    isolated_prefix_only=True, resource_smoke_run=False))
history_root = args.history_root or args.root
for name in ("red-tap", "fix-build", "fix-install", "green-tap", "coverage-tap", "identity-tap"):
    (public / ("history-" + name + ".log")).write_text(scrub((history_root / (name + ".log")).read_text()))
for directory in ("red-tap-node-logs", "green-tap-node-logs", "coverage-failed-node-logs"):
    for path in (history_root / directory).iterdir():
        if path.is_file():
            (public / (directory + "-" + path.name)).write_text(scrub(path.read_text()))
