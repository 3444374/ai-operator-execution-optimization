"""Qualify the PG choice connection with isolated PG18.3 and fixture-only models."""
import argparse
import hashlib
import json
import os
...
assert commit == "80bb7fc508d42a44f2ca485944d800131fa59df3"
...
tap_count = int(re.search(r"Files=3, Tests=(\d+)", tap).group(1))
...
save("qualification.json", dict(source_commit=commit, tests=tests, steps=steps,
    ...
    filter_insert_supported=False,
    excluded_attempts=["behavior-tap: manually released waiting gateway; savepoint assertion missed secondary INSERT failure"],
    diagnostic_comparison_sha256=sha(args.root / "insert-diagnostic/comparison.json")))
for name in ("red-build", "red-tap", "codec-build", "codec-tap", "enable-build", "enable-tap",
             "behavior-tap", "http-tap"):
    (public / ("history-" + name + ".log")).write_text(scrub((args.root / (name + ".log")).read_text()))
with tarfile.open(args.root / "behavior-hang-logs.tgz") as archive:
    for member in archive.getmembers():
        if member.isfile() and member.name.endswith("/003_choice_execution_choice_execution.log"):
            text = archive.extractfile(member).read().decode("utf-8")
            (public / "excluded-savepoint-attempt.log").write_text(scrub(text))
(public / "insert-comparison.json").write_text(scrub((args.root / "insert-diagnostic/comparison.json").read_text()))
shutil.copy2(args.root / "diagnose_insert.py", public / "diagnose_insert.py")
for mode in ("legacy", "choice", "rejected"):
    paths = list(extension.glob("tmp_check/t_003*/" + mode + ".requests"))
    assert len(paths) == 1, mode
    (public / ("http-request-" + mode + ".jsonl")).write_text(scrub(paths[0].read_text()))
