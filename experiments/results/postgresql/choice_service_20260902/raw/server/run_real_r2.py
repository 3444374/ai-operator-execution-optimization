...
with urllib.request.urlopen('http://127.0.0.1:8013/health', timeout=2) as response:
    ...
    save(args.root/'health-r4-r2.json', dict(status=response.status, method='GET', model_request=False))
command = [sys.executable, str(args.repo/'code/scripts/experiments/run_choice_service_checks.py'),
    '--repo', str(args.repo), '--root', str(args.root/'pg-real-r2'), '--prefix', str(args.prefix),
...
with (args.root/'real-run-r2.log').open('x') as log:
    completed = subprocess.run(command, stdout=log, stderr=subprocess.STDOUT,
        env=dict(os.environ, PYTHONPATH=str(args.repo/'code'), PYTHONDONTWRITEBYTECODE='1'))
save(args.root/'real-invocation-r2.json', dict(argv=command, exit_code=completed.returncode,
    automatic_retries=0, previous_failed_run='pg-real',
    previous_failure='audit counted BatchEncoding fields instead of token IDs; fixed in 0a1c12d3'))
