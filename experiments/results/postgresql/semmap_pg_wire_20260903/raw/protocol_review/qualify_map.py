...
try:
    tap = run("tap", as_pg + ["env", "PATH=" + env["PATH"], "PG_TEST_NOCLEAN=1", "make",
              "PG_CONFIG=" + str(prefix / "bin/pg_config"), "installcheck", "REGRESS=", "PROVE_TESTS=t/007_map_execution.pl"], cwd=extension)
finally:
...
save("qualification.json", dict(source_commit=commit, tests=tests, steps=steps,
    ...
    isolated_prefix_only=True, resource_smoke_run=False, generated_map_execution_connected=True))
