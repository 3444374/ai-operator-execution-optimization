...
parser.add_argument("--branch", default="codex/semmap-pg-plan")
...
assert git("branch", "--show-current") == args.branch
...
cases = [
    ("postgres-contracts", ["python3", "-m", "unittest", "discover", "-s", "code/tests/postgres", "-v"], 112),
...
(args.output / "checks.log").write_text("\n".join("## " + record["name"] + "\n" + output for record, output in results).rstrip() + "\n")
