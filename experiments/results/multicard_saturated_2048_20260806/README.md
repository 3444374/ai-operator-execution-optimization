...
> 3. **措辞**：原"统计等价"无 TOST/equivalence margin（仅 3 reps，Welch t-test p≈0.127）→ 改"未检出差异"（"未检出"≠"证明等价"）。"质量等价"见 rich 报告 EM/F1 复核。
...
>
> **范围**：bounded_http 天花板 + **duckdb_ai harness_sharded_diagnostic** + project_static 2-endpoint，饱和配置 1w+3f 的 vLLM service tokens/s 对比。**duckdb 臂是 harness 诊断，非 DuckDB 产品原生排名**。不是完整矩阵（缺 lb_rr / direct_static_50_50 / project_smart / framework-native，见 §7）。
...
|---|---|---|---|---|
| bounded_http c=32（天花板） | **88847** | 447 | **0.5%** | [88977, 88246, 89318] |
| duckdb_ai harness_sharded_diagnostic c=32 | **76780** | 569 | **0.74%** | [77429, 76042, 76868] |
| project_static K=32（8×4） | **78739** | 597 | **0.76%** | [78463, 78186, 79567] |

% of ceiling（bounded 88847）：duckdb **86.4%**，project **88.6%**。project vs duckdb：**+2.55%**（reps 完全无重叠：project min 78186 > duckdb max 77429）。

## 6. 结果解释

- **事实**：饱和配置（group 口径）下 project_static（78739）与 duckdb_ai harness_sharded_diagnostic（76780）service tokens/s 接近（**项目高 +2.55%**），CV 都 <1%，reps **完全无重叠**（project min 78186 > duckdb max 77429）。两者都是 bounded 天花板（88847）的 ~87–89%。
- **推翻 preliminary 假象**：256 行 K=8 时 project 看着慢 40%——那是 **K=8 欠喂**（8 << 饱和点 32）的人为结果，不是项目真实能力。调到饱和 K=32 后，项目不低于 DuckDB-ai harness 诊断。
- **推断**：SQuAD 均匀短答案 + 饱和配置下，项目静态调度的服务吞吐**未检出显著低于** DuckDB-ai harness 诊断（注意：harness_sharded_diagnostic 不是 DuckDB 产品原生 baseline，见顶部身份订正）。
- **不能声称**：
  - 项目"优于"DuckDB-ai：+2.55%、reps 无重叠，但 Welch t-test p≈0.127（3 reps，**无预注册 equivalence margin / TOST**）；"未检出差异"≠"证明等价"。
...
- **含义**：项目方法在饱和配置下**未检出显著低于** DuckDB-ai 的 harness_sharded_diagnostic（**非 DuckDB 产品原生 baseline**；身份见顶部订正）。这把"项目是否值得做"的问题，从"会不会被 DuckDB-ai 的 lean 扩展碾压"收敛到"自适应调度在什么条件下产生净收益"。
- **下一步（补全矩阵 + 测真价值）**：
