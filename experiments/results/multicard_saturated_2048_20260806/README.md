...
| duckdb_ai harness_sharded_diagnostic c=32 | **76780** | 698 | **0.91%** | [77429, 76042, 76868] |
...
- **事实**：饱和配置（group 口径）下 project_static（78739）与 duckdb_ai harness_sharded_diagnostic（76780）service tokens/s，**项目高 +2.55%**，reps **完全无重叠**（project min 78186 > duckdb max 77429）；Welch t-test **t=3.36, df≈4, p≈0.0284**（two-tailed，scipy 一致）→ project **统计显著高于** duckdb harness diagnostic。两者都是 bounded 天花板（88847）的 ~87-89%。
- **推翻 preliminary 假象**：256 行 K=8 时 project 看着慢 40%——那是 **K=8 欠喂**（8 << 饱和点 32）的人为结果，不是项目真实能力。调到饱和 K=32 后，项目不低于 DuckDB-ai harness 诊断。
- **推断**：SQuAD 均匀短答案 + 饱和配置下，项目静态调度 service tokens/s **显著高于** duckdb_ai harness_sharded_diagnostic（+2.55%, p≈0.0284, n=3）。注意：harness_sharded_diagnostic **不是** DuckDB 产品原生 baseline（duckdb 单进程单 endpoint 才是产品轨），所以"显著高 vs harness 诊断"**不等于**"项目优于 DuckDB 产品"；仅表明项目多 endpoint 方法在本 setup 不低于（数值上高于）harness 预切诊断。
- **不能声称**：
