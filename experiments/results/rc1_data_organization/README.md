...
- **4-ep/0.43（KV 饱和，max 98–100%）**：策略分化 39–50k 两簇，**排名反转为 sequential > fixed >> row_cap≈best_fit > length_align**；重排序类塌到 39–40k、SLO 违约 60%。
...
**喂饱门禁**：策略确实喂饱 vLLM（GPU 80–90%、`model_wall≈operator_wall` 无 pipeline 瓶颈），但不榨干 raw 上限——active-work 准入（W65536）是吞吐 binding 杠杆、效应随 regime 反向（2-ep 压住可放开、4-ep 防 thrash 应保留）。
...
- **BFD**：best-fit-decreasing（按 output work 降序装箱）；**TTFT**：time-to-first-token；**CV**：变异系数；**SLO**：服务级延迟上限。
...
> ⚠️ **指标可靠性（AGENTS §7.5-F）**：`vllm_kv_cache_usage_perc` 当前实现可疑（曾在 working set 应近满时显 0.06–0.29%），**修前不宜单独用它量化 KV 压力**。本报告的 4-ep 饱和判定另有 **3 个独立信号交叉印证**：(a) TTFT 翻倍（0.2–0.3s→0.6–1.1s，饱和才需更多 prefill）；(b) prefix 命中塌缩（0.06–0.07，饱和淘汰前无法复用）；(c) 同协议 unthrottled bounded client 在 4-ep 自己 thrash 到 24k（比策略还低）。故 regime 方向判定稳，但精确 KV% 待改采 `gpu_cache_usage_perc` / Prometheus 累计值复核。
