...
本实验延续 `vector_writeback.md`，在确认 pgvector `vector(128)` 批量写回可用后，测试行数扩大时瓶颈是否迁移。
...
```text
motivation/results/pg18_4_fake/pgvector_scaling.csv
motivation/results/pg18_4_fake/pgvector_scaling_fine_contrast.csv
```
...
> 接真实 embedding 小模型或轻量模型服务，先跑 `total_rows = 1024, 4096`，比较 `python batched / ray_task / ray_actor`，并复用本实验的 pgvector 批量写回路径。如果 GPU 环境可用，应同时补一组 small-scale GPU-backed E2E profile，用同一套阶段计时判断瓶颈是否迁移到模型服务队列、GPU 利用率或 writeback。
