...
本实验延续 `system_profile.md`，用于补齐下一轮 baseline。
...
```text
motivation/results/pg18_4_fake/baseline_matrix.csv
motivation/results/pg18_4_fake/actor_batch_workers.csv
```
...
3. 真实 CPU embedding 小模型或轻量模型服务，验证 fake invocation 信号是否迁移。
4. 如果 GPU 环境可用，补一组 small-scale GPU-backed E2E profile，仍然记录 `model_service_s`、`bounded_wait_s`、`fanin_s`、`writeback_s`、in-flight 和 queue wait，用来判断真实计算端点下瓶颈是否迁移。
5. Ray actor / Ray Serve 真实 backpressure 实验，记录 queue wait、in-flight、actor idle time 和 throughput。
