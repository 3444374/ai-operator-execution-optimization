...
- 本地数据库能否连接。连接验证见 `validation/results/pg18_4_connection_validation.md`。
...
$out = 'motivation\results\pg18_4_system_profile_fake_ai_embed.csv'
...
| 只用 fake workload 声称真实 GPU/模型收益 | F6 Unverifiable claim | 下一步接真实 CPU/GPU embedding 或模型服务，并在 PG18.3 内部平台复验 |
...
4. 再接 GPU / Ray Serve / vLLM 类服务，重点看 batch size、actor 并行度、in-flight、queue wait、GPU 利用率和 writeback。
