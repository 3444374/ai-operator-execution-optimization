...
| `image_clip_bottleneck_profile_20260801.md` / `.csv` | **图像 CLIP AI_EMBED** 分阶段瓶颈画像 + §6 go/no-go 门禁（**5K 规范跑**，5000 图 × 100 iters）：CPU preprocess(resize+normalize) ~5.2ms/img vs GPU embed 0.29ms/img，ratio 13–18×（B=256 渐近 ~18），**GO**；瓶颈是 CPU 预处理，不是 decode/transfer/pg_read |
