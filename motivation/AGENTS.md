...
- 定义为什么“数据库驱动 AI workload 的数据执行与存储链路”值得优化。
...
- 本目录不承担完整研究实验规划。后续真正验证三项研究内容、做优化方案、做消融和记录调优结果时，放到 `experiments/`。

## 目录结构

- `plans/`：动机测试相关的场景、路线、集成计划和实验设计，不放原始结果，不规划完整论文实验矩阵。
...
- 动机阶段可以设计必要的大块对照，例如 no-Ray/Ray、single worker/actor pool、主控写回/worker 写回、bounded/unbounded in-flight；当这些对照用于验证正式方法贡献时，应迁入 `experiments/`。
