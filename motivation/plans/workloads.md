...
> **2026-07-29 口径更新**：本文档为早期场景探索记录。当前方向为两项策略（数据组织 + 调度提交）+ 多模态泛化验证 + 贯穿两项策略的算子代价估计。Daft 为数据引擎，文本阶段直接接入。主场景：AI_COMPLETE（文本 LLM）+ AI_EMBED/AI_CLASSIFY（图像，多模态泛化）。vLLM 为部署平台，写回为实验设置。最新方向、优先级和边界以根 `AGENTS.md`“项目范围/当前资格顺序”、`PROJECT_OUTLINE.md` 和 `docs/research/knowledge_hub.md` 为准。
...
- 当前实验只能说明方向筛选和机制验证，不能证明成熟论文方向；课题第一层主动机应来自生产式 GPU-backed E2E profile。
...
- 新文本提醒了一个此前容易遗漏的点：partition/object 粒度会直接影响并行 task 数，不能把“粒度控制”和“并行调度”割裂；更完整的问题应包括 task/actor 并行度、GPU-backed model-service 资源配比、模型服务队列和 backpressure。GPU-backed model service 是主实验链路的现实计算端点，但“算子迁移到 GPU”不作为主贡献。
...
- 不能用 CPU/fake 机制实验替代生产式 GPU-backed 主动机实验；它们只能作为历史预研和工具验证，不能作为真实 GPU 链路变量归因。
- 不能把“跨层调度”直接升级成最终论文贡献。当前还没有 GPU-backed model-service pipeline、actor pool、request routing、queue backlog 或 backpressure 的实验数据。
...
```text
motivation/benchmarks/workload_matrix.py
```

正式运行命令：

```bash
.venv/bin/python motivation/benchmarks/workload_matrix.py \
...
当前没有 Strong Accept，因为三个方向都还缺真实 GPU-backed 模型服务和数据库形态验证。优先补生产式 GPU-backed E2E profile 后，再重新评估 fatal flaw。
...
3. **资源调度层**：CPU preprocess worker 与 model-service/actor 配比、placement / locality、object store 压力控制。
...
1. **生产式 GPU-backed E2E 主动机画像**：构建 AI-SQL-compatible operator surface，采用 pgai/vectorizer-like external worker path，Ray task/actor 调用 GPU-backed embedding endpoint / Ray Serve / vLLM，结果写回数据库。指标至少包括 DB fetch、Arrow build、submit/put、queue wait、model service time、GPU utilization、fan-in、writeback、rows/s 和阶段占比。目标是回答“为什么 AI 算子外部服务链路值得优化”。
2. **GPU-backed 大块消融**：在同一条 E2E 链路中比较 no-Ray vs Ray、single worker vs actor pool、主控 fan-in 写回 vs 多 worker 写回、unbounded vs bounded in-flight、不同 batch/partition 策略。目标是先定位大阶段和大策略的影响。
3. **收益来源细分实验**：在大块消融定位之后，再固定 object 数但改变 task 数、固定 task 数但改变 object 数、固定 `ray.put` 次数但改变 fan-in 依赖数。该实验必须尽量沿用真实 GPU 链路；不能退回 CPU/fake 后直接外推。
4. **GPU-backed model-service pipeline 实验**：控制 preprocess concurrency、model actor 数、in-flight batch 数和队列上限。指标至少包括吞吐、P95 latency、队列长度、GPU utilization、object store 使用量和 backpressure 次数。不研究 GPU kernel 本身。
5. **offline_llm token-aware / prefix-aware routing 实验**：加入 `prompt_tokens`、`completion_tokens`、`prefix_id`、`tenant_id`，比较 row-count batching、token-budget batching、prefix-grouped routing、token-backlog routing。指标至少包括 tokens/s、batch token 分布、模拟 prefill/decode 时间、replica backlog、object 数、fan-in 时间和写回时间。
6. **classify_filter selectivity-aware 实验**：加入普通 predicate selectivity、AI predicate selectivity、cheap model / expensive model cascade，对比先普通 filter 再 AI_FILTER、先 AI_FILTER、cascade 三种路径。指标至少包括 model_calls_saved、rows/s、e2e、object 数、输出行数和动态下游 partition 数。
