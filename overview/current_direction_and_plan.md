...
已完成的 Phase 0 结果位于 `validation/results/`。
...
1. 搭 PostgreSQL + pgvector 真实形态；
2. 外部 worker 读取 documents 表；
3. 批量执行 fake 或本地 embedding；
4. 写回 embeddings 表；
5. 验证 vector search 输出可用；
6. 设备到位后切到真实模型或 GPU embedding service。
...
第 3 周：搭 PostgreSQL + 外部 worker + pgvector / Lance 的最小真实形态，以 embedding/RAG 作为数据库落地 baseline；如果条件允许，再接本地小模型、vLLM 或 Ray Serve endpoint，并记录模型服务队列、token backlog、actor 利用率和 backpressure 指标。
