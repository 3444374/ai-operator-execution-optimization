...
默认动作是**修改实际受影响的既有内容**，而不是每次变化都同步一长串文件：

| 发生什么变化 | 默认更新什么 | 不需要机械更新什么 |
|---|---|---|
| 内部重构，行为不变 | 代码、相关测试 | 总纲、开题、研究笔记、`PROJECT_LOG.md` |
| 公共接口或行为变化 | 代码、测试、对应设计或使用说明 | 不受影响的其他设计 |
| 重要能力状态变化 | `code/INFRA_STATUS.md` 对应能力行，并链接验证 | 每个 README 中的完整状态副本 |
| 完成一次有价值的验证 | 该次 `results/` 记录目录 | 总纲和论文，除非改变了它们引用的判断 |
| 方向、题目、研究内容变化 | `PROJECT_OUTLINE.md`、根入口、确实受影响的开题正文/材料、`PROJECT_LOG.md` | 与方向无关的文档 |
| 研究判断发生变化 | 对应研究分析或设计，以及确实受影响的对外内容 | 所有历史报告的原始结论 |
| 目录或关键入口变化 | 所在目录 README、`PROJECT_INDEX.md`、根 README、`PROJECT_LOG.md` | 与入口无关的记录 |
| 规则变化 | 只改拥有该规则的最窄 `AGENTS.md`；全局行为才改本文件，并记入 `PROJECT_LOG.md` | 其他目录规则 |
| 图表变化 | 图源、导出、`docs/figures/README.md`、对应 audit；影响主线时同步开题/论文引用 | 无关图件 |
| 普通沟通或讨论 | 修改实际受影响的既有内容 | 新建沟通总结、审查总结、吸收报告 |

另外两条长期要求：

- **新信息优先更新其所属主题，不默认追加到文件开头，也不默认新增日期小节。**
- **每次整理应删除被替代的现行说明；历史证据可以保留，但不能与当前指令混写。**
