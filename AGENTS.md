...
（cache-ON batching + prefix-affinity routing 均中性，vLLM APC 已覆盖）→
...
prefix-only 在 cache-off 下无稳定收益；cache-ON 下 batching/routing 同样
中性（<5% 门禁）。写回使用 PostgreSQL + pgvector
