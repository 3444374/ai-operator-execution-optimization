...
phase-change、3:1 weighted、第二硬件与大规模参数搜索不阻塞开题。对外叙事按三个问题展开：
一是记录数难以表示分阶段 AI 工作量，对应数据组织；二是单一服务指标难以表示当前运行状态；
三是总体效率难以表示多个作业各自的完成情况。问题二提供“何时提交、发往哪里”的服务侧信息，
问题三提供“下一份工作来自哪个作业”的作业侧信息，两者共同对应提交、路由与多 Job 调度。
代价估计承担数据库计划比较的共同支撑，并在验证后为两项研究内容提供可选的决策信息。
各子证据仍按相同严谨性要求由实验现象导出设计。
...
- 对外正文优先直接说明研究对象、角色、输入、输出和作用位置。分类澄清采用正面陈述，例如直接写“代价估计承担两项研究内容的共同支撑”，避免主动回应读者尚未接触的内部分类争议。否定式表述主要用于实验结论的证据范围、文献方法的适用条件和确有必要的概念辨析。
- 文献综述中的论文题名使用正式英文原题，系统名称可直接使用英文；中文用于解释研究问题、方法和结论，不用书名号包裹自译的中文论文名。
- 图题、分图标题、框内文字和图例按正文语言规则检查。图题优先写成“对象或条件 + 观察结果”，避免只写“状态不足、能力边界、证据链、候选组合”等抽象分类；必须保留的系统名、内部结构名或英文指标，要在同一图内或紧邻图注说明它承担的作用和读法。
- 正式材料必须区分“总动机、子问题、证据实验、研究内容、实现任务”五个层级。总动机说明一个可跨多个实验成立的不足或矛盾；子问题分解该不足；证据实验只负责验证某个子问题；研究内容回答总动机；实现任务放入技术路线。不得因为实验数量多，就把实验任务直接列成多个平级动机。
- 同一列表、表格、图组或连续页面中的平级项目必须回答同一层次的问题，并保持相近粒度。每个子项都要能回指唯一的上位问题，多个证据可以共同支持同一研究内容，但不得为了追求一一对应而虚构新的研究内容。定稿前按“父项能否概括全部子项、兄弟项是否同类、证据是否收敛到研究问题”逐级检查。
