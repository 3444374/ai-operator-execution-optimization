...
## 6.5 文献优先设计规则（Literature-First Design）

当用户询问”怎么设计 X 系统 / 算法 / 框架 / 实验方案”时，**优先从项目文献清单中的 CCF-A / 顶会论文提取设计模式和策略**，不凭空设计，不凭工程常识拍板。完整方法论见 `research/README.md` §文献优先设计方法论。

执行步骤：

1. **定位问题方向**：X 属于哪个研究岛（DB AI 算子 / GPU 推理服务 / 分布式数据管线 / 结果持久化）？
2. **从文献提取候选方案**：精读组论文的核心技术/架构作为首选参考；补充组论文的关键机制作为对照或变体。
3. **对比与差异化**：已有方案的场景、边界、不足各是什么？你的场景与之有何不同？
4. **提出综合方案**：标注”从 A 借鉴 X，从 B 借鉴 Y，组合解决 C 未覆盖的问题”——每个设计决策必须有文献来源或实验数据支撑。

设计实验 baseline 时，同样优先从文献提取最优策略作为对照。完整 baseline 矩阵见 `experiments/plans/baseline_reference.md`。
