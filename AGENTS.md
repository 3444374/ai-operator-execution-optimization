...
3. 首次进入目录、查找入口或目录职责不明时，读取目标目录 `README.md`；
4. 按当前问题读取相关源码、计划、结果或 runbook 的对应部分。

会话中已提供或已读且未变化的内容直接复用；文件变化、上下文缺失或事实冲突时再补读。
局部文字、格式或链接修订只加载适用规则与受影响内容；涉及接口、研究判断或环境操作时，
继续读取对应领域材料。不要求每次编辑前重读整套文档。

子目录规则只增加该目录的职责、适用范围和验证要求。局部规则可以收紧操作，但不能放宽本文件的
研究范围、安全、证据和隐私要求。规则与事实冲突时，按“源码/原始结果 → 领域权威入口 →
`PROJECT_OUTLINE.md` → README/速览 → 历史计划”的顺序核对；先修错误的缓存文档，不把多个版本
长期并列为现役答案。

规则文件只写会改变 agent 行为的要求；目录内容、当前进度和文件清单写入 README 或索引。
...
例如修改 `experiments/plans/` 时，适用根、`experiments/`、`experiments/plans/` 三层规则；
其他子目录的规则不会因同在仓库内而全部加载。
...
必须先读以下材料；仅修订相关文字或分析已有结果时，读取受影响条款与证据即可，不触发环境操作：
...
2. **放置**：长期行为规则写入能覆盖目标范围的最窄 `AGENTS.md`；目录用途、文件清单和当前入口写入
   ...
   追加行为要求时才新增嵌套 `AGENTS.md`，普通文档不另建 rules/guide 作为并列规则源。
...
6. **验收**：对本次新增或改写内容执行 §6 的禁词搜索；检查 Markdown 本地链接和
