"""Shared generative Map identities and validation; transport versions bind this codec."""
...
_OPEN_FIELDS = (
    set(_OPEN_VALUES)
    | _IDENTITY_FIELDS
    | {"provider_execution_id", "model_id", "generation_constraints"}
)
_TASK_FIELDS = _IDENTITY_FIELDS | {
    "type",
    "protocol_version",
    "sequence",
...
_COMPLETION_FIELDS = _IDENTITY_FIELDS | {
    "type",
    "protocol_version",
    "sequence",
    "semantic_payload_digest",
...
def physical_algorithm_digest() -> str:
    return hashlib.sha256(
        b"semloom-physical-algorithm-v2\0" + _text(PHYSICAL_ALGORITHM) + _text(PHYSICAL_ROLE)
    ).hexdigest()
...
def semantic_payload_digest(
    *, semantic_spec_sha256: str, input_value: str, canonical_messages_utf8: bytes
) -> str:
    ...
    *,
    ...
    identity = b"".join(
        ...
        for value in (
            ...
            physical_algorithm_sha256,
            provider_execution_sha256,
            semantic_payload_sha256,
        )
    )
    return hashlib.sha256(
        b"semloom-completion-v5\0"
        + identity
        + _uint64(sequence)
        + _text(raw_output)
        + _text(finish_reason)
        + _text(response_model_id)
...
def validate_open(
    ...
    try:
        ...
        if any(
            not _matches(constraints[key], value)
            for key, value in plan.generation_constraints().items()
        ):
        ...
        return OpenContext(
            ...
            message["physical_algorithm_digest"],
            message["provider_execution_digest"],
            plan.model_id,
            plan.max_tokens,
        )
...
def validate_task(
    ...
    try:
        ...
        expected = semantic_payload_digest(
            semantic_spec_sha256=plan.digest,
            input_value=input_value,
...
def _completion_evidence(
    context: OpenContext, payload_digest: str, sequence: int, completion: Completion
) -> str:
    return completion_evidence_digest(
...
def _check_model_usage(context: OpenContext, completion: Completion) -> None:
    if (
        completion.response_model_id != context.model_id
        or completion.output_tokens > context.max_tokens
    ):
...
def validate_completion(
    ...
    *,
    ...
    try:
        ...
        sequence = decimal_uint64(message["sequence"])
        if sequence != expected_sequence or not _matches(
            message["semantic_payload_digest"], payload_digest
        ):
        ...
        completion = Completion(
            message["raw_output"],
            message["response_model_id"],
            decimal_uint64(message["prompt_tokens"]),
            decimal_uint64(message["output_tokens"]),
            message["finish_reason"],
        )
        length = validate_completion_values(completion)
        if not _matches(
            message["completion_evidence_digest"],
            _completion_evidence(open_context, payload_digest, sequence, completion),
        ):
