...
- 新增/改造语义算子、PG 注册/载体、生成请求/结果处理、共享执行或公司移植时，先读
  ...
  向公司移植”。完整对照覆盖 SQL/PG 接入、算子语义、请求与结果、资源和外部执行，不能仅核对多算子
  或 gateway；后续切片只重查受影响项。编码前在工程计划或切片记录中写明参考文件/符号与版本、已核对行为、
...
- 评审生产代码和测试代码时，以职责边界与可验证性而非统一行数阈值识别过长函数、魔法值、重复
  逻辑、过深嵌套和过长参数列表；按下列规则处理，不为达到机械指标拆分代码。
- 函数、类和文件保持单一职责；配置、阶段执行、扫描编排和输出分离。编排函数只保留可顺序阅读的
  生命周期，阶段细节提炼为按领域动作命名的函数；前置条件、空输入和失败分支优先用卫语句结束，
  领域流程确需嵌套时保留并命名该流程。
...
- 生产代码和测试代码遵守同一命名、单一职责与去重要求；测试 helper/fixture 保持最小作用域，参数化
  后仍能从用例名称和失败信息直接判断破坏了哪个合同。
