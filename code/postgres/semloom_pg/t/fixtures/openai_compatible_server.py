                ...
                "choices": [
                    {
                        "message": {
                            "role": "assistant",
                            "content": (request_value['messages'][-1]['content'] if self.server.echo_input
                                        else self.server.raw_outputs[output_index]),
                        },
                        "finish_reason": "stop",
                    }
                ],
...
def _valid_request(
    value: object, model_id: str, require_choice: bool = False, map_mode: bool = False
) -> bool:
    return (
        ...
        and (
            not require_choice
            or value.get("structured_outputs") == {"choice": ["TRUE", "FALSE", "UNKNOWN"]}
        )
...
def main() -> int:
    ...
    server.echo_input = args.echo_input
