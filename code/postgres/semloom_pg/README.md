...
`semloom_pg` is the first `REL_18_4` capability slice from
...
Build and test against the exact PostgreSQL version:
...
18.4 server. Set `PGHOST`, `PGPORT`, and `PGUSER` for that server as needed.
