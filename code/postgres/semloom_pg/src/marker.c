...
Datum
semloom_marker_filter(PG_FUNCTION_ARGS)
{
	ereport(ERROR,
			(errcode(ERRCODE_OBJECT_NOT_IN_PREREQUISITE_STATE),
			 errmsg("ai_semantic.filter marker was not lowered to a semantic plan"),
			 errhint("Load semloom_pg before planning and use a supported SemFilter query shape.")));
