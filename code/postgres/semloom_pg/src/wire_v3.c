/* Strict exact SemFilter wire-v3 schema above shared bounded framing. */
#include "postgres.h"
...
static void semloom_v3_hash_bytes(pg_cryptohash_ctx *context,
								  const void *data,
								  Size length);
static void semloom_v3_hash_uint32(pg_cryptohash_ctx *context, uint32 value);
static void semloom_v3_hash_uint64(pg_cryptohash_ctx *context, uint64 value);
static void semloom_v3_hash_slice(pg_cryptohash_ctx *context, AiByteSlice value);
static void semloom_v3_hash_finish(
...
static void semloom_v3_completion_digest(
	...
	char output[SEMLOOM_SHA256_HEX_LENGTH + 1]);
static bool semloom_v3_json_slice(Jsonb *message,
								 const char *key,
								 AiByteSlice *result,
								 AiProviderError *error);
static bool semloom_v3_json_uint64(Jsonb *message,
								  const char *key,
								  uint64 *result,
								  AiProviderError *error);
...
AiProviderStatus
semloom_wire_v3_open(pgsocket socket_fd,
					 const AiOpenSpec *spec,
					 const SemloomWireV3Identity *identity,
					 AiProviderError *error)
{
	...
	appendStringInfo(&request,
		...
		"\"provider_execution_id\":",
		SEMLOOM_WIRE_V3_PROTOCOL_VERSION,
	...
	if (!semloom_wire_common_json_int32(message,
										...
										&integer_value,
										error) ||
		integer_value != SEMLOOM_WIRE_V3_PROTOCOL_VERSION)
		goto mismatch;
...
#undef REQUIRE_IDENTITY
	...
	if (!semloom_wire_common_json_int32(message,
										...
										&integer_value,
										error) ||
		integer_value != SEMLOOM_WIRE_V3_MAX_FRAME_BYTES)
		goto mismatch;
	if (!semloom_wire_common_json_int32(message,
										"max_input_bytes",
										&integer_value,
										error) ||
		integer_value != SEMLOOM_WIRE_V3_MAX_INPUT_BYTES)
		goto mismatch;
...
AiProviderStatus
semloom_wire_v3_drive(pgsocket socket_fd,
					  const AiOpenSpec *spec,
					  ...
					  AiProviderError *error)
{
	...
	if (!semloom_v3_json_slice(message, "sequence", &response_sequence, error) ||
		!semloom_v3_slice_equals_cstring(response_sequence, sequence))
...
#define REQUIRE_IDENTITY(field, expected) \
	...
	REQUIRE_IDENTITY("provider_execution_digest", identity->provider_execution_digest);
#undef REQUIRE_IDENTITY
	if (!semloom_v3_json_slice(message, "semantic_payload_digest", &payload_digest, error) ||
		!semloom_v3_slice_equals(payload_digest, task->semantic_payload_digest) ||
		!semloom_v3_json_slice(message, "raw_output", &raw_output, error) ||
		!semloom_v3_json_slice(message, "response_model_id", &response_model, error) ||
		!semloom_v3_slice_equals(response_model, spec->model_id) ||
		!semloom_v3_json_uint64(message, "prompt_tokens", &prompt_tokens, error) ||
		!semloom_v3_json_uint64(message, "output_tokens", &output_tokens, error) ||
		!semloom_v3_json_slice(message, "finish_reason", &finish_reason, error) ||
		!semloom_v3_slice_equals_cstring(finish_reason, "stop") ||
		!semloom_v3_json_slice(message,
								 "completion_evidence_digest",
								 &evidence_digest,
								 error))
		goto mismatch_or_error;
	semloom_v3_completion_digest(identity,
								 payload_digest,
								 task->sequence,
								 raw_output,
								 finish_reason,
	...
	if (!semloom_v3_slice_equals_cstring(evidence_digest,
										 expected_evidence_digest))
		goto mismatch;
...
static bool
semloom_v3_json_slice(Jsonb *message,
						  const char *key,
						  AiByteSlice *result,
						  AiProviderError *error)
{
...
static bool
semloom_v3_json_uint64(Jsonb *message,
						   const char *key,
						   uint64 *result,
						   AiProviderError *error)
{
	...
	if (!semloom_v3_json_slice(message, key, &value, error))
		return false;
...
static bool
semloom_v3_slice_equals(AiByteSlice actual, AiByteSlice expected)
{
...
static bool
semloom_v3_slice_equals_cstring(AiByteSlice actual, const char *expected)
{
	...
	char output[SEMLOOM_SHA256_HEX_LENGTH + 1])
{
	...
	semloom_v3_hash_bytes(context,
						  SEMLOOM_PROVIDER_EXECUTION_DIGEST_DOMAIN,
						  sizeof(SEMLOOM_PROVIDER_EXECUTION_DIGEST_DOMAIN) - 1);
	semloom_v3_hash_uint32(context, SEMLOOM_WIRE_V3_PROTOCOL_VERSION);
	semloom_v3_hash_slice(context, execution_id);
	semloom_v3_hash_slice(context, spec->model_id);
	semloom_v3_hash_finish(context, output);
}

static void
semloom_v3_completion_digest(
	const SemloomWireV3Identity *identity,
	AiByteSlice payload_digest,
	uint64 sequence,
	AiByteSlice raw_output,
	...
	char output[SEMLOOM_SHA256_HEX_LENGTH + 1])
{
	...
	semloom_v3_hash_bytes(context,
						  identity->provider_execution_digest,
						  SEMLOOM_SHA256_HEX_LENGTH);
	semloom_v3_hash_bytes(context, payload_digest.data, payload_digest.length);
	semloom_v3_hash_uint64(context, sequence);
	semloom_v3_hash_slice(context, raw_output);
	semloom_v3_hash_slice(context, finish_reason);
	semloom_v3_hash_slice(context, response_model);
	semloom_v3_hash_uint64(context, prompt_tokens);
	semloom_v3_hash_uint64(context, output_tokens);
	semloom_v3_hash_finish(context, output);
}

static void
semloom_v3_hash_begin(pg_cryptohash_ctx **context)
{
...
static void
semloom_v3_hash_bytes(pg_cryptohash_ctx *context,
						  const void *data,
						  Size length)
{
...
static void
semloom_v3_hash_uint32(pg_cryptohash_ctx *context, uint32 value)
{
	uint8 bytes[4] = {
		(uint8) (value >> 24),
		(uint8) (value >> 16),
	...
	semloom_v3_hash_bytes(context, bytes, sizeof(bytes));
}

static void
semloom_v3_hash_uint64(pg_cryptohash_ctx *context, uint64 value)
{
	uint8 bytes[8];
	int shift;
...
static void
semloom_v3_hash_slice(pg_cryptohash_ctx *context, AiByteSlice value)
{
	semloom_v3_hash_uint32(context, value.length);
	semloom_v3_hash_bytes(context, value.data, value.length);
}

static void
semloom_v3_hash_finish(pg_cryptohash_ctx *context,
						   char output[SEMLOOM_SHA256_HEX_LENGTH + 1])
{
