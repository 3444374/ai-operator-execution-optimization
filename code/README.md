...
regression and TAP lifecycle/parity tests cover the current query shape, protocol drift, disconnect,
cancellation, Unicode, and `NULL`. The active sequence now proceeds to exact `SemFilter` and its minimal second
physical path. Accepted-prefix, multiple in-flight tasks, incremental SemLoom
